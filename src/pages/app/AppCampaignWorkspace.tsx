...
function PostCard({ p }: { p: any }) {
  return <div className="p-3 border rounded-md space-y-2"><div className="flex justify-between items-center"><Badge variant="outline">{p.platform}</Badge><div className="flex items-center"><SendToBufferDialog platform={p.platform} defaultText={p.long || p.short || ""} /><Button variant="ghost" size="sm" onClick={() => copy(p.long)}><Copy className="h-3 w-3" /></Button></div></div><p className="text-sm font-medium">{p.hook}</p><p className="text-xs text-muted-foreground whitespace-pre-wrap">{p.short}</p><p className="text-xs"><span className="font-semibold">CTA:</span> {p.cta}</p></div>;
}
