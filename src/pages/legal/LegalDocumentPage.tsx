import { useParams, Link } from "react-router-dom";
import { Helmet } from "react-helmet-async";
import { useState, useEffect, useRef } from "react";
import Navbar from "@/components/Navbar";
import Footer from "@/components/Footer";
import { motion } from "framer-motion";
import { ArrowLeft, ChevronDown, ChevronRight, Building2 } from "lucide-react";
import { cn } from "@/lib/utils";
import { Collapsible, CollapsibleContent, CollapsibleTrigger } from "@/components/ui/collapsible";

const PLATFORM = "Velocity Vision";
const ENTITY = "Global Solutions Management LLC, a company incorporated in the State of Delaware, United States, and operator of Velocity Vision";
const CONTACT_CTA = "Use the Contact page and select the route that matches your request.";

type LegalSection = {
  heading: string;
  paragraphs: string[];
  bullets?: string[];
};

type LegalDoc = {
  title: string;
  effectiveDate: string;
  lastUpdated: string;
  version: string;
  sections: LegalSection[];
};

const standardContact = (topic: string) => [
  `For ${topic}, ${CONTACT_CTA}`,
  `${ENTITY}.`,
];

const docs: Record<string, LegalDoc> = {
  "terms-of-service": {
    title: "Platform Terms of Service",
    effectiveDate: "30 June 2026",
    lastUpdated: "1 July 2026",
    version: "5.0",
    sections: [
      {
        heading: "1. Definitions",
        paragraphs: [
          "In these Terms:",
          "“Velocity Vision”, “Platform”, “Service”, “we”, “us” or “our” means the Velocity Vision platform operated by Global Solutions Management LLC.",
          "“Global Solutions Management LLC” means Global Solutions Management LLC, a company incorporated in the State of Delaware, United States.",
          "“Customer”, “you” or “your” means the person, company, agency, organization, partnership, charity, trust, public body, fund, brand, client, estate, project or other legal entity that accesses, purchases, subscribes to, creates, joins, administers or uses Velocity Vision.",
          "“User” means any person who accesses or uses Velocity Vision through, under or in connection with a Customer account, workspace, invitation, login, role, permission, integration, client workspace or agency workspace.",
          "“Workspace” means any account area, dashboard, Data Vault, client workspace, agency workspace, project space, business workspace or other environment made available through Velocity Vision.",
          "“Customer Data” means any data, file, record, list, note, instruction, prompt, content, contact, company record, recipient data, uploaded material, imported material, generated material, exported material, client material, sender information, billing information or workspace content provided, uploaded, imported, entered, generated, activated or used by or for Customer through Velocity Vision.",
          "“Generated Outputs” means AI-assisted or system-generated drafts, templates, outreach copy, social posts, press releases, video scripts, follow-up messages, campaign briefs, summaries, data observations, quality checks, reports, prompts, pipeline notes, translations, multilingual variants, recommendations or other materials produced through Velocity Vision.",
          "“Activation” means any customer-controlled action that sends, exports, publishes, schedules, prepares, triggers, connects, applies, downloads, moves, uses or otherwise operationalises Customer Data or Generated Outputs.",
          "“Protected Parties” means Global Solutions Management LLC and its present and former owners, members, managers, directors, officers, employees, contractors, consultants, agents, representatives, affiliates, licensors, suppliers, service providers, subprocessors, payment providers, infrastructure providers, AI providers, hosting providers, professional advisers, successors and assigns.",
        ],
      },
      {
        heading: "2. Business Use Only",
        paragraphs: [
          "Velocity Vision is provided for lawful business, professional, commercial, agency, organizational and entrepreneurial use only. It is not intended for personal, household or consumer use.",
          "By accessing or using Velocity Vision, you confirm that you are acting in a business, professional, commercial, agency, organizational or entrepreneurial capacity and not as a consumer.",
          "If you use Velocity Vision on behalf of a company, agency, partnership, client, charity, trust, public body, fund, estate, project, brand or other organization, you represent and warrant that you have authority to bind that organization to these Terms.",
          "If you do not agree to these Terms, or if you do not have authority to accept them, you must not access or use Velocity Vision.",
        ],
      },
      {
        heading: "3. No Consumer Use",
        paragraphs: [
          "Velocity Vision is not designed for consumer contracting, household use, personal messaging, personal address books, private family records, consumer credit activity or individual domestic use.",
          "If you access Velocity Vision in a jurisdiction where mandatory consumer laws may apply despite this business-use restriction, nothing in these Terms excludes rights that cannot legally be excluded. However, we may suspend, restrict or terminate any account that we reasonably believe is being used as a consumer account, personal account or household account.",
        ],
      },
      {
        heading: "4. Contract Formation and Electronic Acceptance",
        paragraphs: [
          "You accept these Terms by creating an account, clicking to accept, subscribing, purchasing credits, accessing a workspace, joining an invited workspace, using the platform, uploading data, generating outputs, activating features, connecting integrations or otherwise using Velocity Vision.",
          "Electronic acceptance, checkbox acceptance, account creation, continued use, checkout completion, payment, workspace access and in-platform acceptance each form a binding agreement to the maximum extent permitted by law.",
          "Our platform records, billing records, authentication records, usage logs, IP logs, email records, acceptance timestamps, workspace records and audit logs may be used as evidence of acceptance, access, use, activity, billing, activation and compliance events.",
        ],
      },
      {
        heading: "5. Service Provider and Platform Scope",
        paragraphs: [
          "Velocity Vision is an AI-powered commercial workspace designed to help businesses organize contact data, review data readiness, prepare AI-assisted outreach assets, manage templates, support governed activation, plan cadence, organize replies, support follow-up and move warm contacts into pipeline.",
          "Velocity Vision is software. It is not a law firm, compliance adviser, tax adviser, accountant, financial adviser, investment adviser, medical adviser, insurance adviser, broker, marketing agency, email service provider of record, lead-generation agency, managed campaign provider, data broker, records archive, regulated advisory service or guaranteed revenue service.",
          "Velocity Vision does not guarantee replies, leads, meetings, sales, revenue, funding, deliverability, inbox placement, domain reputation, sender reputation, legal compliance, regulatory compliance, campaign performance, commercial outcomes, investment outcomes or suitability for any particular business, industry, country, recipient group or use case.",
        ],
      },
      {
        heading: "6. Agreement and Document Hierarchy",
        paragraphs: [
          "These Terms apply together with all policies, notices, plan terms, checkout terms, product rules and agreements incorporated by reference, including where applicable:",
          "If there is a conflict between these Terms and another incorporated document, the more specific document applies to the relevant subject matter.",
          "If there is a conflict between these Terms and a separately signed written agreement expressly accepted by Global Solutions Management LLC, the signed written agreement controls only for the conflicting commercial term. All platform conduct, data, security, AI, outreach, acceptable use, compliance, suspension, liability, indemnity, confidentiality, no personal liability and enforcement provisions continue to apply unless expressly overridden in that signed written agreement.",
          "Customer purchase orders, procurement terms, vendor portal terms, customer policies, email footers, onboarding notes, unsigned documents, unilateral terms, tender documents or other customer-side terms do not modify these Terms unless expressly accepted in writing by Global Solutions Management LLC.",
          "No employee, contractor, chatbot, support agent, reseller, adviser, customer, agency or third party has authority to vary these Terms unless the variation is in a written agreement expressly accepted by Global Solutions Management LLC.",
        ],
        bullets: [
          "the Customer Agreement;",
          "the Data Processing Agreement;",
          "the Privacy Policy;",
          "the Acceptable Use Policy;",
          "the Marketing Compliance Policy;",
          "the Cookie Policy;",
          "the Platform Security Policy;",
          "the Service Level Agreement;",
          "product-specific rules;",
          "usage notices;",
          "checkout notices;",
          "credit rules;",
          "plan descriptions;",
          "in-product warnings;",
          "workspace notices;",
          "onboarding notices.",
        ],
      },
      {
        heading: "7. Changes to These Terms",
        paragraphs: [
          "We may update these Terms and incorporated policies from time to time to reflect product changes, legal developments, security requirements, operational needs, billing changes, third-party service changes, AI provider changes, infrastructure changes, market changes or changes to our business.",
          "Material updates may be notified by posting, email, checkout notice, website notice or in-platform notice.",
          "Continued access to or use of Velocity Vision after the effective date of updated terms means you accept the updated terms.",
          "If you do not agree to updated terms, you must stop using Velocity Vision and cancel any paid subscription before the next renewal date.",
        ],
      },
      {
        heading: "8. Accounts, Users and Authority",
        paragraphs: [
          "You must provide accurate account, workspace, billing, tax, contact and organization information and keep it up to date.",
          "You are responsible for all activity under your account, workspaces, users, permissions, invitations, payment methods, sender connections, integrations, credentials and client workspaces.",
          "This includes activity by employees, contractors, invited users, administrators, agencies, clients, consultants, representatives, outsourced providers, end clients and anyone using your login, device, access link, workspace, API connection, sender connection, payment method or integration.",
          "You must:",
          "We may treat all activity under your account, workspace, payment method, sender connection, integration or credentials as authorized by you unless and until you notify us of suspected compromise and we have had a reasonable opportunity to act.",
        ],
        bullets: [
          "keep login credentials secure;",
          "use strong passwords and appropriate authentication measures;",
          "manage internal permissions responsibly;",
          "promptly remove users who no longer require access;",
          "ensure every user complies with these Terms;",
          "maintain internal records of user authority;",
          "review account activity and workspace access;",
          "notify us promptly if you suspect unauthorized access, credential compromise, data misuse, payment abuse, security incident or platform misuse.",
        ],
      },
      {
        heading: "9. Account Ownership Disputes",
        paragraphs: [
          "If there is a dispute about account ownership, workspace control, administrator authority, payment authority, client ownership, user access or legal entitlement to data, we may freeze, restrict, suspend, preserve or disable the relevant account, workspace, export, billing access or user access until the dispute is resolved to our satisfaction.",
          "We are not required to decide internal disputes between shareholders, directors, partners, employees, agencies, clients, contractors or other parties. We may rely on billing records, administrator records, legal documents, court orders, company records, account history or other evidence we consider appropriate.",
          "We are not liable for loss arising from account freezes, access restrictions, export holds or administrative delays caused by ownership or authority disputes.",
        ],
      },
      {
        heading: "10. Minimum Age",
        paragraphs: [
          "You must be at least 18 years old, or the age of legal majority in your jurisdiction if higher, to create an account, administer a workspace, purchase a plan, upload data, manage campaigns, activate outputs or use Velocity Vision.",
          "You must not allow minors to access the platform, upload data, manage outreach, activate campaigns, access client records or use workspace features.",
        ],
      },
      {
        heading: "11. Platform Functionality",
        paragraphs: [
          "Velocity Vision may include tools for data upload, mapping, storage, deduplication, enrichment support, quality review, quality status assignment, segmentation, sender verification, activation checks, AI-assisted templates, outreach packs, social posts, press releases, video scripts, multilingual variants, cadence planning, reply organization, follow-up states, pipeline tracking, billing, credit usage, exports, reports, dashboards and integrations.",
          "Features may be added, changed, suspended, limited, renamed, redesigned, replaced or removed at any time.",
          "We may modify safety controls, usage limits, activation gates, quality checks, pricing logic, billing rules, credit consumption, interface flows, sender controls, verification steps, product labels, integrations, workspace limits or technical requirements to protect Velocity Vision, customers, users, recipients, senders, infrastructure, third-party providers, payment systems, legal compliance, platform reputation or our business.",
          "No product description, roadmap, beta feature, public page, help article, onboarding note, support response, marketing statement or sales statement creates an obligation to maintain any feature unless expressly stated in a signed written agreement accepted by Global Solutions Management LLC.",
        ],
      },
      {
        heading: "12. Self-Serve Software",
        paragraphs: [
          "Velocity Vision is a self-serve software platform. Customer is responsible for configuring, reviewing, approving, activating and using the platform.",
          "Unless expressly agreed in a separate signed written agreement, we do not provide managed campaigns, legal review, compliance sign-off, professional advice, manual prospecting, customer-side approval, human campaign operation, outsourced marketing services, guaranteed lead generation, guaranteed deliverability or agency services.",
          "Any support, guidance, examples, templates, help content, onboarding notes, AI outputs or recommendations are provided for general product-use assistance only and do not transfer responsibility from Customer to Velocity Vision.",
        ],
      },
      {
        heading: "13. Customer Data Responsibility",
        paragraphs: [
          "You are responsible for all Customer Data.",
          "This includes all data, files, records, lists, contacts, prompts, instructions, notes, tags, uploads, imports, pasted content, exported content, generated content, campaign materials, sender details, client materials, relationship data, company records, recipient information, billing information and workspace content that you or your users place into, create through, process through, activate through or export from Velocity Vision.",
          "You represent and warrant that you have all rights, permissions, notices, consents, lawful bases, contractual rights, regulatory permissions and internal authorizations required to collect, store, upload, import, process, analyse, enrich, segment, use, retain, export, transfer and contact the data you place into Velocity Vision.",
          "You must not upload or process:",
          "You are responsible for maintaining records of source, permission, consent, legitimate interest assessment, privacy notice, suppression status, opt-out history, contact restrictions, lawful basis and compliance evidence where required by law.",
        ],
        bullets: [
          "unlawfully obtained data;",
          "data collected by unlawful scraping;",
          "data collected without a lawful basis;",
          "purchased, rented or third-party lists that cannot lawfully be used;",
          "data about minors;",
          "sensitive personal data unless expressly authorized and legally permitted;",
          "health data;",
          "biometric data;",
          "genetic data;",
          "criminal offense data;",
          "financial account data;",
          "government identifier data;",
          "special category data;",
          "data subject to professional secrecy or privilege unless you are authorized to process it;",
          "data subject to contractual restrictions that prevent SaaS processing;",
          "data subject to regulatory restrictions that prevent transfer, storage, outreach or platform use;",
          "data that you are not authorized to transfer, store, process, export, retain or use.",
        ],
      },
      {
        heading: "14. Compliance Evidence",
        paragraphs: [
          "We may require Customer to provide evidence of lawful basis, consent, source, suppression handling, sender authority, recipient permission, client authority, data ownership, opt-out processing, campaign approval, regulated-sector permission or other compliance evidence where we reasonably believe there is legal, regulatory, security, payment, abuse, deliverability, reputational or operational risk.",
          "Failure to provide satisfactory evidence may result in suspension, restriction, export hold, activation hold, sender hold, account review or termination.",
          "We are not responsible for reviewing, validating or approving Customer’s compliance evidence, and any request for evidence does not make us responsible for Customer’s compliance.",
        ],
      },
      {
        heading: "15. Data Vault and Storage",
        paragraphs: [
          "The Data Vault is a workspace tool for organizing and using customer-provided business and contact data. It is not a compliance archive, legal record system, regulated records repository, certified backup system, litigation hold system, accounting record system, public filing system or guaranteed evidence store.",
          "You are responsible for keeping independent copies of important data, legal records, accounting records, compliance evidence, consent records, suppression lists, contracts, approvals, exports and business-critical files.",
          "We may apply storage limits, file-size limits, fair-use limits, retention rules, export limits, download limits, security restrictions, workspace limits, data-type restrictions and technical controls depending on your plan, usage, risk profile or platform requirements.",
          "We are not responsible for Customer’s failure to export, back up, secure, retain, evidence or preserve Customer Data.",
        ],
      },
      {
        heading: "16. Data Quality Review",
        paragraphs: [
          "Velocity Vision may include quality checks, readiness statuses, duplicate indicators, risky-record flags, missing-field prompts, segmentation suggestions, quality scores or AI-assisted observations.",
          "These are workflow aids only. They do not verify legal permission, consent, lawful basis, recipient status, accuracy, deliverability, commercial value, compliance, suitability or completeness of any data.",
          "Customer remains responsible for reviewing and validating all Customer Data before use, export, activation, outreach, publication or reliance.",
        ],
      },
      {
        heading: "17. Outreach, Activation and Sender Governance",
        paragraphs: [
          "Velocity Vision may provide sender verification, data quality flags, activation gates, cadence controls, risky-record checks, plan limits, sender rules, suppression prompts and governance controls. These tools are designed to support safer use, but they do not make your outreach lawful by themselves.",
          "You are solely responsible for deciding whether:",
          "Customer is solely responsible for all outreach activity, whether sent through Velocity Vision, exported from Velocity Vision, copied from Velocity Vision, adapted from Velocity Vision or prepared using Generated Outputs.",
          "We may block, throttle, pause, restrict, disable or review activation, exports, sender connections, campaigns, messages, workspaces or accounts where data, sender setup, recipient targeting, content, volume, complaints, bounce rates, spam signals, opt-out signals, payment risk, legal risk, technical risk or platform signals create concern.",
          "We are not responsible for sender reputation, domain reputation, blacklist status, inbox placement, bounce rates, spam classification, recipient complaints, opt-outs, platform enforcement, email provider action or third-party deliverability decisions.",
        ],
        bullets: [
          "each contact may lawfully be contacted;",
          "your lawful basis, consent, legitimate interest or other permission is valid;",
          "your message is lawful in the recipient’s jurisdiction;",
          "your sender identity is accurate and not misleading;",
          "your content is truthful, non-deceptive and substantiated;",
          "opt-out, unsubscribe, suppression and consent rules apply;",
          "your volume, frequency and targeting are appropriate;",
          "your outreach complies with applicable email, SMS, messaging, privacy, marketing, consumer protection, platform, industry and professional rules;",
          "your campaign complies with the rules of all relevant countries, states, provinces, regions and recipient locations.",
        ],
      },
      {
        heading: "18. Marketing Compliance",
        paragraphs: [
          "Customer is responsible for compliance with all laws, rules, regulations, platform terms and industry standards that apply to outreach, prospecting, lead generation, direct marketing, email, SMS, messaging, social outreach, telephone contact, retargeting, advertising, tracking and follow-up.",
          "This includes, where applicable, laws and rules relating to consent, legitimate interest, opt-outs, unsubscribe mechanisms, suppression lists, sender identity, subject lines, message content, physical address disclosure, recordkeeping, commercial claims, regulated sectors, vulnerable recipients, recipient location, cross-border outreach and agency-managed campaigns.",
          "Velocity Vision does not validate consent, approve lawful basis, certify recipient lists, guarantee compliance, provide legal clearance, monitor all messages, act as sender of record or assume responsibility for customer outreach.",
          "Customer must not use Velocity Vision to bypass, ignore, suppress, conceal or frustrate recipient rights, unsubscribe rights, opt-out requests, suppression obligations, platform rules or legal restrictions.",
        ],
      },
      {
        heading: "19. AI-Assisted Features, Templates and Generated Outputs",
        paragraphs: [
          "Velocity Vision includes AI-assisted tools that may help draft, structure, summarise, classify, translate, segment, organize, review, suggest or generate content and workflow actions.",
          "Generated Outputs may include outreach copy, email drafts, social posts, press releases, video scripts, follow-up messages, campaign briefs, multilingual variants, summaries, pipeline notes, data quality observations, prompts, reports, templates or other materials.",
          "All Generated Outputs are drafts. Customer must review, edit, approve and verify outputs before using, sending, publishing, exporting, relying on or activating them.",
          "Customer is responsible for ensuring that all Generated Outputs are:",
          "AI outputs may be incomplete, inaccurate, outdated, repetitive, biased, unsuitable, non-compliant, unsafe, offensive, misleading or similar to outputs generated for other users.",
          "We do not guarantee uniqueness, copyrightability, legal clearance, factual accuracy, commercial value, compliance, suitability, performance, deliverability or any outcome from Generated Outputs.",
          "Velocity Vision does not provide legal, tax, accounting, financial, medical, employment, investment, regulatory, insurance, professional or compliance advice. Do not rely on AI outputs as a substitute for qualified professional review.",
          "Customer must not use AI-assisted features to generate unlawful, deceptive, harmful, infringing, discriminatory, abusive, regulated, unsafe or misleading content.",
          "Subject to our Privacy Policy and Data Processing Agreement, we may use usage data, telemetry, error data, aggregated data, de-identified data and safety signals to improve product quality, reliability, abuse prevention, security and platform performance.",
        ],
        bullets: [
          "accurate;",
          "lawful;",
          "non-deceptive;",
          "non-infringing;",
          "appropriate for the recipient;",
          "appropriate for the jurisdiction;",
          "appropriate for the channel;",
          "appropriate for the industry;",
          "consistent with Customer’s brand, policies and approvals;",
          "compliant with all applicable laws, regulations, professional duties and platform rules.",
        ],
      },
      {
        heading: "20. AI Governance and Customer Control",
        paragraphs: [
          "Velocity Vision is designed around AI-powered assistance and customer-controlled activation.",
          "Customer remains in control before sending, exporting, publishing, activating or relying on Generated Outputs.",
          "Customer is responsible for:",
          "Customer must not represent that Velocity Vision has approved, reviewed, verified, legally cleared, compliance-checked or guaranteed any Generated Output, campaign, recipient list, data source, template, claim, message, deliverability result or commercial outcome.",
        ],
        bullets: [
          "prompt instructions;",
          "source data;",
          "review of outputs;",
          "approvals;",
          "edits;",
          "recipient selection;",
          "claims and substantiation;",
          "legal and regulatory review;",
          "tone and brand fit;",
          "sender identity;",
          "activation decisions;",
          "final use.",
        ],
      },
      {
        heading: "21. AI Legal Role Allocation",
        paragraphs: [
          "Customer is responsible for determining whether its use of Velocity Vision makes Customer a deployer, user, provider, distributor, importer, operator, controller, business, employer, regulated entity or other legal role under applicable AI, privacy, marketing, consumer protection, professional or sector-specific laws.",
          "Velocity Vision provides AI-assisted software tools and workflow support. Unless expressly agreed in writing, Global Solutions Management LLC does not assume Customer’s legal role, regulated status, professional obligations, deployment obligations, customer-facing obligations, human oversight obligations, impact assessment obligations, transparency obligations or recordkeeping obligations.",
          "Customer must not use Velocity Vision in any way that would make Global Solutions Management LLC responsible for Customer’s regulated AI system, high-risk AI deployment, automated decision system, profiling activity, employment decision, credit decision, housing decision, education decision, insurance decision, medical decision, legal decision or other regulated decision.",
        ],
      },
      {
        heading: "22. Regulated, High-Risk and Restricted Uses",
        paragraphs: [
          "Velocity Vision must not be used for regulated, high-risk, sensitive or legally restricted use unless Customer has all required authorizations and the use is expressly permitted under Customer’s plan and applicable written agreement.",
          "Restricted uses include, without limitation, uses involving:",
          "We may restrict or refuse any use that we reasonably consider high-risk, unlawful, unsafe, reputationally harmful, operationally burdensome, incompatible with the platform or unsuitable for Velocity Vision.",
        ],
        bullets: [
          "medical advice or diagnosis;",
          "legal advice;",
          "financial advice;",
          "investment recommendations;",
          "insurance underwriting;",
          "credit eligibility;",
          "employment decisions;",
          "housing decisions;",
          "education access decisions;",
          "criminal justice decisions;",
          "biometric identification;",
          "children’s data;",
          "sensitive personal data;",
          "political targeting;",
          "regulated financial promotions;",
          "regulated healthcare promotions;",
          "gambling;",
          "controlled goods or services;",
          "weapons;",
          "illegal goods or services;",
          "vulnerable-person targeting;",
          "emergency services;",
          "safety-critical systems.",
        ],
      },
      {
        heading: "23. Subscriptions, Credits, Billing and Taxes",
        paragraphs: [
          "Plans, subscriptions, credits, top-ups, billing cycles, included features, usage limits, currencies, renewal terms and applicable fees are shown at checkout, in billing settings, in product notices or in an applicable order form.",
          "By purchasing a plan, subscription, credit package or paid feature, you authorize us and our payment processors to charge the payment method provided for applicable fees, taxes, renewals, top-ups, overages and other charges.",
          "Unless expressly stated otherwise, subscriptions renew automatically until canceled. You are responsible for canceling before the renewal date if you do not want to renew.",
          "Campaign Credits are product-usage units for credit-priced AI generation. The current live credit-priced action is full campaign-pack generation. Credits are not charged per email sent, per contact activated, or for ordinary export. If additional actions become credit-priced in future, their credit cost must be disclosed in the product before consumption.",
          "Credits are not money, stored value, electronic money, gift cards or currency. Credits have no cash value, are not transferable, are not redeemable for cash and may expire, be limited, be withdrawn or be subject to plan rules.",
          "Stored data may remain available when credits run out, but chargeable actions may pause until credits are added, the plan is upgraded or billing is restored.",
          "Unless required by law or expressly stated in writing by Global Solutions Management LLC, fees, subscriptions and credit purchases are non-refundable once the paid period begins, credits are made available or services are activated.",
          "You are responsible for all taxes, VAT, sales tax, use tax, withholding, duties, levies, exchange-rate effects, bank fees, card fees, foreign transaction fees and payment-provider charges.",
          "Prices may be displayed in one or more currencies. Currency conversion tools, estimates or localized price displays are provided for convenience only. The final amount charged may depend on checkout currency, payment processor rates, card issuer rates, taxes, location, exchange-rate changes and payment-provider charges.",
          "We may suspend, restrict or terminate access for failed payments, expired payment methods, chargebacks, payment disputes, fraud signals, overdue invoices, tax issues or billing abuse.",
        ],
      },
      {
        heading: "24. Payment Recovery and Collection",
        paragraphs: [
          "Customer is responsible for all fees, taxes, usage charges, renewal charges, overages, payment-provider fees, chargeback fees, collection costs and legal costs incurred in recovering unpaid amounts to the maximum extent permitted by law.",
          "We may use internal or external collection processes, payment processors, legal advisers or debt recovery providers to recover unpaid amounts.",
          "Suspension, termination, non-use, failure to activate, failure to export, non-deliverability, campaign failure, customer-side configuration issues or dissatisfaction with Generated Outputs does not remove Customer’s obligation to pay fees already incurred.",
        ],
      },
      {
        heading: "25. Chargebacks and Payment Disputes",
        paragraphs: [
          "You must contact us promptly if you believe there is a billing error. You agree to work with us in good faith before initiating a chargeback, payment reversal, payment dispute or card dispute.",
          "Unfounded, abusive, fraudulent or unresolved chargebacks may result in suspension, termination, recovery action, loss of credits, account restrictions and refusal of future service.",
          "We may retain account, billing, usage and transaction records as reasonably necessary to investigate payment disputes, prevent fraud, comply with law and enforce these Terms.",
          "Customer remains responsible for fees, taxes and charges incurred before suspension or termination.",
        ],
      },
      {
        heading: "26. Trials, Promotions, Beta Features and Pre-Release Tools",
        paragraphs: [
          "We may offer trials, previews, promotions, beta features, experimental features, early-access tools or pre-release functionality.",
          "These are provided for evaluation only, may be changed or withdrawn at any time and may be subject to additional limits or terms.",
          "Beta and experimental features may be incomplete, unstable, inaccurate, unavailable, insecure, unsupported or unsuitable for production use. They are provided as-is and excluded from any service level commitments unless expressly stated otherwise.",
          "We may terminate, limit or withdraw any trial, promotion, beta feature or early-access tool without liability.",
        ],
      },
      {
        heading: "27. Agency, Client and Multi-Workspace Use",
        paragraphs: [
          "Agency Workspace users may operate multiple client, campaign, brand or business workspaces from one account where their plan permits it.",
          "The agency account holder remains solely responsible for:",
          "Velocity Vision does not enter into a contract with an agency’s end clients unless separately agreed in writing by Global Solutions Management LLC.",
          "Agency users must not represent that Velocity Vision has approved, reviewed, endorsed, guaranteed, legally cleared or compliance-cleared any client campaign, data source, outreach list, message, template, Generated Output, sender setup, deliverability result, recipient group or activation.",
          "The agency account holder is liable for all end-client activity under the agency account and must indemnify the Protected Parties for claims arising from client workspaces, client data, client outreach, client instructions, agency-managed use or end-client disputes.",
          "You may not resell, white-label, sublicense, rent, lease, broker, provide bureau access to, provide service-provider access to or commercially exploit Velocity Vision for third parties unless your plan expressly permits it or we agree in writing.",
        ],
        bullets: [
          "client relationships;",
          "client permissions;",
          "client data;",
          "client privacy notices;",
          "client lawful basis;",
          "client approvals;",
          "workspace configuration;",
          "sender setup;",
          "suppression lists;",
          "campaign content;",
          "Generated Outputs;",
          "activation decisions;",
          "exports;",
          "billing;",
          "user access;",
          "compliance obligations;",
          "client disputes;",
          "end-client instructions;",
          "local-law obligations.",
        ],
      },
      {
        heading: "28. Prohibited Use",
        paragraphs: [
          "You must comply with the Acceptable Use Policy and must not use Velocity Vision for unlawful, harmful, deceptive, abusive, exploitative, high-risk, infringing or platform-damaging activity.",
          "You must not:",
          "We may determine, in our reasonable judgment, whether conduct violates this section.",
        ],
        bullets: [
          "send spam or unlawful unsolicited messaging;",
          "use deceptive headers, misleading subject lines or false sender identities;",
          "impersonate any person, brand, company or authority;",
          "hide, falsify or manipulate sender identity;",
          "bypass unsubscribe, opt-out, suppression or contact restriction requirements;",
          "upload unlawfully obtained data;",
          "upload data without required rights or lawful basis;",
          "use the platform for phishing, credential theft, malware, fraud, scams or social engineering;",
          "reverse engineer, scrape, copy, benchmark, probe, scan or security test the platform without permission;",
          "interfere with platform infrastructure, rate limits, billing systems, safety controls or access controls;",
          "bypass sender verification, data-quality holds, activation gates, plan limits, suppression controls or compliance prompts;",
          "generate or distribute unlawful, defamatory, discriminatory, harassing, hateful, exploitative, sexually abusive, violent, infringing or deceptive content;",
          "use the platform for illegal goods or services;",
          "target vulnerable persons unlawfully or exploitatively;",
          "conduct regulated activity without required authorization;",
          "upload malicious code or unsafe files;",
          "overload, degrade or abuse the platform;",
          "create multiple accounts to avoid limits, suspension, billing, risk controls or enforcement;",
          "misrepresent Velocity Vision’s role in any campaign, output, result or client work;",
          "use Velocity Vision in a way that exposes us, our providers, customers, recipients, infrastructure or third parties to legal, regulatory, reputational, operational, financial or security risk.",
        ],
      },
      {
        heading: "29. Competitor, Benchmarking and Model Training Restrictions",
        paragraphs: [
          "Customer must not use Velocity Vision to build, train, fine-tune, benchmark, evaluate, test or improve any competing product, AI model, dataset, platform, workflow, template library, automation system, commercial database or outreach tool without our prior written consent.",
          "Customer must not scrape, copy, extract, monitor, harvest or use platform content, prompts, templates, workflows, outputs, interface patterns, scoring logic, safety logic, product logic or system behavior for competitive analysis, model training, dataset creation, product replication, reverse engineering or benchmarking.",
        ],
      },
      {
        heading: "30. Sanctions, Export Control and Restricted Territories",
        paragraphs: [
          "You must comply with all applicable sanctions, export control, trade control, anti-bribery, anti-corruption and anti-money laundering laws.",
          "You must not use Velocity Vision if you are located in, organized under the laws of, ordinarily resident in, owned or controlled by, or acting on behalf of any person or entity subject to sanctions, embargoes or trade restrictions that would prohibit access to the platform.",
          "You must not use Velocity Vision to support prohibited transactions, restricted parties, sanctioned entities, embargoed territories, unlawful exports, unlawful re-exports or unlawful circumvention of trade controls.",
          "You represent and warrant that you and your users are not prohibited from using Velocity Vision under applicable sanctions, export control or trade laws.",
          "We may restrict, suspend or terminate access where sanctions, export control, trade compliance or payment risk is identified or suspected.",
        ],
      },
      {
        heading: "31. Intellectual Property",
        paragraphs: [
          "Global Solutions Management LLC and its licensors own the Velocity Vision platform, software, interfaces, workflows, systems, product logic, templates, prompts, safety controls, designs, documentation, branding, know-how, data models, scoring logic, processes and all related intellectual property.",
          "You receive a limited, revocable, non-exclusive, non-transferable, non-sublicensable right to access and use Velocity Vision in accordance with these Terms, your plan and applicable policies.",
          "You retain ownership of your Customer Data and customer-provided content.",
          "You grant us the rights necessary to host, store, process, transmit, display, secure, back up, analyse and operate Customer Data and customer-provided content as needed to provide, maintain, secure, support and improve Velocity Vision, subject to the Privacy Policy and Data Processing Agreement.",
          "You must not copy, reproduce, modify, resell, sublicense, frame, mirror, scrape, extract, reverse engineer, decompile, disassemble, replicate or create derivative works from the platform, workflows, templates, interface, product logic, scoring logic, prompts, safety controls or systems except as expressly permitted.",
        ],
      },
      {
        heading: "32. Generated Output Ownership and Use",
        paragraphs: [
          "As between you and Global Solutions Management LLC, and to the extent permitted by law and subject to your compliance with these Terms, you may use Generated Outputs produced for you through Velocity Vision for your lawful business purposes.",
          "We do not guarantee that Generated Outputs are unique, protectable, copyrightable, registrable, non-infringing, accurate, compliant or free from third-party rights.",
          "Similar or identical outputs may be generated for other users.",
          "You are responsible for reviewing, clearing, approving and lawfully using Generated Outputs before use.",
          "Platform templates, system prompts, workflow structures, product logic, interface design, scoring logic, safety controls, platform processes and underlying technology remain owned by Global Solutions Management LLC or its licensors and are not transferred to you.",
        ],
      },
      {
        heading: "33. Takedown and Content Removal",
        paragraphs: [
          "We may remove, disable, restrict, quarantine or block any Customer Data, Generated Output, file, campaign, message, sender connection, integration, export or workspace content that we reasonably believe may violate law, infringe rights, create risk, breach these Terms, expose the platform to liability or harm third parties.",
          "We are not liable for loss arising from removal, blocking, disabling, quarantine, moderation, restriction or refusal of content, outputs, campaigns or data where we act to protect the platform, users, recipients, third-party providers, rights holders, regulators or our business.",
        ],
      },
      {
        heading: "34. Feedback",
        paragraphs: [
          "If you provide ideas, suggestions, requests, improvements, comments or other feedback about Velocity Vision, you grant Global Solutions Management LLC a worldwide, perpetual, irrevocable, royalty-free, sublicensable and transferable right to use, modify, commercialise and incorporate that feedback without restriction, obligation or compensation.",
        ],
      },
      {
        heading: "35. Third-Party Services and Integrations",
        paragraphs: [
          "Velocity Vision may rely on, connect to or integrate with third-party providers for hosting, authentication, payments, analytics, email connectivity, AI processing, data storage, monitoring, communications, translation, CRM tools, domain services, DNS, CDNs, logging, support tools or other infrastructure.",
          "Third-party services are not controlled by Global Solutions Management LLC. They may have their own terms, policies, pricing, security practices, outages, rate limits, account restrictions, suspensions, data practices and compliance requirements.",
          "You authorize us to transmit, process and receive data through third-party services as reasonably necessary to provide the platform, operate integrations, process payments, support AI-assisted features and maintain security.",
          "We are not responsible for third-party service failures, outages, errors, policy changes, pricing changes, data handling, deliverability decisions, payment interruptions, email provider restrictions, account suspensions, integration failures or external platform behavior outside our reasonable control.",
          "You are responsible for complying with the terms and policies of any third-party services you connect to or use with Velocity Vision.",
          "We may disconnect, suspend, restrict or disable third-party integrations at any time where required by law, provider rules, technical constraints, security concerns, abuse signals, payment issues or operational risk.",
        ],
      },
      {
        heading: "36. Privacy and Data Processing",
        paragraphs: [
          "Our Privacy Policy explains how we collect and use personal information about website visitors, platform users, prospects, customers, billing contacts, support contacts and individuals who interact with us directly.",
          "The Data Processing Agreement governs personal data that customers upload and that we process as a processor, service provider or equivalent role on Customer’s behalf.",
          "Customer is responsible for determining whether Customer acts as controller, business, processor, service provider, joint controller, independent controller or another legal role under applicable law.",
          "Customer is responsible for providing all required privacy notices to its own contacts, customers, prospects, recipients, employees, contractors, end clients and users.",
          "Customer is responsible for responding to legal rights requests from its own contacts, customers, prospects and end clients unless applicable law requires us to respond directly.",
          "Customer must not use Velocity Vision to avoid, bypass or frustrate privacy, marketing, data protection, platform or recipient rights.",
        ],
      },
      {
        heading: "37. International Data Transfers",
        paragraphs: [
          "Velocity Vision may be operated from, supported from, hosted in or accessed from multiple jurisdictions, including the United States and other countries where we or our service providers operate.",
          "Customer is responsible for ensuring that Customer Data may lawfully be transferred to, accessed from, processed in and stored in all relevant jurisdictions.",
          "Where required by applicable data protection law, international transfers may be governed by the Data Processing Agreement, applicable standard contractual clauses, transfer addenda, supplementary measures or other lawful transfer mechanisms.",
          "Customer must not upload or process data through Velocity Vision where transfer, access, processing or storage would breach applicable law, contract, professional duty, regulatory obligation or data subject restriction.",
        ],
      },
      {
        heading: "38. Regulatory, Legal and Third-Party Requests",
        paragraphs: [
          "If we receive a subpoena, court order, regulator request, law enforcement request, data subject request, rights-holder complaint, payment-provider request, email-provider request, third-party platform request, security complaint or legal notice relating to Customer, Customer Data, Customer’s users, Customer’s outreach, Customer’s clients or Customer’s workspace, we may respond, disclose information, preserve records, restrict access, notify Customer or take other action as we consider legally required or reasonably appropriate.",
          "Customer is responsible for costs, losses, fees, penalties, legal fees and operational burden arising from requests, complaints, investigations or claims caused by Customer Data, Customer outreach, Customer instructions, Customer users, Customer clients, Customer integrations or Customer’s use of Velocity Vision.",
          "We are not required to challenge any legal, regulatory, payment-provider, email-provider or third-party request unless required by law or expressly agreed in writing.",
        ],
      },
      {
        heading: "39. Monitoring, Review and Enforcement",
        paragraphs: [
          "We may, but are not required to, monitor, scan, log, review, analyse, investigate, preserve or restrict accounts, workspaces, Customer Data, Generated Outputs, usage patterns, sender connections, integrations, activation activity, exports, billing activity and platform signals to operate the platform, improve safety, prevent abuse, enforce these Terms, comply with law, respond to complaints, protect infrastructure, protect recipients, protect third-party providers and protect our business.",
          "No monitoring, review, automated check, manual check, warning, flag, hold, approval flow or failure to act creates any responsibility for Customer’s data, outputs, campaigns, compliance, outreach or legal obligations.",
          "We may use automated systems, human review, third-party tools and risk scoring to identify suspected abuse, fraud, unlawful activity, security risk, policy breach or platform risk.",
        ],
      },
      {
        heading: "40. Security and Customer Responsibilities",
        paragraphs: [
          "We use commercially reasonable safeguards designed to protect the platform. However, no system is completely secure, error-free or immune from attack.",
          "Customer is responsible for:",
          "We are not responsible for losses caused by weak passwords, shared credentials, compromised devices, unsafe exports, unauthorized internal users, insecure customer systems, customer-side misconfiguration, customer-side malware, user error, customer integrations or third-party account compromise.",
        ],
        bullets: [
          "securing its devices, networks, browsers and accounts;",
          "protecting passwords, credentials, API keys and integration access;",
          "managing workspace permissions;",
          "reviewing exports and downloads;",
          "securing data after export;",
          "ensuring users are trained and authorized;",
          "maintaining backups of important data;",
          "applying appropriate internal policies;",
          "promptly notifying us of suspected compromise.",
        ],
      },
      {
        heading: "41. Confidentiality",
        paragraphs: [
          "You must not disclose non-public information about Velocity Vision, including security controls, platform logic, private roadmap information, beta features, pricing not publicly available, technical processes, non-public documentation, non-public product plans, vulnerability information or other confidential information, except as required to use the platform or as required by law.",
          "This section does not prevent you from using your own Customer Data or Generated Outputs in accordance with these Terms.",
        ],
      },
      {
        heading: "42. Responsible Disclosure and Security Testing",
        paragraphs: [
          "You must not conduct vulnerability scanning, penetration testing, load testing, scraping, probing, automated testing, reverse engineering, credential testing, exploit testing or security research on Velocity Vision without our prior written permission.",
          "If you discover a suspected vulnerability, you must report it responsibly and must not exploit it, access data, disrupt systems, disclose it publicly, retain data, copy data or use it for any purpose other than responsible reporting.",
          "We may suspend or terminate any account involved in unauthorized security testing or platform interference.",
        ],
      },
      {
        heading: "43. Suspension and Termination",
        paragraphs: [
          "We may suspend, restrict, throttle, disable, delete or terminate access to any account, workspace, user, sender, activation, export, integration, feature, plan or credit balance if we reasonably believe there is:",
          "We may act immediately and without prior notice where necessary to protect Velocity Vision, customers, users, recipients, infrastructure, third-party providers, payment systems, AI systems, sender systems, legal compliance, platform integrity or our business.",
          "After termination, you may lose access to your workspace, Generated Outputs, Customer Data, reports, integrations and credits.",
          "We may retain limited records where required or reasonably necessary for legal, tax, audit, security, billing, dispute, fraud prevention, compliance, enforcement or legitimate business purposes.",
        ],
        bullets: [
          "legal risk;",
          "security risk;",
          "payment failure;",
          "chargeback risk;",
          "fraud risk;",
          "platform abuse;",
          "policy breach;",
          "data misuse;",
          "deliverability risk;",
          "complaint activity;",
          "regulatory concern;",
          "sanctions risk;",
          "harmful content;",
          "unlawful outreach;",
          "infrastructure risk;",
          "AI misuse;",
          "third-party provider risk;",
          "reputational risk;",
          "risk to us, customers, recipients, third-party providers or the public.",
        ],
      },
      {
        heading: "44. Data Export and Deletion",
        paragraphs: [
          "Where available, you may export certain Customer Data during an active subscription, subject to plan limits, technical limits, security checks, payment status and policy compliance.",
          "We are not required to provide exports after termination, suspension, non-payment, chargeback, legal restriction, security concern, account closure or platform enforcement, except where required by law or expressly agreed in writing.",
          "Deletion requests are handled in accordance with the Privacy Policy, Data Processing Agreement and applicable law.",
          "Some records may be retained where required or reasonably necessary for legal, tax, audit, security, billing, dispute, fraud prevention, compliance or enforcement purposes.",
        ],
      },
      {
        heading: "45. Service Levels",
        paragraphs: [
          "Any service level, support target, uptime target or response target applies only where expressly stated in the Service Level Agreement or a signed written agreement accepted by Global Solutions Management LLC.",
          "Unless expressly agreed otherwise, service levels do not apply to:",
          "Service credits, if any, are the sole and exclusive remedy for failure to meet an applicable service level and are subject to the terms, exclusions and claim process in the Service Level Agreement.",
        ],
        bullets: [
          "beta features;",
          "trials;",
          "free plans;",
          "third-party services;",
          "customer systems;",
          "customer integrations;",
          "customer-side misconfiguration;",
          "internet issues;",
          "email provider issues;",
          "AI provider issues;",
          "payment provider issues;",
          "hosting provider issues;",
          "force majeure events;",
          "external service events;",
          "misuse;",
          "unsupported use;",
          "suspended accounts;",
          "accounts with overdue payments;",
          "regulatory restrictions;",
          "sanctions restrictions;",
          "maintenance;",
          "security interventions;",
          "deliverability outcomes;",
          "outreach results;",
          "Generated Outputs.",
        ],
      },
      {
        heading: "46. Disclaimers",
        paragraphs: [
          "Velocity Vision is provided on an “as is” and “as available” basis to the maximum extent permitted by law.",
          "We do not warrant that the platform will be uninterrupted, error-free, secure against every threat, compatible with every system, available in every location, suitable for every jurisdiction, suitable for every industry or capable of meeting every customer requirement.",
          "We do not warrant or guarantee:",
          "No information provided through Velocity Vision is professional advice. You should obtain appropriate legal, tax, accounting, financial, regulatory, technical or professional advice where needed.",
        ],
        bullets: [
          "data-quality scoring;",
          "sender verification results;",
          "Generated Outputs;",
          "AI suggestions;",
          "reply classification;",
          "pipeline values;",
          "profitability estimates;",
          "export formatting;",
          "integrations;",
          "translations;",
          "campaign performance;",
          "deliverability;",
          "inbox placement;",
          "sender reputation;",
          "domain reputation;",
          "replies;",
          "meetings;",
          "sales;",
          "funding;",
          "revenue;",
          "legal compliance;",
          "regulatory compliance;",
          "recipient response;",
          "business outcomes;",
          "avoidance of spam filters;",
          "avoidance of complaints;",
          "avoidance of regulatory inquiry.",
        ],
      },
      {
        heading: "47. Limitation of Liability",
        paragraphs: [
          "To the maximum extent permitted by law, the total aggregate liability of the Protected Parties arising out of or relating to Velocity Vision, these Terms or any incorporated policy is limited to the amounts paid by Customer to Global Solutions Management LLC for the platform during the twelve months immediately before the event giving rise to the claim.",
          "If Customer used Velocity Vision for free or paid no fees during that period, the total aggregate liability of the Protected Parties is limited to USD 100 to the maximum extent permitted by law.",
          "To the maximum extent permitted by law, the Protected Parties are not liable for indirect, incidental, special, consequential, exemplary, enhanced or punitive damages, or for loss of profits, revenue, goodwill, opportunity, data, use, contracts, customers, business interruption, deliverability, sender reputation, domain reputation, anticipated savings, business value, reputation, wasted expenditure, replacement services, regulatory exposure, loss of access, loss of credits or loss arising from customer-side use.",
          "The limitations apply whether the claim is based on contract, tort, negligence, strict liability, statute, equity or any other legal theory, even if a Protected Party has been advised of the possibility of such damages.",
          "Customer agrees that the limitations of liability are an essential basis of the bargain and apply even if any limited remedy fails of its essential purpose.",
          "Nothing in these Terms excludes or limits liability where liability cannot lawfully be excluded or limited.",
        ],
      },
      {
        heading: "48. No Personal Liability of Owners, Managers, Directors, Officers or Other Protected Parties",
        paragraphs: [
          "To the maximum extent permitted by law, Customer agrees that it will bring any claim relating to Velocity Vision, these Terms, Customer’s account, Customer’s workspace, Customer Data, Generated Outputs, billing, suspension, termination, outreach activity or platform use only against Global Solutions Management LLC and not against any owner, member, manager, director, officer, employee, contractor, consultant, agent, representative, affiliate, licensor, supplier, service provider, subprocessor, payment provider, infrastructure provider, AI provider, hosting provider or professional adviser personally.",
          "Customer waives, to the maximum extent permitted by law, any right to seek personal liability from any Protected Party other than Global Solutions Management LLC for acts or omissions connected with Velocity Vision, except where such waiver is prohibited by mandatory law.",
        ],
      },
      {
        heading: "49. Indemnity",
        paragraphs: [
          "Customer agrees to defend, indemnify and hold harmless the Protected Parties from and against all claims, demands, investigations, complaints, proceedings, losses, liabilities, damages, penalties, fines, settlements, costs and expenses, including reasonable legal fees, arising from or related to:",
          "We may control the defence of any matter subject to indemnity. Customer must cooperate with us and must not settle any claim in a way that imposes obligations, admissions, restrictions or liability on any Protected Party without our prior written consent.",
        ],
        bullets: [
          "Customer’s account;",
          "Customer’s users;",
          "Customer’s workspaces;",
          "Customer Data;",
          "uploaded content;",
          "prompts;",
          "Generated Outputs;",
          "outreach activity;",
          "sender identity;",
          "contact lists;",
          "client activity;",
          "agency workspaces;",
          "end-client instructions;",
          "Customer’s breach of these Terms;",
          "Customer’s breach of law;",
          "infringement or alleged infringement of third-party rights;",
          "unlawful marketing;",
          "data protection violations;",
          "privacy violations;",
          "suppression or opt-out failures;",
          "regulated activity;",
          "customer-side systems;",
          "third-party integrations connected by Customer;",
          "payment disputes;",
          "chargebacks;",
          "tax obligations;",
          "regulatory requests;",
          "law enforcement requests;",
          "recipient complaints;",
          "data subject requests;",
          "misuse of the platform.",
        ],
      },
      {
        heading: "50. Force Majeure and External Service Events",
        paragraphs: [
          "Global Solutions Management LLC will not be liable for any delay, failure, interruption, suspension, loss, degradation, unavailability, data processing delay, delivery failure, support delay, billing disruption, feature limitation or non-performance caused by events, circumstances or conditions beyond its reasonable control.",
          "Force Majeure Events include, without limitation:",
          "Where a Force Majeure Event occurs, we may take any action reasonably necessary to protect Velocity Vision, customers, users, recipients, infrastructure, data, third-party providers, payment systems, sender systems, platform integrity or our business. This may include suspending, limiting, delaying, disabling, modifying, throttling, rerouting or terminating access to any account, workspace, feature, sender connection, integration, activation, export, AI-assisted function, billing function or support channel.",
          "Force Majeure Events do not create any right to a refund, credit, service credit, damages, compensation, chargeback, termination payment or extension unless expressly required by law or expressly stated in a separate signed written agreement accepted by Global Solutions Management LLC.",
          "Force Majeure Events do not relieve Customer of payment obligations for fees already incurred, subscriptions already activated, credits already made available, taxes, chargebacks, usage charges, indemnity obligations, confidentiality obligations, data compliance obligations, acceptable use obligations, outreach compliance obligations or obligations that by their nature should continue.",
          "We are not required to continue providing the platform, any feature, any integration, any sender connection, any AI-assisted function, any activation capability or any third-party-dependent service where doing so would create legal, regulatory, security, financial, operational, reputational, sanctions, payment, infrastructure, deliverability or third-party provider risk.",
          "If a Force Majeure Event continues for more than 30 days, we may terminate, suspend, restructure, migrate, modify or discontinue any affected account, workspace, plan, integration, feature or service without liability.",
          "We will use commercially reasonable efforts, where practical and lawful, to reduce the impact of a Force Majeure Event. However, we are not required to accept substitute performance, alternative payment methods, alternative technical arrangements, additional legal risk, increased operational burden, third-party policy breaches, security risk, sanctions risk or non-contractual performance in order to overcome a Force Majeure Event.",
          "This section applies in addition to, and does not limit, our rights under the sections on Third-Party Services, Suspension and Termination, Disclaimers, Limitation of Liability, Service Levels, Security and Customer Responsibilities.",
        ],
        bullets: [
          "acts of God, natural disasters, fire, flood, storm, earthquake, extreme weather, epidemic, pandemic, public health emergency or quarantine restriction;",
          "war, invasion, armed conflict, terrorism, sabotage, civil unrest, riot, insurrection, national emergency or military action;",
          "government action, regulatory action, court order, sanctions, export controls, import restrictions, embargoes, trade restrictions, law changes, enforcement action, licensing restrictions or orders from any public authority;",
          "interruption, failure, degradation or restriction of internet infrastructure, telecommunications networks, data centres, cloud providers, hosting providers, CDN providers, DNS providers, domain registrars, email providers, payment processors, banking networks, card networks, authentication providers, analytics providers, AI providers, API providers, CRM providers, translation providers, monitoring providers or other third-party services;",
          "third-party platform changes, API changes, rate limits, account restrictions, policy changes, technical restrictions, suspension decisions, deliverability decisions, spam filtering, blacklist events, sender reputation issues, domain reputation issues, email provider enforcement or external platform behavior outside our reasonable control;",
          "cyberattack, denial-of-service attack, malware, ransomware, credential compromise, security incident, vulnerability exploitation, unauthorized access, infrastructure attack or malicious third-party activity;",
          "labor disputes, strikes, lockouts, shortages, supply-chain failures, utility failures, power failures, hardware failures, software defects, emergency maintenance or inability to obtain necessary services, equipment, personnel, permissions, licenses or infrastructure;",
          "payment disruption, chargeback network disruption, banking failure, processor outage, fraud controls, tax authority restriction, currency restriction, foreign exchange restriction or inability to process payments through ordinary channels;",
          "any other event, circumstance or cause beyond our reasonable control, whether foreseeable or unforeseeable, that prevents, delays, restricts, degrades or materially increases the burden, risk or cost of providing the platform.",
        ],
      },
      {
        heading: "51. No Guarantee of Global Availability",
        paragraphs: [
          "Velocity Vision may be accessed from multiple countries, but not all features, plans, data types, integrations, outreach activities, AI functions, payment methods, templates or activation tools are lawful, available or appropriate in every jurisdiction.",
          "We may restrict, geoblock, suspend, modify or refuse access to Velocity Vision or any feature in any country, region, state, province, territory or sector where we reasonably believe there is legal, regulatory, sanctions, technical, payment, security, operational or reputational risk.",
          "Customer is responsible for ensuring that its use of Velocity Vision is lawful in every jurisdiction relevant to Customer, its users, its data, its recipients, its senders, its clients, its campaigns and its business.",
        ],
      },
      {
        heading: "52. Local Law and Mandatory Rights",
        paragraphs: [
          "These Terms apply to the maximum extent permitted by applicable law.",
          "If mandatory law in a jurisdiction gives Customer rights that cannot be excluded, restricted or modified, these Terms do not exclude those rights, but all exclusions, limitations, disclaimers, indemnities and protections continue to apply to the maximum extent permitted by that law.",
          "Where a provision cannot be enforced as written in a particular jurisdiction, it will be interpreted, narrowed or reformed only to the minimum extent necessary to make it enforceable, and the rest of the provision and these Terms remain effective.",
        ],
      },
      {
        heading: "53. No Third-Party Beneficiaries",
        paragraphs: [
          "These Terms are for the benefit of Customer, Global Solutions Management LLC and the Protected Parties.",
          "No end client, recipient, contact, prospect, agency client, user, customer customer, supplier or other third party has rights under these Terms unless expressly stated in a signed written agreement accepted by Global Solutions Management LLC.",
        ],
      },
      {
        heading: "54. No Fiduciary, Partnership or Agency Relationship",
        paragraphs: [
          "Nothing in these Terms creates a partnership, joint venture, fiduciary relationship, employment relationship, franchise, agency relationship or representative authority between Customer and Global Solutions Management LLC.",
          "Customer has no authority to bind Global Solutions Management LLC or represent that Velocity Vision has approved, endorsed, reviewed, guaranteed or cleared any Customer activity.",
        ],
      },
      {
        heading: "55. Injunctive Relief",
        paragraphs: [
          "Customer agrees that breach of provisions relating to intellectual property, confidentiality, platform misuse, security abuse, unlawful access, reverse engineering, misuse of data, unlawful disclosure, competitor misuse, model training misuse or prohibited use may cause irreparable harm.",
          "We may seek injunctive or equitable relief without needing to prove actual damages or post a bond, where permitted by law.",
        ],
      },
      {
        heading: "56. Assignment",
        paragraphs: [
          "Customer may not assign, transfer or delegate these Terms, Customer’s account, workspace, plan, credits or rights to use Velocity Vision without our prior written consent.",
          "We may assign or transfer these Terms, accounts, customer relationships, data processing arrangements and platform operations in connection with a merger, acquisition, restructuring, financing, sale of assets, corporate reorganization, transfer of business, change of control or platform migration.",
        ],
      },
      {
        heading: "57. Governing Law and Jurisdiction",
        paragraphs: [
          "These Terms are governed by the laws of the State of Delaware, United States, without regard to conflict-of-law principles.",
          "Subject to any mandatory law that cannot be waived, the state and federal courts located in Delaware have exclusive jurisdiction over disputes arising out of or relating to these Terms or Velocity Vision.",
          "Customer and Global Solutions Management LLC each waive any objection to jurisdiction, venue or inconvenient forum in those courts.",
          "Despite the above, Global Solutions Management LLC may seek injunctive relief, debt recovery, payment enforcement, intellectual property enforcement, confidentiality enforcement, security enforcement or urgent protective relief in any court or forum of competent jurisdiction.",
        ],
      },
      {
        heading: "58. Optional Arbitration at Our Election",
        paragraphs: [
          "To the maximum extent permitted by law, Global Solutions Management LLC may elect to have any dispute, claim or controversy arising out of or relating to Velocity Vision, these Terms, Customer’s account, Customer’s workspace, Customer Data, Generated Outputs, payment, suspension or termination resolved by confidential binding arbitration instead of court proceedings.",
          "If arbitration is elected, the arbitration will take place in Delaware, in English, before a single arbitrator, under commercially reasonable arbitration rules selected by Global Solutions Management LLC, unless mandatory law requires otherwise.",
          "Nothing in this section prevents Global Solutions Management LLC from seeking injunctive relief, debt recovery, enforcement of intellectual property rights, confidentiality protection, security protection or urgent protective relief in court.",
        ],
      },
      {
        heading: "59. Class Action and Jury Trial Waiver",
        paragraphs: [
          "To the maximum extent permitted by law, Customer and Global Solutions Management LLC agree that disputes will be resolved only on an individual basis and not as part of any class, collective, consolidated, representative or group action.",
          "To the maximum extent permitted by law, Customer and Global Solutions Management LLC waive any right to a jury trial in any dispute arising out of or relating to these Terms or Velocity Vision.",
        ],
      },
      {
        heading: "60. Limitation Period",
        paragraphs: [
          "To the maximum extent permitted by law, any claim arising out of or relating to Velocity Vision, these Terms, a workspace, billing, credits, Generated Outputs, Customer Data, outreach activity, suspension or termination must be brought within one year after the event giving rise to the claim.",
          "Claims brought after that period are permanently barred, unless a longer period is required by mandatory law.",
        ],
      },
      {
        heading: "61. Notices",
        paragraphs: [
          "We may provide notices by posting on the website, posting in the platform, sending email to the account contact, sending billing notices, displaying checkout notices or using other reasonable communication methods.",
          "Customer is responsible for keeping contact details current and monitoring notices.",
          "Legal notices to Global Solutions Management LLC must be submitted through the Contact page using the route that matches the request, unless a separate signed written agreement specifies another notice method.",
        ],
      },
      {
        heading: "62. Severability",
        paragraphs: [
          "If any provision of these Terms is held invalid, unlawful or unenforceable, the remaining provisions remain in full force.",
          "The invalid provision will be interpreted, narrowed or replaced to achieve the original intent as closely as permitted by law.",
        ],
      },
      {
        heading: "63. No Waiver",
        paragraphs: [
          "Failure to enforce any provision of these Terms is not a waiver of that provision or any other provision.",
          "Any waiver must be in writing and expressly accepted by Global Solutions Management LLC.",
        ],
      },
      {
        heading: "64. Entire Agreement",
        paragraphs: [
          "These Terms and all incorporated documents form the entire agreement between Customer and Global Solutions Management LLC regarding access to and use of Velocity Vision, unless a separate signed written agreement expressly states otherwise.",
          "Customer acknowledges that it has not relied on any statement, representation, promise, assurance, forecast, projection, demo, example, output, estimate or marketing statement not expressly included in these Terms or a signed written agreement accepted by Global Solutions Management LLC.",
        ],
      },
      {
        heading: "65. Interpretation",
        paragraphs: [
          "Headings are for convenience only and do not affect interpretation.",
          "Words such as “including”, “includes”, “for example” and “without limitation” are illustrative and do not limit the words that follow.",
          "References to law include amendments, replacements and successor laws.",
          "References to Customer include Customer’s users, employees, contractors, agents, agencies, clients and representatives where the context allows.",
          "Any rule of interpretation that ambiguity is interpreted against the drafting party does not apply to these Terms.",
        ],
      },
      {
        heading: "66. Survival",
        paragraphs: [
          "Any provision that by its nature should survive suspension, termination, expiry or account closure will survive, including provisions relating to ownership, Customer Data responsibility, payment obligations, taxes, confidentiality, prohibited use, privacy, data processing, disclaimers, limitation of liability, no personal liability, indemnity, dispute resolution, governing law, class action waiver, jury trial waiver, records retention and enforcement.",
        ],
      },
      {
        heading: "67. Contact",
        paragraphs: [
          "For legal notices, questions about these Terms or policy-related requests, use the Contact page and select the route that matches your request.",
          "Global Solutions Management LLC Delaware, United States Operator of Velocity Vision",
        ],
      },
    ],
  },
  "client-services-agreement": {
    title: "Customer Agreement",
    effectiveDate: "30 June 2026",
    lastUpdated: "1 July 2026",
    version: "6.0",
    sections: [
      {
        heading: "Overview",
        paragraphs: [
          `Platform: ${PLATFORM}. Operator: ${ENTITY}.`,
          `This Customer Agreement governs paid plans, subscriptions, credits, top-ups, paid workspaces, agency workspaces, billing, commercial use, support access and related paid features of the ${PLATFORM} platform.`,
          `This Customer Agreement supplements the Platform Terms of Service. By purchasing, subscribing to, renewing, upgrading, topping up, accessing or using any paid feature of ${PLATFORM}, Customer agrees to this Customer Agreement, the Platform Terms of Service and all incorporated policies.`,
          `This document may be displayed in your browser's preferred language using an automated machine-translation layer, including GTranslate, Google Translate or similar translation technology. Translations are provided for convenience only. The English version controls if there is any conflict, ambiguity, inconsistency, error or difference between versions.`,
        ],
      },
      {
        heading: "1. Definitions",
        paragraphs: [
          `In this Customer Agreement:`,
          `"Velocity Vision", "Platform", "Service", "we", "us" or "our" means the ${PLATFORM} platform operated by Global Solutions Management LLC.`,
          `"Global Solutions Management LLC" means Global Solutions Management LLC, a company incorporated in the State of Delaware, United States.`,
          `"Customer", "you" or "your" means the person, company, agency, organization, partnership, charity, trust, public body, fund, brand, client, estate, project or other legal entity that purchases, subscribes to, renews, upgrades, tops up, administers or uses any paid ${PLATFORM} feature.`,
          `"User" means any person who accesses or uses ${PLATFORM} through, under or in connection with Customer's account, workspace, invitation, login, role, permission, integration, client workspace or agency workspace.`,
          `"Workspace" means any account area, dashboard, Data Vault, client workspace, agency workspace, project space, business workspace or other environment made available through ${PLATFORM}.`,
          `"Plan" means a paid subscription, package, workspace tier, agency tier, credit package, top-up, promotional package, trial conversion, founder plan, annual plan, monthly plan, custom plan or other commercial access arrangement offered by ${PLATFORM}.`,
          `"Credits" means platform usage units used for chargeable actions such as AI-assisted generation, outreach packs, social posts, press releases, video scripts, follow-up assets, multilingual variants, data preparation, processing, exports, activation or other chargeable platform actions.`,
          `"Customer Data" means any data, file, record, list, note, instruction, prompt, content, contact, company record, recipient data, uploaded material, imported material, generated material, exported material, client material, sender information, billing information or workspace content provided, uploaded, imported, entered, generated, activated or used by or for Customer through ${PLATFORM}.`,
          `"Generated Outputs" means AI-assisted or system-generated drafts, templates, outreach copy, social posts, press releases, video scripts, follow-up messages, campaign briefs, summaries, data observations, quality checks, reports, prompts, pipeline notes, translations, multilingual variants, recommendations or other materials produced through ${PLATFORM}.`,
          `"Activation" means any customer-controlled action that sends, exports, publishes, schedules, prepares, triggers, connects, applies, downloads, moves, uses or otherwise operationalises Customer Data or Generated Outputs.`,
          `"Protected Parties" means Global Solutions Management LLC and its present and former owners, members, managers, directors, officers, employees, contractors, consultants, agents, representatives, affiliates, licensors, suppliers, service providers, subprocessors, payment providers, infrastructure providers, AI providers, hosting providers, professional advisers, successors and assigns.`,
        ],
      },
      {
        heading: "2. Agreement Purpose",
        paragraphs: [
          `This Customer Agreement applies to Customers who purchase, subscribe to, renew, upgrade, top up, access or use paid features of ${PLATFORM}.`,
          `This is a software and workspace agreement. It does not create an agency retainer, managed marketing service, outsourced campaign service, professional advisory engagement, legal compliance engagement, tax engagement, financial advisory engagement, deliverability consulting engagement, data brokerage engagement, email service provider-of-record relationship or revenue guarantee.`,
          `${PLATFORM} is self-serve software. Customer is responsible for configuring, reviewing, approving, activating and lawfully using the platform.`,
          `No managed service, professional service, legal review, compliance sign-off, human campaign review, customer-side approval, deliverability consulting, marketing agency service, advisory engagement or outsourced business service is included unless expressly stated in a separate signed written statement of work accepted by Global Solutions Management LLC.`,
        ],
      },
      {
        heading: "3. Relationship with Platform Terms",
        paragraphs: [
          `This Customer Agreement supplements and does not replace the Platform Terms of Service.`,
          `The Platform Terms of Service apply to all access, accounts, workspaces, users, data, AI-assisted features, Generated Outputs, acceptable use, outreach activity, privacy, data processing, third-party services, suspension, disclaimers, limitations of liability, indemnities, governing law, no personal liability, dispute terms and enforcement.`,
          `If there is a conflict between this Customer Agreement and the Platform Terms of Service, this Customer Agreement controls only for the specific commercial billing, Plan, Credit, renewal or subscription issue. The Platform Terms of Service continue to control all platform conduct, data, security, AI, outreach, acceptable use, compliance, suspension, liability, indemnity, confidentiality, no personal liability and enforcement matters unless expressly overridden in a signed written agreement accepted by Global Solutions Management LLC.`,
          `Customer purchase orders, procurement terms, vendor portal terms, customer policies, customer contract templates, email footers, onboarding notes, unsigned documents, unilateral terms, tender documents or other customer-side terms do not modify this Customer Agreement unless expressly accepted in writing by Global Solutions Management LLC.`,
          `No employee, contractor, chatbot, support agent, reseller, adviser, customer, agency or third party has authority to vary this Customer Agreement unless the variation is in a written agreement expressly accepted by Global Solutions Management LLC.`,
        ],
      },
      {
        heading: "4. Business Use Only",
        paragraphs: [
          `${PLATFORM} is provided for lawful business, professional, commercial, agency, organizational and entrepreneurial use only. It is not intended for personal, household or consumer use.`,
          `By purchasing or using a paid Plan, Customer confirms that it is acting in a business, professional, commercial, agency, organizational or entrepreneurial capacity and not as a consumer.`,
          `If Customer purchases or uses ${PLATFORM} on behalf of a company, agency, partnership, client, charity, trust, public body, fund, estate, project, brand or other organization, the person accepting this Customer Agreement represents and warrants that they have authority to bind that organization.`,
          `If Customer does not agree to this Customer Agreement, or if the person accepting does not have authority to bind Customer, Customer must not purchase, subscribe to, access or use paid features of ${PLATFORM}.`,
        ],
      },
      {
        heading: "5. No Consumer Use",
        paragraphs: [
          `${PLATFORM} is not designed for consumer contracting, household use, personal messaging, personal address books, private family records, consumer credit activity or individual domestic use.`,
          `If Customer accesses ${PLATFORM} in a jurisdiction where mandatory consumer laws may apply despite this business-use restriction, nothing in this Customer Agreement excludes rights that cannot legally be excluded. However, we may suspend, restrict or terminate any account that we reasonably believe is being used as a consumer account, personal account or household account.`,
        ],
      },
      {
        heading: "6. Contract Formation and Electronic Acceptance",
        paragraphs: [
          `Customer accepts this Customer Agreement by creating an account, clicking to accept, subscribing, purchasing a Plan, purchasing Credits, topping up, renewing, upgrading, accessing a paid Workspace, joining an invited Workspace, using paid features, uploading data, generating outputs, activating features, connecting integrations or otherwise using ${PLATFORM}.`,
          `Electronic acceptance, checkbox acceptance, checkout completion, payment, renewal, Workspace access and continued use each form a binding agreement to the maximum extent permitted by law.`,
          `Our platform records, billing records, checkout records, payment processor records, authentication records, usage logs, IP logs, email records, acceptance timestamps, Workspace records and audit logs may be used as evidence of acceptance, access, use, billing, renewal, Credit usage, activation, cancellation, compliance events and enforcement decisions.`,
          `Customer agrees that electronic records and platform logs are admissible and reliable evidence to the maximum extent permitted by law.`,
        ],
      },
      {
        heading: "7. Plans and Workspace Access",
        paragraphs: [
          `A Plan may include Workspace access, Data Vault usage, templates, AI-assisted generation, governed activation, cadence planning, reply organization, follow-up support, pipeline tools, reporting, exports, agency workspaces, support access, Credits and other platform features.`,
          `Available features, pricing, currencies, limits, support levels, Credit allocations, renewal terms and usage rights may differ by Plan, region, account status, payment status, risk profile, checkout configuration, promotional offer or written order.`,
          `Starter, Growth, Agency Workspace, founder, promotional, annual, monthly, beta, trial and custom Plans may have different rights, restrictions, limits and pricing.`,
          `Workspace access is subject to successful payment, verification, compliance with all platform policies and our right to suspend or restrict access under this Customer Agreement and the Platform Terms of Service.`,
          `We may change, rename, restructure, replace, withdraw or limit Plans at any time. Changes may apply immediately where required for legal, security, payment, provider, technical or operational reasons, or at renewal where commercially appropriate.`,
          `No product description, pricing page, support message, public page, roadmap, screenshot, onboarding note or help article guarantees that any Plan, price, feature, Credit allocation, integration or support level will remain available indefinitely.`,
        ],
      },
      {
        heading: "8. Plan Limits and Fair Use",
        paragraphs: [
          `Plans may include limits on users, workspaces, client workspaces, data storage, uploads, exports, Credits, generated assets, AI-assisted actions, activation volume, sender connections, campaigns, support access, integrations, file sizes, API usage, rate limits or other usage measures.`,
          `Customer must not attempt to bypass Plan limits, fair-use limits, usage controls, billing rules, activation gates, safety controls, sender controls, verification controls or Credit consumption logic.`,
          `We may apply, modify or enforce fair-use limits where usage is excessive, abusive, technically burdensome, commercially unreasonable, inconsistent with Plan design, harmful to the platform, harmful to third-party providers or materially increases our cost of providing the Service.`,
          `We may throttle, restrict, suspend, require upgrade, charge additional fees or terminate access where Customer exceeds Plan limits or uses the platform in a way that creates unreasonable technical, legal, security, operational, provider, payment, AI-cost or reputational risk.`,
        ],
      },
      {
        heading: "9. Credits, Top-Ups and Usage",
        paragraphs: [
          `Credits are platform usage units. Credits are not cash balances, bank deposits, stored-value instruments, electronic money, gift cards, prepaid cards, securities, financial products, refundable currency or legal tender.`,
          `Credits may be consumed by AI-assisted generation, outreach packs, social posts, press releases, video scripts, follow-up assets, multilingual variants, data preparation, processing, activation, exports or other chargeable platform actions depending on the Plan and product configuration.`,
          `Credit consumption may vary by action type, feature, file size, data volume, output type, language, AI intensity, integration, campaign type, Workspace configuration, safety checks, retry logic, third-party provider requirements or future product changes.`,
          `Unused, expired, promotional, test, founder, bonus or goodwill Credits may be subject to Plan-specific rules and may expire, be withdrawn, be limited, be corrected or be removed.`,
          `Credits have no cash value, are not transferable, are not redeemable for cash, are not refundable unless expressly required by law and cannot be exchanged outside the platform.`,
          `We may correct obvious Credit errors, pricing errors, duplicate grants, fraudulent grants, abuse, miscalculations, technical errors, promotional misuse or billing mistakes.`,
          `Stored data may remain available when Credits run out, but chargeable actions may pause until Credits are added, billing is restored or the Plan is upgraded.`,
        ],
      },
      {
        heading: "10. AI-Heavy Actions",
        paragraphs: [
          `Campaign Credits may power AI-heavy actions across the Workspace, including outreach packs, social posts, press releases, video scripts, follow-up assets, campaign variants, data preparation and multilingual versions.`,
          `Customer acknowledges that Credit consumption for AI-assisted features may depend on third-party AI provider costs, infrastructure costs, model selection, output volume, translation requirements, retry logic, safety checks, file size, data complexity and product configuration.`,
          `We may change Credit consumption rules, Plan allowances, AI action pricing or feature availability to reflect infrastructure cost, AI provider changes, abuse prevention, legal risk, safety controls, product changes or commercial viability.`,
          `Credits provide access to platform actions and AI-assisted generation. They do not guarantee replies, meetings, sales, revenue, deliverability, campaign performance, legal compliance, recipient response, output accuracy or commercial outcome.`,
        ],
      },
      {
        heading: "11. Payment Authorization",
        paragraphs: [
          `By purchasing, subscribing, renewing, upgrading, topping up or using paid features, Customer authorizes Global Solutions Management LLC and its payment processors to charge the payment method provided for subscriptions, renewals, upgrades, top-ups, overages, taxes, payment-provider charges and other applicable fees.`,
          `Customer must provide accurate billing, payment, tax, company and contact information and keep it up to date.`,
          `Customer represents and warrants that it is authorized to use the payment method provided and that all payments are lawful.`,
          `We may use third-party payment processors. Payment processors may have their own terms, privacy notices, risk controls, fraud checks, account restrictions, processing delays, currency conversion rates and fees.`,
          `We are not responsible for payment processor outages, payment failures, bank declines, card issuer decisions, foreign exchange charges, card fees, banking fees, payment network issues or payment provider restrictions outside our reasonable control.`,
        ],
      },
      {
        heading: "12. Pricing, Currency and Taxes",
        paragraphs: [
          `Prices may be displayed in one or more currencies. Currency conversion tools, estimates or localized price displays are provided for convenience only.`,
          `The final amount charged may depend on checkout currency, payment processor rates, card issuer rates, bank rates, taxes, location, exchange-rate changes and payment-provider charges.`,
          `Customer is responsible for all taxes, VAT, GST, sales tax, use tax, withholding, duties, levies, assessments, bank fees, card fees, foreign transaction fees, exchange-rate effects and payment-provider charges unless we are legally required to collect them.`,
          `If Customer is required by law to withhold tax from payments, Customer must gross up the payment so that Global Solutions Management LLC receives the full amount that would have been received without withholding, unless prohibited by mandatory law.`,
          `We may collect taxes where required or where we reasonably determine collection is appropriate. Tax treatment may depend on Customer's location, billing details, tax status, usage, local law and payment processor configuration.`,
          `Customer is responsible for providing valid tax registration details, exemption certificates or other tax information where applicable.`,
        ],
      },
      {
        heading: "13. Renewals",
        paragraphs: [
          `Unless expressly stated otherwise at checkout or in a signed written agreement, subscription Plans renew automatically according to the billing cycle shown at purchase.`,
          `Customer authorizes us and our payment processors to charge the payment method on file for renewal fees, taxes and applicable charges.`,
          `Customer is responsible for canceling before the renewal date if it does not want to renew.`,
          `Failure to use the platform, failure to activate campaigns, dissatisfaction with results, non-deliverability, non-use of Credits, change of business plans, user error, customer-side configuration problems, customer data issues or lack of commercial outcome does not cancel renewal obligations or create a refund right.`,
        ],
      },
      {
        heading: "14. Cancellation",
        paragraphs: [
          `Customer may cancel a subscription through the available billing settings or by any cancellation method we expressly make available.`,
          `Cancellation stops future renewal but does not automatically refund past charges, current billing periods, used Credits, unused Credits, top-ups, setup fees, annual fees, promotional fees or fees for services already activated, unless required by law or expressly stated in writing by Global Solutions Management LLC.`,
          `Access may continue until the end of the paid period unless immediate termination, suspension, downgrade or restriction is required for non-payment, chargeback, fraud, legal risk, security risk, provider risk, sanctions risk, policy breach or platform protection.`,
          `If a Plan is canceled, downgraded or paused, access to features, Credits, activation, exports, users, agency workspaces, integrations, stored data or support may change at the end of the paid period or earlier where permitted under this Customer Agreement or the Platform Terms of Service.`,
        ],
      },
      {
        heading: "15. Refunds",
        paragraphs: [
          `Unless required by law or expressly stated in writing by Global Solutions Management LLC, all fees, subscriptions, renewals, top-ups, Credits, annual Plans, monthly Plans, promotional packages, founder packages, agency packages, setup fees and paid features are non-refundable once the paid period begins, Credits are made available, access is activated or services are used.`,
          `Refunds are not provided for:`,
        ],
        bullets: [
          "failure to use the platform;",
          "unused Credits;",
          "used Credits;",
          "expired Credits;",
          "cancellation after renewal;",
          "customer-side configuration issues;",
          "failure to generate desired outputs;",
          "dissatisfaction with Generated Outputs;",
          "lack of replies;",
          "lack of leads;",
          "lack of meetings;",
          "lack of sales;",
          "lack of revenue;",
          "deliverability outcomes;",
          "sender reputation issues;",
          "email provider restrictions;",
          "third-party outages;",
          "customer data quality issues;",
          "unlawful or unusable customer data;",
          "suspension caused by Customer;",
          "termination caused by Customer;",
          "breach of this Customer Agreement or platform policies.",
          "Where a refund is granted as a goodwill gesture, it does not create a future entitlement, waiver, admission, obligation or precedent.",
        ],
      },
      {
        heading: "16. Failed Payments, Overdue Amounts and Collection",
        paragraphs: [
          `If payment fails, is declined, reversed, disputed, charged back or becomes overdue, we may suspend, restrict, downgrade, terminate or block access to the account, Workspace, Credits, exports, activation, integrations, agency workspaces and paid features.`,
          `Customer remains responsible for all fees, taxes, usage charges, renewal charges, overages, payment-provider fees, chargeback fees, collection costs and legal costs incurred in recovering unpaid amounts to the maximum extent permitted by law.`,
          `We may use internal or external collection processes, payment processors, legal advisers or debt recovery providers to recover unpaid amounts.`,
          `Suspension, termination, non-use, non-deliverability, customer-side configuration issues or dissatisfaction with Generated Outputs does not remove Customer's obligation to pay fees already incurred.`,
          `Customer may not withhold payment, set off amounts, reverse payment or initiate a chargeback because of a dispute, complaint, alleged claim or dissatisfaction, except where mandatory law gives Customer a non-waivable right.`,
        ],
      },
      {
        heading: "17. Chargebacks and Payment Disputes",
        paragraphs: [
          `Customer must contact us promptly if it believes there is a billing error. Customer agrees to work with us in good faith before initiating a chargeback, payment reversal, payment dispute or card dispute.`,
          `Unfounded, abusive, fraudulent or unresolved chargebacks may result in suspension, termination, recovery action, loss of Credits, account restrictions and refusal of future service.`,
          `We may retain account, billing, usage, checkout, payment and transaction records as reasonably necessary to investigate payment disputes, prevent fraud, comply with law and enforce this Customer Agreement.`,
          `Customer remains responsible for fees, taxes and charges incurred before suspension or termination.`,
        ],
      },
      {
        heading: "18. Trials, Promotions and Founder Offers",
        paragraphs: [
          `We may offer trials, previews, promotions, discounts, founder offers, bonus Credits, goodwill Credits, beta access or early-access tools.`,
          `These offers are discretionary, may be limited, may be withdrawn, may expire, may be changed and may be subject to additional terms.`,
          `Trial, founder, promotional, bonus, test or goodwill Credits have no cash value and may be removed, corrected, limited or withdrawn where there is abuse, error, non-payment, cancellation, expiry or policy breach.`,
          `Promotional pricing may apply only for the period stated. After the promotional period, standard pricing may apply unless Customer cancels before renewal.`,
        ],
      },
      {
        heading: "19. Beta, Preview and Experimental Features",
        paragraphs: [
          `Some features may be in beta, preview, founder release, limited release, early access or experimental mode.`,
          `Beta and experimental features may be incomplete, unstable, inaccurate, unavailable, insecure, unsupported, rate-limited, changed or withdrawn at any time.`,
          `Beta and experimental features are provided as-is and are excluded from service level commitments, uptime commitments, support targets and refund rights unless expressly stated otherwise.`,
          `Customer should not rely on beta or experimental features for business-critical, regulated, legal, compliance, financial, safety-critical or high-risk use.`,
        ],
      },
      {
        heading: "20. Customer Obligations",
        paragraphs: [
          `Customer is responsible for Customer Data, instructions, configuration, users, senders, clients, Generated Outputs, approval decisions, activation decisions, follow-up actions, pipeline records, exports, integrations and use of the platform.`,
          `Customer must:`,
        ],
        bullets: [
          "ensure all uploaded data can lawfully be processed and used for the intended purpose;",
          "maintain lawful basis, consent, permission, legitimate interest or other required justification where applicable;",
          "provide privacy notices where required;",
          "maintain suppression lists, unsubscribe records and opt-out records where required;",
          "review Generated Outputs before use;",
          "ensure sender identity is accurate and not misleading;",
          "ensure outreach content is lawful, truthful and substantiated;",
          "comply with anti-spam, privacy, data protection, advertising, consumer protection, platform, sector-specific, AI, export control, sanctions and local laws;",
          "maintain internal approvals and evidence where required;",
          "supervise all users and client workspaces;",
          "prevent misuse of the platform.",
          "Customer must not use Velocity Vision to bypass legal obligations, recipient rights, opt-outs, unsubscribe requests, suppression lists, platform rules, sender controls, safety controls, Credit rules or Plan limits.",
        ],
      },
      {
        heading: "21. Global Outreach Compliance",
        paragraphs: [
          `Customer is solely responsible for compliance with all laws, rules, regulations, regulatory guidance, platform terms and industry standards that apply to outreach, prospecting, lead generation, direct marketing, email, SMS, messaging, social outreach, telephone contact, retargeting, advertising, tracking and follow-up in every relevant jurisdiction.`,
          `This includes, where applicable:`,
        ],
        bullets: [
          "consent requirements;",
          "legitimate interest requirements;",
          "soft opt-in requirements;",
          "opt-out and unsubscribe requirements;",
          "sender identification requirements;",
          "physical address requirements;",
          "suppression-list requirements;",
          "subject-line and header requirements;",
          "commercial-claim substantiation;",
          "recipient-location rules;",
          "sector-specific marketing rules;",
          "vulnerable-recipient rules;",
          "business-to-business marketing rules;",
          "business-to-consumer marketing rules;",
          "agency-managed campaign rules;",
          "platform and provider rules.",
          `${PLATFORM} does not validate consent, approve lawful basis, certify recipient lists, guarantee compliance, provide legal clearance, monitor all messages, act as sender of record or assume responsibility for Customer outreach.`,
          `Customer must not use ${PLATFORM} to bypass, ignore, suppress, conceal or frustrate recipient rights, unsubscribe rights, opt-out requests, suppression obligations, platform rules or legal restrictions.`,
        ],
      },
      {
        heading: "22. Compliance Evidence",
        paragraphs: [
          `We may require Customer to provide evidence of lawful basis, consent, source, suppression handling, sender authority, recipient permission, client authority, data ownership, opt-out processing, campaign approval, regulated-sector permission or other compliance evidence where we reasonably believe there is legal, regulatory, security, payment, abuse, deliverability, reputational or operational risk.`,
          `Failure to provide satisfactory evidence may result in suspension, restriction, export hold, activation hold, sender hold, account review, termination or refusal of future service.`,
          `Any request for evidence does not make us responsible for Customer's compliance and does not mean that we approve, validate or certify Customer's data, campaign, outreach, sender setup or Generated Outputs.`,
        ],
      },
      {
        heading: "23. Customer-Controlled Activation",
        paragraphs: [
          `${PLATFORM} is designed around AI-powered assistance and customer-controlled activation.`,
          `Customer remains responsible for deciding what is sent, exported, published, scheduled, activated, copied, used or relied on.`,
          `Customer must review, edit, approve and verify Generated Outputs before using them.`,
          `Customer must not represent that ${PLATFORM} has approved, reviewed, verified, legally cleared, compliance-checked or guaranteed any Generated Output, campaign, recipient list, data source, template, claim, message, deliverability result or commercial outcome.`,
        ],
      },
      {
        heading: "24. AI-Assisted Features and Generated Outputs",
        paragraphs: [
          `${PLATFORM} may include AI-assisted tools that help draft, structure, summarise, classify, translate, segment, organize, review, suggest or generate content and workflow actions.`,
          `Generated Outputs are drafts only.`,
          `Customer is responsible for ensuring that all Generated Outputs are accurate, lawful, non-deceptive, non-infringing, appropriate for the recipient, appropriate for the jurisdiction, appropriate for the channel, appropriate for the industry, consistent with Customer's brand and policies, and compliant with applicable laws, regulations, professional duties and platform rules.`,
          `Generated Outputs may be incomplete, inaccurate, outdated, repetitive, biased, unsuitable, non-compliant, unsafe, offensive, misleading or similar to outputs generated for other users.`,
          `We do not guarantee uniqueness, copyrightability, legal clearance, factual accuracy, commercial value, compliance, suitability, performance, deliverability or any outcome from Generated Outputs.`,
          `${PLATFORM} does not provide legal, tax, accounting, financial, medical, employment, investment, regulatory, insurance, professional or compliance advice. Customer must not rely on AI outputs as a substitute for qualified professional review.`,
        ],
      },
      {
        heading: "25. AI Legal Role Allocation",
        paragraphs: [
          `Customer is responsible for determining whether its use of ${PLATFORM} makes Customer a deployer, user, provider, distributor, importer, operator, controller, business, employer, regulated entity or other legal role under applicable AI, privacy, marketing, consumer protection, professional or sector-specific laws.`,
          `${PLATFORM} provides AI-assisted software tools and workflow support. Unless expressly agreed in writing, Global Solutions Management LLC does not assume Customer's legal role, regulated status, professional obligations, deployment obligations, customer-facing obligations, human oversight obligations, impact assessment obligations, transparency obligations, risk management obligations, monitoring obligations or recordkeeping obligations.`,
          `Customer must not use ${PLATFORM} in any way that would make Global Solutions Management LLC responsible for Customer's regulated AI system, high-risk AI deployment, automated decision system, profiling activity, employment decision, credit decision, housing decision, education decision, insurance decision, medical decision, legal decision or other regulated decision.`,
          `Customer is responsible for human oversight, review, approval, transparency notices, end-user disclosures, logs, monitoring, impact assessments and compliance records where required by applicable AI, privacy, employment, consumer protection, sector-specific or professional laws.`,
        ],
      },
      {
        heading: "26. Regulated, High-Risk and Restricted Uses",
        paragraphs: [
          `${PLATFORM} must not be used for regulated, high-risk, sensitive or legally restricted use unless Customer has all required authorizations and the use is expressly permitted under Customer's Plan and applicable written agreement.`,
          `Restricted uses include, without limitation, uses involving:`,
        ],
        bullets: [
          "medical advice or diagnosis;",
          "legal advice;",
          "financial advice;",
          "investment recommendations;",
          "insurance underwriting;",
          "credit eligibility;",
          "employment decisions;",
          "housing decisions;",
          "education access decisions;",
          "criminal justice decisions;",
          "biometric identification;",
          "children's data;",
          "sensitive personal data;",
          "political targeting;",
          "regulated financial promotions;",
          "regulated healthcare promotions;",
          "gambling;",
          "controlled goods or services;",
          "weapons;",
          "illegal goods or services;",
          "vulnerable-person targeting;",
          "emergency services;",
          "safety-critical systems.",
          `We may restrict or refuse any use that we reasonably consider high-risk, unlawful, unsafe, reputationally harmful, operationally burdensome, incompatible with the platform or unsuitable for ${PLATFORM}.`,
        ],
      },
      {
        heading: "27. Agency Workspace Customers",
        paragraphs: [
          `Agency Workspace Customers may operate multiple client, campaign, brand or business workspaces from one account where their Plan permits it.`,
          `The agency account holder remains the contracting Customer and is responsible for:`,
        ],
        bullets: [
          "client relationships;",
          "client permissions;",
          "client data;",
          "client privacy notices;",
          "client lawful basis;",
          "client approvals;",
          "workspace configuration;",
          "sender setup;",
          "suppression lists;",
          "campaign content;",
          "Generated Outputs;",
          "activation decisions;",
          "exports;",
          "billing allocation;",
          "user access;",
          "compliance obligations;",
          "client disputes;",
          "end-client instructions;",
          "local-law obligations.",
          `${PLATFORM} does not enter into a contract with an agency's end clients unless separately agreed in writing by Global Solutions Management LLC.`,
          "End clients do not receive direct rights against us unless we separately contract with them in writing.",
          `Agency Customers must not represent that ${PLATFORM} has approved, reviewed, endorsed, guaranteed, legally cleared or compliance-cleared any client campaign, data source, outreach list, message, template, Generated Output, sender setup, deliverability result, recipient group or activation.`,
          "The agency account holder is liable for all end-client activity under the agency account and must indemnify the Protected Parties for claims arising from client workspaces, client data, client outreach, client instructions, agency-managed use or end-client disputes.",
        ],
      },
      {
        heading: "28. Resale, White-Labelling and Client Access",
        paragraphs: [
          `Customer may not resell, white-label, sublicense, rent, lease, broker, provide bureau access to, provide service-provider access to or commercially exploit ${PLATFORM} for third parties unless the applicable Plan expressly permits it or we agree in writing.`,
          `Customer must not remove, obscure or misrepresent ${PLATFORM} branding, legal terms, safety notices, platform notices, AI notices, opt-out prompts, sender prompts or compliance notices unless expressly permitted in writing.`,
          `Client access, team access or agency workspace access remains subject to this Customer Agreement, the Platform Terms of Service and all platform policies.`,
          `Customer is responsible for ensuring that every client, team member, contractor, consultant or user accessing the platform through Customer complies with all applicable terms.`,
        ],
      },
      {
        heading: "29. Account Ownership and Authority Disputes",
        paragraphs: [
          `If there is a dispute about account ownership, Workspace control, administrator authority, payment authority, client ownership, user access or legal entitlement to data, we may freeze, restrict, suspend, preserve or disable the relevant account, Workspace, export, billing access or user access until the dispute is resolved to our satisfaction.`,
          `We are not required to decide internal disputes between shareholders, directors, partners, employees, agencies, clients, contractors or other parties.`,
          `We may rely on billing records, administrator records, legal documents, court orders, company records, account history or other evidence we consider appropriate.`,
          `We are not liable for loss arising from account freezes, access restrictions, export holds or administrative delays caused by ownership or authority disputes.`,
        ],
      },
      {
        heading: "30. Platform Support; No Professional Services",
        paragraphs: [
          `Support may help with product usage, account access, billing questions, workspace navigation, workflow questions and technical issues.`,
          `Unless expressly agreed in a signed written statement of work accepted by Global Solutions Management LLC, support does not include:`,
        ],
        bullets: [
          "managed campaign delivery;",
          "legal review;",
          "tax advice;",
          "financial advice;",
          "accounting advice;",
          "compliance sign-off;",
          "deliverability consulting;",
          "data protection advice;",
          "sender reputation management;",
          "customer-side approvals;",
          "professional services;",
          "human review of campaigns;",
          "manual prospecting;",
          "outsourced marketing services;",
          "guaranteed outcome work.",
          `Any support, examples, templates, help content, onboarding notes, AI outputs or recommendations are provided for general product-use assistance only and do not transfer responsibility from Customer to ${PLATFORM}.`,
        ],
      },
      {
        heading: "31. Statements of Work",
        paragraphs: [
          `If Global Solutions Management LLC separately agrees to provide implementation, migration, onboarding, configuration, training, technical support, custom setup or other services, those services must be set out in a signed written statement of work or written order accepted by Global Solutions Management LLC.`,
          `No statement of work will include legal advice, compliance approval, managed campaign operation, deliverability guarantee, professional advice, tax advice, financial advice, regulated advice or outcome guarantee unless expressly stated and lawfully permitted.`,
          `Customer remains responsible for data, compliance, approvals, outreach, Generated Outputs, sender setup, client permissions and activation decisions even where support, onboarding or implementation assistance is provided.`,
        ],
      },
      {
        heading: "32. No Outcome Guarantee",
        paragraphs: [
          `Commercial outcomes depend on Customer's data quality, sender reputation, domain reputation, message quality, offer strength, lawful permissions, audience behavior, market conditions, timing, third-party systems, customer response, recipient decisions and external factors outside our control.`,
          `We do not guarantee:`,
        ],
        bullets: [
          "replies;",
          "leads;",
          "meetings;",
          "sales;",
          "conversion;",
          "funding;",
          "investment;",
          "press coverage;",
          "revenue;",
          "profit;",
          "pipeline value;",
          "deliverability;",
          "inbox placement;",
          "sender reputation;",
          "domain reputation;",
          "open rates;",
          "click rates;",
          "recipient response;",
          "legal compliance;",
          "campaign performance;",
          "customer retention;",
          "commercial outcome.",
          "No projection, dashboard, estimate, score, pipeline value, quality status, campaign preview, Generated Output, template, report, example or support message creates an outcome guarantee.",
        ],
      },
      {
        heading: "33. Customer Data and Output Rights",
        paragraphs: [
          `Customer retains ownership of Customer Data and customer-provided content.`,
          `Subject to payment, compliance with this Customer Agreement and the Platform Terms of Service, Customer may use Generated Outputs produced for Customer through ${PLATFORM} for Customer's lawful business purposes.`,
          `We do not guarantee that Generated Outputs are unique, protectable, copyrightable, registrable, non-infringing, accurate, compliant or free from third-party rights.`,
          `Similar or identical outputs may be generated for other users.`,
          `Customer is responsible for reviewing, clearing, approving and lawfully using Generated Outputs before use.`,
          `Platform templates, system prompts, workflow structures, product logic, interface design, scoring logic, safety controls, platform processes and underlying technology remain owned by Global Solutions Management LLC or its licensors and are not transferred to Customer.`,
        ],
      },
      {
        heading: "34. Platform Intellectual Property",
        paragraphs: [
          `Global Solutions Management LLC and its licensors own the ${PLATFORM} platform, software, interfaces, workflows, systems, product logic, templates, prompts, safety controls, designs, documentation, branding, know-how, data models, scoring logic, processes and all related intellectual property.`,
          `Customer receives a limited, revocable, non-exclusive, non-transferable, non-sublicensable right to access and use ${PLATFORM} in accordance with this Customer Agreement, the Platform Terms of Service, the applicable Plan and platform policies.`,
          `Customer must not copy, reproduce, modify, resell, sublicense, frame, mirror, scrape, extract, reverse engineer, decompile, disassemble, replicate or create derivative works from the platform, workflows, templates, interface, product logic, scoring logic, prompts, safety controls or systems except as expressly permitted.`,
        ],
      },
      {
        heading: "35. Competitor and Training Restrictions",
        paragraphs: [
          `Customer must not use ${PLATFORM} to build, train, fine-tune, benchmark, evaluate, test or improve any competing product, AI model, dataset, platform, workflow, template library, automation system, commercial database or outreach tool without our prior written consent.`,
          `Customer must not scrape, copy, extract, monitor, harvest or use platform content, prompts, templates, workflows, outputs, interface patterns, scoring logic, safety logic, product logic or system behavior for competitive analysis, model training, dataset creation, product replication, reverse engineering or benchmarking.`,
        ],
      },
      {
        heading: "36. Confidentiality",
        paragraphs: [
          `Each party may receive non-public business, technical, operational, pricing, product, security, financial, customer, platform or commercial information from the other.`,
          `Each party must protect confidential information using reasonable care and use it only for the relationship unless disclosure is required by law, court order, regulator request, professional advisers, payment providers, infrastructure providers or service providers bound by confidentiality obligations.`,
          `Customer must not disclose non-public information about ${PLATFORM}, including security controls, platform logic, private roadmap information, beta features, pricing not publicly available, technical processes, non-public documentation, vulnerability information or other confidential information, except as required to use the platform or as required by law.`,
          `Confidentiality obligations survive termination.`,
        ],
      },
      {
        heading: "37. Monitoring, Review and Enforcement",
        paragraphs: [
          `We may, but are not required to, monitor, scan, log, review, analyse, investigate, preserve or restrict accounts, workspaces, Customer Data, Generated Outputs, usage patterns, sender connections, integrations, activation activity, exports, billing activity and platform signals to operate the platform, improve safety, prevent abuse, enforce this Customer Agreement, comply with law, respond to complaints, protect infrastructure, protect recipients, protect third-party providers and protect our business.`,
          `No monitoring, review, automated check, manual check, warning, flag, hold, approval flow or failure to act creates any responsibility for Customer's data, outputs, campaigns, compliance, outreach or legal obligations.`,
          `We may use automated systems, human review, third-party tools and risk scoring to identify suspected abuse, fraud, unlawful activity, security risk, policy breach or platform risk.`,
        ],
      },
      {
        heading: "38. Takedown and Content Removal",
        paragraphs: [
          `We may remove, disable, restrict, quarantine or block any Customer Data, Generated Output, file, campaign, message, sender connection, integration, export or Workspace content that we reasonably believe may violate law, infringe rights, create risk, breach this Customer Agreement, expose the platform to liability or harm third parties.`,
          `We are not liable for loss arising from removal, blocking, disabling, quarantine, moderation, restriction or refusal of content, outputs, campaigns or data where we act to protect the platform, users, recipients, third-party providers, rights holders, regulators or our business.`,
        ],
      },
      {
        heading: "39. Suspension and Restriction",
        paragraphs: [
          `We may suspend, restrict, throttle, downgrade, disable, delete or terminate access to any account, Workspace, user, sender, activation, export, integration, Plan, feature or Credit balance if we reasonably believe there is:`,
        ],
        bullets: [
          "non-payment;",
          "failed payment;",
          "chargeback risk;",
          "fraud risk;",
          "legal risk;",
          "security risk;",
          "platform abuse;",
          "policy breach;",
          "data misuse;",
          "deliverability risk;",
          "complaint activity;",
          "regulatory concern;",
          "sanctions risk;",
          "harmful content;",
          "unlawful outreach;",
          "infrastructure risk;",
          "AI misuse;",
          "third-party provider risk;",
          "reputational risk;",
          "risk to us, customers, recipients, third-party providers or the public.",
          `We may act immediately and without prior notice where necessary to protect ${PLATFORM}, customers, users, recipients, infrastructure, third-party providers, payment systems, AI systems, sender systems, legal compliance, platform integrity or our business.`,
          "Suspension or restriction does not create a refund right and does not relieve Customer of payment obligations already incurred.",
        ],
      },
      {
        heading: "40. Termination",
        paragraphs: [
          `Customer may terminate by canceling the applicable Plan using the available cancellation method.`,
          `We may terminate this Customer Agreement, any Plan, any account, any Workspace or any paid feature where permitted under this Customer Agreement, the Platform Terms of Service, applicable law or a signed written agreement.`,
          `After termination, Customer may lose access to Workspaces, Generated Outputs, Customer Data, reports, integrations, Credits, support and paid features.`,
          `Termination does not affect fees already incurred, taxes, payment obligations, indemnity obligations, confidentiality obligations, data compliance obligations, acceptable use obligations, outreach compliance obligations or provisions that by their nature survive.`,
        ],
      },
      {
        heading: "41. Data Export and Retention",
        paragraphs: [
          `Where technically available and legally permitted, Customer may export certain Customer Data during an active paid subscription, subject to Plan limits, technical limits, security checks, payment status and policy compliance.`,
          `We are not required to provide exports after termination, suspension, non-payment, chargeback, legal restriction, security concern, account closure or platform enforcement, except where required by law or expressly agreed in writing.`,
          `Customer is responsible for exporting and preserving Customer Data before cancellation, downgrade, suspension or termination.`,
          `We may retain limited records where required or reasonably necessary for legal, tax, audit, security, billing, dispute, fraud prevention, compliance, enforcement or legitimate business purposes.`,
        ],
      },
      {
        heading: "42. Downgrades and Loss of Features",
        paragraphs: [
          `If Customer downgrades, cancels, pauses, fails to pay or moves to a lower Plan, access to features, users, data volumes, exports, agency workspaces, integrations, Credits, support, reports, activation tools and stored data may be reduced, disabled, restricted or removed.`,
          `Customer is responsible for exporting data before downgrading or canceling where export is available and required.`,
          `We are not liable for loss of functionality, loss of access, loss of Credits, loss of historical reports, loss of integrations or loss of data availability caused by downgrade, cancellation, non-payment, Plan change or termination, except where mandatory law requires otherwise.`,
        ],
      },
      {
        heading: "43. Privacy and Data Processing",
        paragraphs: [
          `The Privacy Policy explains how we collect and use personal information about website visitors, platform users, prospects, customers, billing contacts, support contacts and individuals who interact with us directly.`,
          `The Data Processing Agreement governs personal data that customers upload and that we process as a processor, service provider or equivalent role on Customer's behalf.`,
          `Customer is responsible for determining whether Customer acts as controller, business, processor, service provider, joint controller, independent controller, deployer, provider or another legal role under applicable law.`,
          `Customer is responsible for providing all required privacy notices to its own contacts, customers, prospects, recipients, employees, contractors, end clients and users.`,
          `Customer is responsible for responding to legal rights requests from its own contacts, customers, prospects and end clients unless applicable law requires us to respond directly.`,
          `Customer must not use ${PLATFORM} to avoid, bypass or frustrate privacy, marketing, data protection, platform or recipient rights.`,
        ],
      },
      {
        heading: "44. International Data Transfers",
        paragraphs: [
          `${PLATFORM} may be operated from, supported from, hosted in or accessed from multiple jurisdictions, including the United States and other countries where we or our service providers operate.`,
          `Customer is responsible for ensuring that Customer Data may lawfully be transferred to, accessed from, processed in and stored in all relevant jurisdictions.`,
          `Where required by applicable data protection law, international transfers may be governed by the Data Processing Agreement, applicable standard contractual clauses, transfer addenda, supplementary measures or other lawful transfer mechanisms.`,
          `Customer must not upload or process data through ${PLATFORM} where transfer, access, processing or storage would breach applicable law, contract, professional duty, regulatory obligation or data subject restriction.`,
        ],
      },
      {
        heading: "45. Regulatory, Legal and Third-Party Requests",
        paragraphs: [
          `If we receive a subpoena, court order, regulator request, law enforcement request, data subject request, rights-holder complaint, payment-provider request, email-provider request, third-party platform request, security complaint or legal notice relating to Customer, Customer Data, Customer's users, Customer's outreach, Customer's clients or Customer's Workspace, we may respond, disclose information, preserve records, restrict access, notify Customer or take other action as we consider legally required or reasonably appropriate.`,
          `Customer is responsible for costs, losses, fees, penalties, legal fees and operational burden arising from requests, complaints, investigations or claims caused by Customer Data, Customer outreach, Customer instructions, Customer users, Customer clients, Customer integrations or Customer's use of ${PLATFORM}.`,
          `We are not required to challenge any legal, regulatory, payment-provider, email-provider or third-party request unless required by law or expressly agreed in writing.`,
        ],
      },
      {
        heading: "46. Third-Party Services",
        paragraphs: [
          `${PLATFORM} may rely on, connect to or integrate with third-party providers for hosting, authentication, payments, analytics, email connectivity, AI processing, data storage, monitoring, communications, translation, CRM tools, domain services, DNS, CDNs, logging, support tools or other infrastructure.`,
          `Third-party services are not controlled by Global Solutions Management LLC. They may have their own terms, policies, pricing, security practices, outages, rate limits, account restrictions, suspensions, data practices and compliance requirements.`,
          `We are not responsible for third-party service failures, outages, errors, policy changes, pricing changes, data handling, deliverability decisions, payment interruptions, email provider restrictions, account suspensions, integration failures or external platform behavior outside our reasonable control.`,
          `Customer is responsible for complying with the terms and policies of any third-party services it connects to or uses with ${PLATFORM}.`,
        ],
      },
      {
        heading: "47. Service Levels",
        paragraphs: [
          `Any service level, support target, uptime target or response target applies only where expressly stated in the Service Level Agreement or a signed written agreement accepted by Global Solutions Management LLC.`,
          `Unless expressly agreed otherwise, service levels do not apply to:`,
        ],
        bullets: [
          "beta features;",
          "trials;",
          "free plans;",
          "third-party services;",
          "customer systems;",
          "customer integrations;",
          "customer-side misconfiguration;",
          "internet issues;",
          "email provider issues;",
          "AI provider issues;",
          "payment provider issues;",
          "hosting provider issues;",
          "force majeure events;",
          "external service events;",
          "misuse;",
          "unsupported use;",
          "suspended accounts;",
          "accounts with overdue payments;",
          "regulatory restrictions;",
          "sanctions restrictions;",
          "maintenance;",
          "security interventions;",
          "deliverability outcomes;",
          "outreach results;",
          "Generated Outputs.",
          "Service credits, if any, are the sole and exclusive remedy for failure to meet an applicable service level and are subject to the terms, exclusions and claim process in the Service Level Agreement.",
        ],
      },
      {
        heading: "48. Force Majeure and External Service Events",
        paragraphs: [
          `Global Solutions Management LLC will not be liable for any delay, failure, interruption, suspension, loss, degradation, unavailability, data processing delay, delivery failure, support delay, billing disruption, feature limitation or non-performance caused by events, circumstances or conditions beyond its reasonable control.`,
          `Force majeure and external service events include, without limitation, natural disasters, public health emergencies, war, terrorism, civil unrest, government action, regulatory action, sanctions, export controls, law changes, court orders, cloud provider failures, hosting failures, DNS failures, CDN failures, payment processor failures, banking failures, AI provider failures, API provider failures, email provider failures, CRM provider failures, internet failures, cyberattacks, denial-of-service attacks, malware, ransomware, labor disputes, supply-chain issues, utility failures, emergency maintenance, third-party policy changes, account restrictions, spam filtering, blacklist events, sender reputation issues, domain reputation issues and any other event beyond our reasonable control.`,
          `Force majeure and external service events do not create any right to a refund, credit, service credit, damages, compensation, chargeback, termination payment or extension unless expressly required by law or expressly stated in a separate signed written agreement accepted by Global Solutions Management LLC.`,
          `Force majeure and external service events do not relieve Customer of payment obligations for fees already incurred, subscriptions already activated, Credits already made available, taxes, chargebacks, usage charges, indemnity obligations, confidentiality obligations, data compliance obligations, acceptable use obligations, outreach compliance obligations or obligations that by their nature should continue.`,
        ],
      },
      {
        heading: "49. No Global Availability Guarantee",
        paragraphs: [
          `${PLATFORM} may be accessed from multiple countries, but not all features, Plans, data types, integrations, outreach activities, AI functions, payment methods, templates or activation tools are lawful, available or appropriate in every jurisdiction.`,
          `We may restrict, geoblock, suspend, modify or refuse access to ${PLATFORM} or any feature in any country, region, state, province, territory or sector where we reasonably believe there is legal, regulatory, sanctions, technical, payment, security, operational or reputational risk.`,
          `Customer is responsible for ensuring that its use of ${PLATFORM} is lawful in every jurisdiction relevant to Customer, its users, its data, its recipients, its senders, its clients, its campaigns and its business.`,
        ],
      },
      {
        heading: "50. Sanctions, Export Control and Anti-Corruption",
        paragraphs: [
          `Customer must comply with all applicable sanctions, export control, trade control, anti-bribery, anti-corruption and anti-money laundering laws.`,
          `Customer must not use ${PLATFORM} if Customer or any User is located in, organized under the laws of, ordinarily resident in, owned or controlled by, or acting on behalf of any person or entity subject to sanctions, embargoes or trade restrictions that would prohibit access to the platform.`,
          `Customer must not use ${PLATFORM} to support prohibited transactions, restricted parties, sanctioned entities, embargoed territories, unlawful exports, unlawful re-exports or unlawful circumvention of trade controls.`,
          `Customer represents and warrants that Customer and its Users are not prohibited from using ${PLATFORM} under applicable sanctions, export control or trade laws.`,
          `We may restrict, suspend or terminate access where sanctions, export control, trade compliance, anti-corruption or payment risk is identified or suspected.`,
        ],
      },
      {
        heading: "51. Disclaimers",
        paragraphs: [
          `${PLATFORM}, paid Plans, Credits, AI-assisted features, Generated Outputs, templates, exports, support and integrations are provided on an "as is" and "as available" basis to the maximum extent permitted by law.`,
          `We do not warrant that the platform, paid features, Credits, AI-assisted tools, Generated Outputs, templates, exports, integrations or support will be uninterrupted, error-free, secure against every threat, compatible with every system, available in every location, suitable for every jurisdiction, suitable for every industry or capable of meeting every customer requirement.`,
          `We do not warrant or guarantee data quality, sender verification results, Generated Outputs, AI suggestions, reply classification, pipeline values, profitability estimates, export formatting, integrations, translations, campaign performance, deliverability, inbox placement, sender reputation, domain reputation, replies, meetings, sales, funding, revenue, legal compliance, regulatory compliance, recipient response, business outcomes, avoidance of spam filters, avoidance of complaints or avoidance of regulatory inquiry.`,
        ],
      },
      {
        heading: "52. Limitation of Liability",
        paragraphs: [
          `To the maximum extent permitted by law, the total aggregate liability of the Protected Parties arising out of or relating to this Customer Agreement, ${PLATFORM}, any Plan, any Credits, any Workspace, any billing issue, any Generated Output, any support interaction or any incorporated policy is limited to the amounts paid by Customer to Global Solutions Management LLC for the platform during the twelve months immediately before the event giving rise to the claim.`,
          `If Customer used ${PLATFORM} for free or paid no fees during that period, the total aggregate liability of the Protected Parties is limited to USD 100 to the maximum extent permitted by law.`,
          `To the maximum extent permitted by law, the Protected Parties are not liable for indirect, incidental, special, consequential, exemplary, enhanced or punitive damages, or for loss of profits, revenue, goodwill, opportunity, data, use, contracts, customers, business interruption, deliverability, sender reputation, domain reputation, anticipated savings, business value, reputation, wasted expenditure, replacement services, regulatory exposure, loss of access, loss of Credits or loss arising from customer-side use.`,
          `The limitations apply whether the claim is based on contract, tort, negligence, strict liability, statute, equity or any other legal theory, even if a Protected Party has been advised of the possibility of such damages.`,
          `Customer agrees that the limitations of liability are an essential basis of the bargain and apply even if any limited remedy fails of its essential purpose.`,
          `Nothing in this Customer Agreement excludes or limits liability where liability cannot lawfully be excluded or limited.`,
        ],
      },
      {
        heading: "53. No Personal Liability of Directors, Managers, Staff or Other Protected Parties",
        paragraphs: [
          `To the maximum extent permitted by law, Customer agrees that it will bring any claim relating to ${PLATFORM}, this Customer Agreement, a Plan, Credits, billing, Customer's account, Customer's Workspace, Customer Data, Generated Outputs, suspension, termination, outreach activity or platform use only against Global Solutions Management LLC and not against any owner, member, manager, director, officer, employee, contractor, consultant, agent, representative, affiliate, licensor, supplier, service provider, subprocessor, payment provider, infrastructure provider, AI provider, hosting provider or professional adviser personally.`,
          `Customer waives, to the maximum extent permitted by law, any right to seek personal liability from any Protected Party other than Global Solutions Management LLC for acts or omissions connected with ${PLATFORM}, except where such waiver is prohibited by mandatory law.`,
          `The Protected Parties are intended third-party beneficiaries of all disclaimers, limitations of liability, indemnities, no-personal-liability provisions, defence rights, confidentiality protections, intellectual property protections and enforcement rights in this Customer Agreement and may rely on and enforce those protections to the maximum extent permitted by law.`,
        ],
      },
      {
        heading: "54. Indemnity",
        paragraphs: [
          `Customer agrees to defend, indemnify and hold harmless the Protected Parties from and against all claims, demands, investigations, complaints, proceedings, losses, liabilities, damages, penalties, fines, settlements, costs and expenses, including reasonable legal fees, arising from or related to:`,
        ],
        bullets: [
          "Customer's account;",
          "Customer's users;",
          "Customer's Workspaces;",
          "Customer Data;",
          "uploaded content;",
          "prompts;",
          "Generated Outputs;",
          "outreach activity;",
          "sender identity;",
          "contact lists;",
          "client activity;",
          "agency workspaces;",
          "end-client instructions;",
          "Customer's breach of this Customer Agreement;",
          "Customer's breach of the Platform Terms of Service;",
          "Customer's breach of law;",
          "infringement or alleged infringement of third-party rights;",
          "unlawful marketing;",
          "data protection violations;",
          "privacy violations;",
          "suppression or opt-out failures;",
          "regulated activity;",
          "customer-side systems;",
          "third-party integrations connected by Customer;",
          "payment disputes;",
          "chargebacks;",
          "tax obligations;",
          "regulatory requests;",
          "law enforcement requests;",
          "recipient complaints;",
          "data subject requests;",
          "misuse of the platform.",
          "We may control the defence of any matter subject to indemnity. Customer must cooperate with us and must not settle any claim in a way that imposes obligations, admissions, restrictions or liability on any Protected Party without our prior written consent.",
          "Customer's indemnity obligations apply to the maximum extent permitted by law, including where claims are brought by customers, end clients, recipients, users, regulators, authorities, payment providers, email providers, data subjects, rights holders, competitors or third-party platforms.",
        ],
      },
      {
        heading: "55. Local Law and Mandatory Rights",
        paragraphs: [
          `This Customer Agreement applies to the maximum extent permitted by applicable law.`,
          `If mandatory law in a jurisdiction gives Customer rights that cannot be excluded, restricted or modified, this Customer Agreement does not exclude those rights, but all exclusions, limitations, disclaimers, indemnities and protections continue to apply to the maximum extent permitted by that law.`,
          `Where a provision cannot be enforced as written in a particular jurisdiction, it will be interpreted, narrowed or reformed only to the minimum extent necessary to make it enforceable, and the rest of the provision and this Customer Agreement remain effective.`,
        ],
      },
      {
        heading: "56. No Third-Party Rights Against Us",
        paragraphs: [
          `End clients, recipients, prospects, contacts, Customer customers, agency clients, users, suppliers, contractors and other third parties do not receive direct rights against Global Solutions Management LLC under this Customer Agreement unless expressly stated in a signed written agreement accepted by Global Solutions Management LLC.`,
          `Customer is responsible for claims, disputes, complaints and obligations arising between Customer and its own clients, users, recipients, prospects, contacts, suppliers, contractors and third parties.`,
        ],
      },
      {
        heading: "57. No Fiduciary, Partnership or Agency Relationship",
        paragraphs: [
          `Nothing in this Customer Agreement creates a partnership, joint venture, fiduciary relationship, employment relationship, franchise, agency relationship, representative authority or professional advisory relationship between Customer and Global Solutions Management LLC.`,
          `Customer has no authority to bind Global Solutions Management LLC or represent that ${PLATFORM} has approved, endorsed, reviewed, guaranteed or cleared any Customer activity.`,
        ],
      },
      {
        heading: "58. Injunctive Relief",
        paragraphs: [
          `Customer agrees that breach of provisions relating to intellectual property, confidentiality, platform misuse, security abuse, unlawful access, reverse engineering, misuse of data, unlawful disclosure, competitor misuse, model training misuse or prohibited use may cause irreparable harm.`,
          `We may seek injunctive or equitable relief without needing to prove actual damages or post a bond, where permitted by law.`,
        ],
      },
      {
        heading: "59. Governing Law and Jurisdiction",
        paragraphs: [
          `This Customer Agreement is governed by the laws of the State of Delaware, United States, without regard to conflict-of-law principles.`,
          `Subject to any mandatory law that cannot be waived, the state and federal courts located in Delaware have exclusive jurisdiction over disputes arising out of or relating to this Customer Agreement, ${PLATFORM}, any Plan, Credits, billing, Workspace access, suspension or termination.`,
          `Customer and Global Solutions Management LLC each waive any objection to jurisdiction, venue or inconvenient forum in those courts.`,
          `Despite the above, Global Solutions Management LLC may seek injunctive relief, debt recovery, payment enforcement, intellectual property enforcement, confidentiality enforcement, security enforcement or urgent protective relief in any court or forum of competent jurisdiction.`,
        ],
      },
      {
        heading: "60. Optional Arbitration at Our Election",
        paragraphs: [
          `To the maximum extent permitted by law, Global Solutions Management LLC may elect to have any dispute, claim or controversy arising out of or relating to ${PLATFORM}, this Customer Agreement, Customer's account, Customer's Workspace, Customer Data, Generated Outputs, payment, suspension or termination resolved by confidential binding arbitration instead of court proceedings.`,
          `If arbitration is elected, the arbitration will take place in Delaware, in English, before a single arbitrator, under commercially reasonable arbitration rules selected by Global Solutions Management LLC, unless mandatory law requires otherwise.`,
          `Nothing in this section prevents Global Solutions Management LLC from seeking injunctive relief, debt recovery, enforcement of intellectual property rights, confidentiality protection, security protection or urgent protective relief in court.`,
        ],
      },
      {
        heading: "61. Class Action and Jury Trial Waiver",
        paragraphs: [
          `To the maximum extent permitted by law, Customer and Global Solutions Management LLC agree that disputes will be resolved only on an individual basis and not as part of any class, collective, consolidated, representative or group action.`,
          `To the maximum extent permitted by law, Customer and Global Solutions Management LLC waive any right to a jury trial in any dispute arising out of or relating to this Customer Agreement, ${PLATFORM}, any Plan, Credits, billing, Workspace access, suspension or termination.`,
        ],
      },
      {
        heading: "62. Limitation Period",
        paragraphs: [
          `To the maximum extent permitted by law, any claim arising out of or relating to this Customer Agreement, ${PLATFORM}, a Plan, Credits, billing, a Workspace, Generated Outputs, Customer Data, outreach activity, suspension or termination must be brought within one year after the event giving rise to the claim.`,
          `Claims brought after that period are permanently barred, unless a longer period is required by mandatory law.`,
        ],
      },
      {
        heading: "63. Assignment",
        paragraphs: [
          `Customer may not assign, transfer or delegate this Customer Agreement, Customer's account, Workspace, Plan, Credits or rights to use ${PLATFORM} without our prior written consent.`,
          `We may assign or transfer this Customer Agreement, accounts, customer relationships, data processing arrangements, billing relationships and platform operations in connection with a merger, acquisition, restructuring, financing, sale of assets, corporate reorganization, transfer of business, change of control or platform migration.`,
        ],
      },
      {
        heading: "64. Notices",
        paragraphs: [
          `We may provide notices by posting on the website, posting in the platform, sending email to the account contact, sending billing notices, displaying checkout notices or using other reasonable communication methods.`,
          `Customer is responsible for keeping contact details current and monitoring notices.`,
          `Legal notices to Global Solutions Management LLC must be submitted through the Contact page using the route that matches the request, unless a separate signed written agreement specifies another notice method.`,
        ],
      },
      {
        heading: "65. Severability",
        paragraphs: [
          `If any provision of this Customer Agreement is held invalid, unlawful or unenforceable, the remaining provisions remain in full force.`,
          `The invalid provision will be interpreted, narrowed or replaced to achieve the original intent as closely as permitted by law.`,
        ],
      },
      {
        heading: "66. No Waiver",
        paragraphs: [
          `Failure to enforce any provision of this Customer Agreement is not a waiver of that provision or any other provision.`,
          `Any waiver must be in writing and expressly accepted by Global Solutions Management LLC.`,
        ],
      },
      {
        heading: "67. Entire Agreement",
        paragraphs: [
          `This Customer Agreement, the Platform Terms of Service and all incorporated documents form the entire agreement between Customer and Global Solutions Management LLC regarding paid access to and commercial use of ${PLATFORM}, unless a separate signed written agreement expressly states otherwise.`,
          `Customer acknowledges that it has not relied on any statement, representation, promise, assurance, forecast, projection, demo, example, output, estimate, support message or marketing statement not expressly included in this Customer Agreement, the Platform Terms of Service or a signed written agreement accepted by Global Solutions Management LLC.`,
        ],
      },
      {
        heading: "68. Interpretation",
        paragraphs: [
          `Headings are for convenience only and do not affect interpretation.`,
          `Words such as "including", "includes", "for example" and "without limitation" are illustrative and do not limit the words that follow.`,
          `References to law include amendments, replacements and successor laws.`,
          `References to Customer include Customer's users, employees, contractors, agents, agencies, clients and representatives where the context allows.`,
          `Any rule of interpretation that ambiguity is interpreted against the drafting party does not apply to this Customer Agreement.`,
        ],
      },
      {
        heading: "69. Survival",
        paragraphs: [
          `Any provision that by its nature should survive suspension, termination, expiry, cancellation, downgrade or account closure will survive, including provisions relating to payment obligations, taxes, Credits, refunds, chargebacks, ownership, Customer Data responsibility, confidentiality, prohibited use, privacy, data processing, disclaimers, limitation of liability, no personal liability, indemnity, dispute resolution, governing law, class action waiver, jury trial waiver, records retention and enforcement.`,
        ],
      },
      {
        heading: "70. Contact",
        paragraphs: [
          `For customer agreement, billing, subscription, Credit or contract questions, ${CONTACT_CTA}`,
          `Global Solutions Management LLC. Delaware, United States. Operator of ${PLATFORM}.`,
        ],
      },
    ],
  },
  "data-processing-agreement": {
    title: "Data Processing Agreement",
    effectiveDate: "30 June 2026",
    lastUpdated: "1 July 2026",
    version: "7.0",
    sections: [
      {
        heading: "1. Definitions",
        paragraphs: [
          "In this Data Processing Agreement:",
          "“DPA” means this Data Processing Agreement.",
          "“Velocity Vision”, “Platform”, “Service”, “we”, “us” or “our” means the Velocity Vision platform operated by Global Solutions Management LLC.",
          "“Global Solutions Management LLC” means Global Solutions Management LLC, a company incorporated in the State of Delaware, United States.",
          "“Customer”, “you” or “your” means the person, company, agency, organization, partnership, charity, trust, public body, fund, brand, client, estate, project or other legal entity that uses Velocity Vision and determines the purposes and means of processing Customer Personal Data.",
          "“Customer Personal Data” means Personal Data that Customer uploads, imports, enters, provides, generates, activates, exports, processes or otherwise makes available through Velocity Vision and that Global Solutions Management LLC processes on behalf of Customer as Processor, Service Provider, Contractor or equivalent role.",
          "“Personal Data” means any information relating to an identified or identifiable individual, or any equivalent term under applicable Data Protection Laws.",
          "“Data Protection Laws” means all privacy, data protection, cybersecurity, electronic communications, direct marketing, AI-related and similar laws that apply to the processing of Customer Personal Data, including where applicable the GDPR, UK GDPR, Data Protection Act 2018, PECR, Swiss FADP, CCPA/CPRA, US state privacy laws, EU AI Act, ePrivacy rules and other applicable privacy, marketing, AI and data protection laws.",
          "“GDPR” means Regulation (EU) 2016/679.",
          "“CCPA” means the California Consumer Privacy Act as amended by the California Privacy Rights Act and applicable regulations.",
          "“Controller”, “Processor”, “Business”, “Service Provider”, “Contractor”, “Consumer”, “Data Subject”, “Process”, “Processing”, “Sell”, “Share”, “Sensitive Personal Information” and similar terms have the meanings given to them under applicable Data Protection Laws.",
          "“Subprocessor” means any third party engaged by Global Solutions Management LLC to process Customer Personal Data on behalf of Customer in connection with Velocity Vision.",
          "“Security Incident” means a confirmed breach of security leading to accidental or unlawful destruction, loss, alteration, unauthorized disclosure of or access to Customer Personal Data processed by Global Solutions Management LLC on behalf of Customer.",
          "“Restricted Data” means categories of Personal Data that are sensitive, regulated, high-risk or restricted under applicable law, contract, professional duty or platform policy.",
          "“AI Processing” means processing involving AI-assisted tools, AI models, AI infrastructure, AI prompts, AI-generated outputs, AI-assisted classification, AI-assisted review, AI-assisted generation, AI-assisted translation, AI-assisted segmentation or AI-supported workflow features.",
          "“Protected Parties” means Global Solutions Management LLC and its present and former owners, members, managers, directors, officers, employees, staff, contractors, consultants, agents, representatives, affiliates, licensors, suppliers, service providers, subprocessors, payment providers, infrastructure providers, AI providers, hosting providers, professional advisers, successors and assigns.",
        ],
      },
      {
        heading: "2. Purpose and Incorporation",
        paragraphs: [
          "This DPA applies when Global Solutions Management LLC processes Customer Personal Data on behalf of Customer in the course of providing Velocity Vision.",
          "For Customer Personal Data, Customer is the Controller, Business or equivalent determining party, and Global Solutions Management LLC is the Processor, Service Provider, Contractor or equivalent processing party, except where Global Solutions Management LLC processes Personal Data for its own purposes as described in the Privacy Policy.",
          "This DPA is incorporated into and forms part of the agreement between Customer and Global Solutions Management LLC.",
          "If there is a conflict between this DPA and the Platform Terms of Service or Customer Agreement, this DPA controls only for the specific processing of Customer Personal Data. The Platform Terms of Service and Customer Agreement continue to control platform use, billing, credits, intellectual property, acceptable use, suspension, disclaimers, limitation of liability, indemnity, no personal liability, governing law and enforcement, unless expressly overridden by this DPA.",
          "This DPA does not create direct contractual rights for Customer’s data subjects, recipients, prospects, clients, agency clients or other third parties unless mandatory law requires otherwise.",
        ],
      },
      {
        heading: "3. Delaware Operator and No Personal Liability",
        paragraphs: [
          "Velocity Vision is operated by Global Solutions Management LLC, a Delaware limited liability company.",
          "To the maximum extent permitted by law, Customer agrees that any claim relating to this DPA, Customer Personal Data, processing, security, transfers, subprocessors, AI Processing, deletion, audits, regulatory assistance, data subject requests or Security Incidents will be brought only against Global Solutions Management LLC and not personally against any owner, member, manager, director, officer, employee, staff member, contractor, consultant, agent, representative, affiliate, licensor, supplier, service provider, Subprocessor, payment provider, infrastructure provider, AI provider, hosting provider or professional adviser.",
          "The Protected Parties are intended third-party beneficiaries of all disclaimers, limitations of liability, indemnities, no-personal-liability provisions, defence rights, confidentiality protections, audit restrictions, legal request protections and enforcement rights in this DPA and may rely on and enforce those protections to the maximum extent permitted by law.",
          "Nothing in this DPA excludes liability where liability cannot lawfully be excluded.",
        ],
      },
      {
        heading: "4. Independent Controller Processing",
        paragraphs: [
          "Global Solutions Management LLC may process certain Personal Data as an independent Controller, Business or equivalent role for its own legitimate business purposes, including:",
          "Such processing is governed by the Privacy Policy and not by this DPA, except where Data Protection Laws require otherwise.",
        ],
        bullets: [
          "account administration;",
          "billing;",
          "payment processing;",
          "tax records;",
          "fraud prevention;",
          "platform security;",
          "abuse prevention;",
          "sanctions screening;",
          "legal compliance;",
          "support operations;",
          "service communications;",
          "product analytics;",
          "business operations;",
          "corporate records;",
          "dispute handling;",
          "enforcing legal rights.",
        ],
      },
      {
        heading: "5. Processing Details",
        paragraphs: [
          "The subject matter of processing is the provision, operation, maintenance, security and support of Velocity Vision as a self-serve, AI-powered commercial workspace.",
          "The duration of processing is the period during which Customer maintains an account, workspace, subscription, plan, trial, paid feature or other access to Velocity Vision, plus any period required for deletion, backup expiry, legal retention, tax retention, dispute handling, security, fraud prevention, compliance or enforcement.",
          "The nature and purpose of processing is to provide Velocity Vision, including to:",
        ],
        bullets: [
          "host Customer Personal Data;",
          "store Customer Personal Data;",
          "receive uploads, imports and entries;",
          "map fields;",
          "validate fields;",
          "deduplicate records;",
          "quality-check data;",
          "segment records;",
          "generate AI-assisted outputs;",
          "prepare outreach assets;",
          "support templates;",
          "support activation controls;",
          "schedule or route workflow actions;",
          "organize replies;",
          "support follow-up;",
          "support pipeline tracking;",
          "provide reporting;",
          "provide exports;",
          "secure the platform;",
          "monitor abuse;",
          "troubleshoot issues;",
          "support users;",
          "maintain billing and usage records;",
          "operate integrations;",
          "process support requests;",
          "comply with legal obligations.",
        ],
      },
      {
        heading: "6. Types of Personal Data",
        paragraphs: [
          "Customer Personal Data may include:",
          "Customer controls what Customer Personal Data is uploaded, imported, processed, generated, activated or exported through Velocity Vision.",
        ],
        bullets: [
          "names;",
          "email addresses;",
          "telephone numbers;",
          "company names;",
          "job titles;",
          "business contact details;",
          "professional profiles;",
          "location;",
          "country;",
          "region;",
          "language preference;",
          "tags;",
          "notes;",
          "source information;",
          "relationship history;",
          "reply data;",
          "pipeline data;",
          "customer status;",
          "prospect status;",
          "sender information;",
          "campaign information;",
          "suppression status;",
          "opt-out information;",
          "business records;",
          "agency client contact data;",
          "other fields uploaded, imported, generated or configured by Customer.",
        ],
      },
      {
        heading: "7. Categories of Data Subjects",
        paragraphs: [
          "Categories of data subjects may include:",
          "Customer is responsible for ensuring that the categories of data subjects processed through Velocity Vision are lawful and appropriate for the intended use.",
        ],
        bullets: [
          "prospects;",
          "customers;",
          "business contacts;",
          "employees of organizations;",
          "officers of organizations;",
          "contractors;",
          "suppliers;",
          "agency client contacts;",
          "professional contacts;",
          "recipients;",
          "senders;",
          "Customer users;",
          "client users;",
          "other individuals whose data Customer chooses to process through Velocity Vision.",
        ],
      },
      {
        heading: "8. Prohibited and Restricted Data",
        paragraphs: [
          "Customer must not upload, import, process, generate, activate, export or store through Velocity Vision:",
          "Restricted Data may only be processed if expressly authorized in a separate signed written agreement accepted by Global Solutions Management LLC and if Customer has all required legal rights, notices, consents, lawful bases, approvals and safeguards.",
          "We may suspend, restrict, delete, quarantine, return, block, refuse or report processing of data that we reasonably believe is prohibited, restricted, unlawful, unsafe, non-compliant or unsuitable for Velocity Vision.",
        ],
        bullets: [
          "children’s data;",
          "health data;",
          "biometric data;",
          "genetic data;",
          "criminal offense data;",
          "financial account data;",
          "payment card data;",
          "government identifier data;",
          "precise geolocation data;",
          "special category data under GDPR or UK GDPR;",
          "sensitive personal information under CCPA or similar laws;",
          "data subject to professional secrecy or privilege unless Customer is authorized to process it;",
          "data subject to confidentiality obligations that prohibit SaaS processing;",
          "data subject to contractual restrictions that prohibit SaaS processing;",
          "data subject to sector-specific restrictions that prohibit the intended processing;",
          "data unlawfully obtained;",
          "data collected without required notices, consents, permissions or lawful basis;",
          "data that cannot lawfully be transferred to, accessed from or processed by Global Solutions Management LLC or its Subprocessors.",
        ],
      },
      {
        heading: "9. Customer Instructions",
        paragraphs: [
          "Global Solutions Management LLC will process Customer Personal Data only on documented Customer instructions, including instructions given through:",
          "Customer instructs Global Solutions Management LLC to process Customer Personal Data as necessary to provide, secure, support, maintain, improve and operate Velocity Vision in accordance with this DPA, the Platform Terms of Service, the Customer Agreement, the Privacy Policy and applicable product documentation.",
          "We may refuse, suspend, delay or decline any instruction if we reasonably believe it is unlawful, unsafe, technically impractical, outside the platform scope, contrary to Data Protection Laws, contrary to this DPA, contrary to platform policies, harmful to the platform or likely to expose Global Solutions Management LLC, any Protected Party, Customer, data subjects, recipients, third-party providers or other customers to risk.",
          "We will inform Customer if we believe an instruction infringes Data Protection Laws, unless legally prohibited from doing so.",
          "Customer is solely responsible for the legality, accuracy, completeness and appropriateness of its instructions.",
        ],
        bullets: [
          "platform configuration;",
          "uploads;",
          "imports;",
          "field mapping;",
          "settings;",
          "API calls;",
          "integrations;",
          "activation actions;",
          "generation actions;",
          "export actions;",
          "deletion actions;",
          "support requests;",
          "written communications;",
          "use of specific features.",
        ],
      },
      {
        heading: "10. Customer Obligations",
        paragraphs: [
          "Customer is responsible for:",
          "Customer represents and warrants that:",
          "Customer must not use Velocity Vision to avoid, bypass, conceal or frustrate privacy rights, marketing rights, opt-out rights, data subject rights, platform rules or legal restrictions.",
        ],
        bullets: [
          "lawfulness of Customer Personal Data;",
          "accuracy of Customer Personal Data;",
          "quality of Customer Personal Data;",
          "source of Customer Personal Data;",
          "notices to data subjects;",
          "consent records where required;",
          "legitimate interest assessments where required;",
          "suppression lists;",
          "opt-out records;",
          "privacy policies;",
          "data subject response processes;",
          "outreach law compliance;",
          "cross-border transfer assessment;",
          "AI deployment compliance;",
          "client authority;",
          "agency client instructions;",
          "user permissions;",
          "internal approvals;",
          "regulated-sector obligations;",
          "determining whether use of Velocity Vision is lawful in each relevant jurisdiction.",
          "it has all rights, permissions, notices, consents, lawful bases and authorizations required to process Customer Personal Data through Velocity Vision;",
          "its instructions comply with Data Protection Laws;",
          "it will not upload or process prohibited or restricted data unless expressly authorized in writing;",
          "it will not use Velocity Vision for unlawful outreach, unlawful profiling, unlawful automated decision-making, unlawful AI deployment or unlawful marketing;",
          "it will respond to data subject requests unless applicable law requires Global Solutions Management LLC to respond directly.",
        ],
      },
      {
        heading: "11. Processor Obligations",
        paragraphs: [
          "Global Solutions Management LLC will:",
          "Global Solutions Management LLC does not determine the purposes of Customer outreach, Customer marketing, Customer recipient selection, Customer lawful basis, Customer AI deployment, Customer client instructions or Customer activation decisions.",
        ],
        bullets: [
          "process Customer Personal Data only in accordance with documented Customer instructions;",
          "ensure persons authorized to process Customer Personal Data are subject to confidentiality obligations;",
          "implement appropriate technical and organizational measures;",
          "assist Customer with data subject requests where reasonably possible and required by Data Protection Laws;",
          "assist Customer with security, breach, DPIA and regulatory obligations where reasonably possible and required by Data Protection Laws;",
          "use Subprocessors in accordance with this DPA;",
          "provide reasonable information to demonstrate compliance with this DPA;",
          "delete or return Customer Personal Data in accordance with this DPA;",
          "notify Customer of confirmed Security Incidents in accordance with this DPA.",
        ],
      },
      {
        heading: "12. Service Provider and Contractor Terms for US Privacy Laws",
        paragraphs: [
          "Where CCPA, CPRA or similar US state privacy laws apply, Global Solutions Management LLC acts as a Service Provider, Contractor or equivalent role for Customer Personal Data processed on behalf of Customer.",
          "Global Solutions Management LLC will not:",
          "Global Solutions Management LLC will provide the same level of privacy protection required of Service Providers or Contractors under applicable US state privacy laws to the extent those laws apply to Global Solutions Management LLC.",
          "Customer grants Global Solutions Management LLC the right to take reasonable and appropriate steps to ensure Customer Personal Data is processed consistently with Customer’s obligations under applicable privacy laws.",
          "Customer may take reasonable and appropriate steps to stop and remediate unauthorized use of Customer Personal Data, subject to the audit and information limits in this DPA.",
        ],
        bullets: [
          "sell Customer Personal Data;",
          "share Customer Personal Data for cross-context behavioral advertising;",
          "retain, use or disclose Customer Personal Data for any purpose other than the business purposes specified in this DPA or otherwise permitted by applicable law;",
          "retain, use or disclose Customer Personal Data for a commercial purpose other than the business purposes specified in this DPA or otherwise permitted by applicable law;",
          "retain, use or disclose Customer Personal Data outside the direct business relationship between Customer and Global Solutions Management LLC, except as permitted by applicable law;",
          "combine Customer Personal Data with Personal Data received from another source except as permitted by applicable law, including where necessary to detect security incidents, prevent fraud, debug, provide the Service, improve the Service, or perform permitted internal business purposes.",
        ],
      },
      {
        heading: "13. AI Processing",
        paragraphs: [
          "Velocity Vision may use AI-assisted tools and AI Subprocessors to provide features such as data quality review, segmentation support, template generation, outreach pack generation, summaries, follow-up suggestions, translations, multilingual variants, campaign assets, reports, prompt handling and workflow support.",
          "Customer instructs Global Solutions Management LLC to process Customer Personal Data through AI-assisted features when Customer uses, enables or configures those features.",
          "Customer is responsible for:",
          "Global Solutions Management LLC will not use Customer Personal Data to train a publicly available foundation model or general-purpose AI model except where Customer expressly authorizes that use or where the data has been aggregated, anonymised or de-identified in accordance with applicable law so that it no longer identifies Customer or data subjects.",
          "AI-generated outputs may contain Personal Data if Customer’s inputs contain Personal Data or if Customer instructs the platform to generate outputs using Personal Data. Customer remains responsible for reviewing, approving, lawfully using and lawfully retaining those outputs.",
        ],
        bullets: [
          "deciding whether Customer Personal Data may lawfully be processed by AI-assisted tools;",
          "deciding whether AI-assisted features are appropriate for the intended purpose;",
          "providing notices where required;",
          "obtaining consent where required;",
          "conducting data protection impact assessments where required;",
          "conducting AI impact assessments where required;",
          "assessing high-risk AI restrictions where required;",
          "assessing automated decision-making restrictions where required;",
          "reviewing AI-generated outputs;",
          "approving outputs before use;",
          "ensuring human oversight;",
          "ensuring compliance with AI, privacy, marketing, consumer protection and sector-specific laws.",
        ],
      },
      {
        heading: "14. AI Legal Role Allocation",
        paragraphs: [
          "Customer is responsible for determining whether its use of Velocity Vision makes Customer a deployer, user, provider, distributor, importer, operator, controller, business, employer, regulated entity or other legal role under applicable AI, privacy, marketing, consumer protection, employment, professional or sector-specific laws.",
          "Velocity Vision provides AI-assisted software tools and workflow support. Unless expressly agreed in writing, Global Solutions Management LLC does not assume Customer’s legal role, regulated status, professional obligations, deployment obligations, customer-facing obligations, human oversight obligations, impact assessment obligations, transparency obligations, risk management obligations, monitoring obligations or recordkeeping obligations.",
          "Customer must not use Velocity Vision in any way that would make Global Solutions Management LLC responsible for Customer’s regulated AI system, high-risk AI deployment, automated decision system, profiling activity, employment decision, credit decision, housing decision, education decision, insurance decision, medical decision, legal decision or other regulated decision.",
          "Customer is responsible for human oversight, review, approval, transparency notices, end-user disclosures, logs, monitoring, impact assessments and compliance records where required by applicable AI, privacy, employment, consumer protection, sector-specific or professional laws.",
        ],
      },
      {
        heading: "15. Automated Decision-Making, Profiling and High-Risk AI",
        paragraphs: [
          "Customer must not use Velocity Vision for legally significant automated decision-making, profiling, eligibility decisions, employment decisions, credit decisions, housing decisions, education access decisions, insurance decisions, healthcare decisions, criminal justice decisions, biometric identification, vulnerable-person targeting or other high-risk AI uses unless expressly authorized in a separate signed written agreement accepted by Global Solutions Management LLC.",
          "Customer remains solely responsible for determining whether its use of Customer Personal Data, AI-assisted features, Generated Outputs, segmentation, scoring, quality review or activation creates profiling, automated decision-making, high-risk AI use, regulated AI use or other legally restricted activity.",
          "We may suspend, restrict or refuse any AI Processing that we reasonably believe creates legal, regulatory, ethical, reputational, security, operational or data protection risk.",
        ],
      },
      {
        heading: "16. De-Identified and Aggregated Data",
        paragraphs: [
          "Global Solutions Management LLC may process aggregated, anonymised or de-identified data to maintain, secure, analyse, improve and develop Velocity Vision, provided the data does not identify Customer or data subjects.",
          "Where Data Protection Laws apply to de-identified data, Global Solutions Management LLC will take reasonable measures to ensure the data cannot reasonably be used to infer information about, or otherwise be linked to, a particular individual, will publicly commit where required to maintain and use the data in de-identified form, and will not attempt to re-identify the data except as permitted by law for testing the effectiveness of de-identification safeguards.",
          "Aggregated, anonymised and de-identified data is not Customer Personal Data where it no longer identifies or reasonably relates to an identifiable individual.",
        ],
      },
      {
        heading: "17. Confidentiality and Access Controls",
        paragraphs: [
          "Global Solutions Management LLC will ensure that personnel authorized to process Customer Personal Data are subject to confidentiality obligations.",
          "Access to Customer Personal Data will be limited to personnel, contractors and Subprocessors who need access for platform operation, support, security, billing, legal compliance, incident response, troubleshooting, maintenance or other permitted purposes.",
          "Customer is responsible for controlling access by its own users, employees, contractors, clients, agency users, administrators and invited users.",
        ],
      },
      {
        heading: "18. Technical and Organizational Measures",
        paragraphs: [
          "Global Solutions Management LLC will maintain technical and organizational measures appropriate to the nature, scope, context and risk of the processing.",
          "Measures may include, as appropriate:",
          "Customer acknowledges that no internet service, SaaS platform, AI service, email integration, hosting provider or cloud environment is completely secure, error-free or immune from attack.",
          "Customer is responsible for:",
        ],
        bullets: [
          "encrypted transmission;",
          "hosted access controls;",
          "role-based permissions;",
          "authentication controls;",
          "administrative access restrictions;",
          "logging;",
          "monitoring;",
          "backups;",
          "vulnerability management;",
          "malware protection;",
          "environment separation;",
          "provider access controls;",
          "operational procedures;",
          "incident response procedures;",
          "confidentiality obligations;",
          "subprocessor controls;",
          "security review of key providers;",
          "reasonable data minimisation within platform operations;",
          "availability and resilience measures appropriate to the Service.",
          "using strong credentials;",
          "managing user permissions;",
          "restricting workspace access;",
          "protecting connected email accounts;",
          "protecting third-party integrations;",
          "securing devices;",
          "securing exports;",
          "training users;",
          "maintaining internal policies;",
          "promptly reporting suspected compromise;",
          "keeping independent copies of important records.",
        ],
      },
      {
        heading: "19. Security Measures Schedule",
        paragraphs: [
          "The security measures under this DPA are designed for the Velocity Vision platform and may evolve over time.",
          "Customer acknowledges that specific security controls may depend on the hosting environment, third-party providers, plan, feature set, integration, account configuration, product maturity, operational needs and risk profile.",
          "Global Solutions Management LLC may update security measures provided the overall level of security is not materially reduced in a way that creates unreasonable risk to Customer Personal Data.",
          "Security measures do not create any guarantee that Customer Personal Data will be immune from every security threat, vulnerability, attack, unauthorized access or incident.",
        ],
      },
      {
        heading: "20. Subprocessors",
        paragraphs: [
          "Customer gives Global Solutions Management LLC general authorization to appoint Subprocessors to provide Velocity Vision.",
          "Subprocessors may be used for:",
          "Global Solutions Management LLC will impose data protection obligations on Subprocessors that are materially consistent with this DPA and appropriate to the nature of the services provided.",
          "Global Solutions Management LLC remains responsible for Subprocessors’ processing of Customer Personal Data to the extent required by applicable Data Protection Laws and subject to the limitations of liability in the Platform Terms of Service, Customer Agreement and this DPA.",
        ],
        bullets: [
          "hosting;",
          "cloud infrastructure;",
          "data storage;",
          "authentication;",
          "payments;",
          "analytics;",
          "monitoring;",
          "logging;",
          "security;",
          "communications;",
          "support;",
          "email connectivity;",
          "AI Processing;",
          "translation;",
          "CRM tools;",
          "domain services;",
          "backup;",
          "incident response;",
          "platform operations;",
          "other infrastructure and operational services.",
        ],
      },
      {
        heading: "21. Subprocessor Information and Objections",
        paragraphs: [
          "Where legally required, Customer may request current Subprocessor information through the Contact page or applicable Subprocessor information page if made available.",
          "Global Solutions Management LLC may add, replace or remove Subprocessors from time to time.",
          "Where legally required, Global Solutions Management LLC will provide notice of new Subprocessors by posting an updated Subprocessor list, platform notice, email notice or other reasonable method.",
          "If Customer reasonably objects to a new Subprocessor on data protection grounds, Customer must notify Global Solutions Management LLC within the stated notice period, or if no period is stated, within 15 days after notice.",
          "Customer’s sole remedy if an objection cannot reasonably be resolved is to stop using the affected feature or terminate the affected Service in accordance with the Customer Agreement. Objection to a Subprocessor does not create a refund right unless expressly required by law.",
        ],
      },
      {
        heading: "22. Machine Translation",
        paragraphs: [
          "The platform may display public legal pages, interface text or rendered page text using automated machine-translation technology, including GTranslate, Google Translate or similar services.",
          "Machine translation is provided for convenience only. The English version controls.",
          "Machine translation is not intended to translate Customer Personal Data for storage in the platform database unless a specific translation feature is made available and Customer chooses to use it.",
          "Where a translation feature processes Customer Personal Data, that processing is subject to this DPA and applicable Subprocessor terms.",
          "Customer must not rely on machine translation as legal, compliance, regulatory or professional translation.",
        ],
      },
      {
        heading: "23. International Transfers",
        paragraphs: [
          "Customer Personal Data may be processed in the United States, United Kingdom, European Economic Area and other jurisdictions where Global Solutions Management LLC or its Subprocessors operate.",
          "Customer authorizes Global Solutions Management LLC and its Subprocessors to process Customer Personal Data in these jurisdictions as necessary to provide Velocity Vision.",
          "Customer is responsible for ensuring that Customer Personal Data may lawfully be transferred to, accessed from, processed in and stored in all relevant jurisdictions.",
          "Where Data Protection Laws require a transfer mechanism, transfers may rely on one or more of the following, as applicable:",
        ],
        bullets: [
          "adequacy decisions;",
          "the European Commission’s standard contractual clauses;",
          "Swiss transfer mechanisms;",
          "data privacy frameworks;",
          "approved certification mechanisms;",
          "binding corporate rules;",
          "derogations;",
          "other lawful transfer mechanisms.",
        ],
      },
      {
        heading: "24. Standard Contractual Clauses",
        paragraphs: [
          "Where Customer Personal Data subject to GDPR is transferred to Global Solutions Management LLC or a Subprocessor in a country that does not provide adequate protection, the applicable European Commission standard contractual clauses are incorporated by reference and apply as required by Data Protection Laws.",
          "For transfers from Customer as Controller to Global Solutions Management LLC as Processor, Module Two applies.",
          "For transfers from Customer as Processor to Global Solutions Management LLC as Subprocessor, Module Three applies.",
          "For onward transfers to Subprocessors, Global Solutions Management LLC will use an appropriate lawful transfer mechanism where required.",
          "The details of processing in this DPA form the relevant annexes to the standard contractual clauses to the extent applicable.",
          "If there is a conflict between this DPA and the standard contractual clauses, the standard contractual clauses control only to the extent necessary for the relevant restricted transfer.",
        ],
      },
      {
        heading: "25. UK and Swiss Transfers",
        paragraphs: [
          "Where Customer Personal Data subject to UK GDPR is transferred to Global Solutions Management LLC or a Subprocessor in a country that does not provide adequate protection, the applicable UK transfer mechanism, including the UK International Data Transfer Addendum or UK International Data Transfer Agreement, is incorporated by reference where required.",
          "Where Customer Personal Data subject to Swiss data protection law is transferred to Global Solutions Management LLC or a Subprocessor in a country that does not provide adequate protection, the applicable transfer mechanism will be interpreted to cover Swiss requirements where required.",
          "Customer is responsible for determining whether transfer impact assessments, supplementary measures, notices or additional safeguards are required for Customer’s use case.",
        ],
      },
      {
        heading: "26. Transfer Impact Assessments",
        paragraphs: [
          "Taking into account the nature of processing and information available to Global Solutions Management LLC, we will provide reasonable information to assist Customer with transfer impact assessments where required by Data Protection Laws.",
          "Assistance outside standard documentation, security information, Subprocessor information or platform functionality may be charged at reasonable rates.",
          "Customer remains responsible for determining whether transfers are lawful for its Customer Personal Data, data subjects, industry, jurisdiction and use case.",
        ],
      },
      {
        heading: "27. Data Privacy Framework",
        paragraphs: [
          "If Global Solutions Management LLC or a relevant Subprocessor is certified under an applicable data privacy framework, including the EU-U.S. Data Privacy Framework, UK Extension or Swiss-U.S. Data Privacy Framework, transfers may rely on that framework where legally available.",
          "If a data privacy framework is invalidated, suspended, unavailable or no longer applicable, transfers may rely on standard contractual clauses, UK transfer mechanisms, Swiss transfer mechanisms or another lawful transfer mechanism.",
          "Nothing in this DPA requires Global Solutions Management LLC to maintain certification under any data privacy framework unless expressly agreed in writing.",
        ],
      },
      {
        heading: "28. Data Subject Requests",
        paragraphs: [
          "Taking into account the nature of processing and information available to Global Solutions Management LLC, we will provide reasonable assistance to Customer for data subject access, deletion, correction, portability, objection, restriction, opt-out, consent withdrawal and similar rights requests where required by Data Protection Laws.",
          "Where available, Customer should use platform tools to search, access, export, correct or delete Customer Personal Data.",
          "If Global Solutions Management LLC receives a request directly from a data subject relating to Customer Personal Data, we may redirect the request to Customer unless legally required to respond directly.",
          "Customer is responsible for verifying the identity and authority of requesters, determining whether a request is valid, determining applicable exemptions and responding within legally required timeframes.",
          "Assistance outside standard platform functionality may be charged at reasonable rates.",
        ],
      },
      {
        heading: "29. Regulatory Assistance and DPIAs",
        paragraphs: [
          "Taking into account the nature of processing and information available to Global Solutions Management LLC, we will provide reasonable assistance to Customer where required by Data Protection Laws for:",
          "Customer remains responsible for determining whether a DPIA, AI impact assessment, legitimate interest assessment, transfer impact assessment, prior consultation, regulatory notice or other compliance step is required.",
          "Assistance outside standard documentation or platform functionality may be charged at reasonable rates.",
        ],
        bullets: [
          "data protection impact assessments;",
          "AI impact assessments;",
          "legitimate interest assessments;",
          "prior consultations with regulators;",
          "security assessments;",
          "breach assessments;",
          "records of processing;",
          "regulatory inquiries;",
          "processor compliance information.",
        ],
      },
      {
        heading: "30. Legal, Regulatory and Third-Party Requests",
        paragraphs: [
          "If Global Solutions Management LLC receives a subpoena, court order, regulator request, law enforcement request, data subject request, rights-holder complaint, payment-provider request, email-provider request, AI-provider request, third-party platform request, security complaint or legal notice relating to Customer Personal Data, Customer’s users, Customer’s outreach, Customer’s clients or Customer’s workspace, we may respond, disclose information, preserve records, restrict access, notify Customer or take other action as we consider legally required or reasonably appropriate.",
          "Where legally permitted and commercially reasonable, we may notify Customer of such requests.",
          "We are not required to challenge any legal, regulatory, payment-provider, email-provider, AI-provider or third-party request unless required by law or expressly agreed in writing.",
          "Customer is responsible for costs, losses, fees, penalties, legal fees and operational burden arising from requests, complaints, investigations or claims caused by Customer Personal Data, Customer outreach, Customer instructions, Customer users, Customer clients, Customer integrations or Customer’s use of Velocity Vision.",
        ],
      },
      {
        heading: "31. Security Incidents",
        paragraphs: [
          "Global Solutions Management LLC will notify affected Customers without undue delay after becoming aware of a confirmed Security Incident affecting Customer Personal Data.",
          "Notification may include, where available:",
          "Notification of a Security Incident is not an admission of fault, liability, unlawful conduct or breach of this DPA.",
          "Customer remains responsible for determining whether regulator, data subject, client, recipient, contractual or other notifications are required, unless applicable law requires Global Solutions Management LLC to notify directly.",
          "Customer must notify Global Solutions Management LLC promptly of any suspected or actual compromise involving Customer’s account, users, credentials, devices, exports, connected email accounts, integrations or Customer-side systems.",
        ],
        bullets: [
          "the nature of the Security Incident;",
          "categories of affected data;",
          "approximate number of affected data subjects or records, where known;",
          "likely consequences, where known;",
          "mitigation steps taken or proposed;",
          "recommended Customer actions;",
          "contact route for follow-up.",
        ],
      },
      {
        heading: "32. Security Incident Exclusions",
        paragraphs: [
          "The following are not Security Incidents caused by Global Solutions Management LLC unless directly caused by our breach of this DPA:",
          "Customer remains responsible for incidents arising from Customer-side systems, users, instructions, exports, integrations, sender accounts and data handling.",
        ],
        bullets: [
          "Customer-side credential compromise;",
          "weak passwords;",
          "shared accounts;",
          "unauthorized internal users;",
          "compromised Customer devices;",
          "compromised Customer email accounts;",
          "compromised Customer integrations;",
          "unsafe exports;",
          "Customer-side malware;",
          "Customer misconfiguration;",
          "Customer disclosure;",
          "Customer user error;",
          "agency client misuse;",
          "third-party provider incidents outside our reasonable control;",
          "events caused by Customer instructions.",
        ],
      },
      {
        heading: "33. Return, Deletion and Retention",
        paragraphs: [
          "During the account term, Customer may export or delete certain Customer Personal Data through available platform tools, subject to plan limits, technical limits, security checks, payment status and policy compliance.",
          "After termination, verified deletion request or expiry of the account, Global Solutions Management LLC will delete or return Customer Personal Data within a reasonable period unless retention is required or permitted for:",
          "Backups, logs and archival copies may persist for a limited period before ordinary deletion cycles complete.",
          "Global Solutions Management LLC is not required to delete records that it is legally required or reasonably permitted to retain, provided such retained records are protected in accordance with this DPA and applicable law.",
        ],
        bullets: [
          "legal compliance;",
          "tax;",
          "audit;",
          "security;",
          "billing;",
          "dispute handling;",
          "fraud prevention;",
          "abuse prevention;",
          "sanctions compliance;",
          "regulatory compliance;",
          "backup integrity;",
          "platform integrity;",
          "enforcement;",
          "legitimate business records.",
        ],
      },
      {
        heading: "34. Audit and Information Rights",
        paragraphs: [
          "Global Solutions Management LLC will make available reasonable information necessary to demonstrate compliance with this DPA where required by Data Protection Laws.",
          "Customer must first rely on available documentation, policies, security summaries, Subprocessor information, certifications, audit summaries or written responses.",
          "On-site audits are available only where legally required and only after:",
          "Audits must be limited to once per calendar year unless a confirmed Security Incident affecting Customer Personal Data requires additional audit rights under mandatory law.",
          "Customer may not use audits to access other customers’ data, platform source code, security-sensitive information, confidential information, trade secrets, privileged materials or information that would increase security risk.",
          "We may refuse or limit audit requests that are excessive, abusive, commercially unreasonable, legally privileged, security-sensitive, duplicative, unrelated to Customer Personal Data or not required by Data Protection Laws.",
        ],
        bullets: [
          "reasonable written notice;",
          "a written audit scope;",
          "confidentiality undertakings;",
          "security restrictions;",
          "confirmation that the audit will not compromise other customers, systems, security, confidentiality, trade secrets or platform operations;",
          "reimbursement of reasonable costs;",
          "scheduling during normal business hours.",
        ],
      },
      {
        heading: "35. Records and Logs",
        paragraphs: [
          "Global Solutions Management LLC may maintain platform logs, security logs, access logs, billing logs, usage logs, system logs, audit records and support records for security, operations, fraud prevention, billing, legal compliance, dispute handling, enforcement and platform integrity.",
          "Customer acknowledges that logs may contain limited Personal Data.",
          "Logs are retained for periods determined by operational, legal, security, billing and compliance needs.",
          "Customer is responsible for maintaining its own compliance records, consent records, lawful basis records, suppression records, outreach records, AI deployment records and transfer records.",
        ],
      },
      {
        heading: "36. Agency and Client Workspaces",
        paragraphs: [
          "Where Customer operates as an agency, consultant, reseller, service provider or intermediary for its own clients, Customer remains responsible for:",
          "Global Solutions Management LLC does not contract directly with Customer’s end clients unless expressly agreed in writing.",
          "Customer must ensure that its end clients and client users comply with this DPA, the Platform Terms of Service, Customer Agreement and applicable Data Protection Laws.",
          "Customer indemnifies the Protected Parties for claims arising from agency client data, client instructions, client workspaces, client outreach, client AI deployment or end-client disputes.",
        ],
        bullets: [
          "client authority;",
          "client instructions;",
          "client data;",
          "client privacy notices;",
          "client lawful basis;",
          "client consents;",
          "client suppression lists;",
          "client opt-outs;",
          "client data subject requests;",
          "client outreach compliance;",
          "client AI deployment compliance;",
          "client transfer compliance;",
          "end-client disputes.",
        ],
      },
      {
        heading: "37. Cross-Border and Global Compliance",
        paragraphs: [
          "Velocity Vision may be accessed globally, but not all data types, outreach activities, AI features, transfers, storage locations, integrations or use cases are lawful in every jurisdiction.",
          "Customer is responsible for determining whether use of Velocity Vision is lawful in every country, state, province, territory, sector and recipient location relevant to Customer, Customer Personal Data, Customer users, Customer clients, data subjects, recipients and campaigns.",
          "Customer must not use Velocity Vision where doing so would breach local privacy, marketing, AI, data localization, secrecy, export control, sanctions, sector-specific or professional rules.",
          "We may restrict, suspend or refuse processing where we reasonably believe there is legal, regulatory, sanctions, security, payment, operational, reputational or third-party provider risk.",
        ],
      },
      {
        heading: "38. Sanctions, Export Control and Restricted Jurisdictions",
        paragraphs: [
          "Customer must not upload, process, transfer, export or make available Customer Personal Data through Velocity Vision in violation of applicable sanctions, export control, trade control, anti-bribery, anti-corruption or anti-money laundering laws.",
          "Customer represents and warrants that neither Customer nor its users are prohibited from using Velocity Vision under applicable sanctions, export control or trade laws.",
          "We may restrict, suspend, delete, block, refuse or terminate processing where sanctions, export control, trade compliance, payment or jurisdictional risk is identified or suspected.",
        ],
      },
      {
        heading: "39. Monitoring and Enforcement",
        paragraphs: [
          "We may, but are not required to, monitor, scan, log, review, analyse, investigate, preserve or restrict accounts, workspaces, Customer Personal Data, Generated Outputs, usage patterns, sender connections, integrations, activation activity, exports, billing activity and platform signals to operate the platform, improve safety, prevent abuse, enforce this DPA, comply with law, respond to complaints, protect infrastructure, protect recipients, protect third-party providers and protect our business.",
          "No monitoring, review, automated check, manual check, warning, flag, hold, approval flow or failure to act creates any responsibility for Customer’s data, outputs, campaigns, compliance, outreach or legal obligations.",
          "We may use automated systems, human review, third-party tools and risk scoring to identify suspected abuse, fraud, unlawful activity, security risk, policy breach or platform risk.",
        ],
      },
      {
        heading: "40. Takedown, Quarantine and Refusal",
        paragraphs: [
          "We may remove, disable, restrict, quarantine, return, delete or block any Customer Personal Data, Generated Output, file, campaign, message, sender connection, integration, export or workspace content that we reasonably believe may violate law, infringe rights, create risk, breach this DPA, expose the platform to liability or harm third parties.",
          "We are not liable for loss arising from removal, blocking, disabling, quarantine, moderation, restriction or refusal of content, outputs, campaigns or data where we act to protect the platform, users, recipients, third-party providers, rights holders, regulators, data subjects or our business.",
        ],
      },
      {
        heading: "41. Liability",
        paragraphs: [
          "Liability under this DPA is subject to the limitations, exclusions and caps in the Platform Terms of Service and Customer Agreement, except to the extent those limitations are prohibited by applicable Data Protection Laws.",
          "To the maximum extent permitted by law, the total aggregate liability of the Protected Parties arising out of or relating to this DPA is included within and does not increase the total aggregate liability cap under the Platform Terms of Service and Customer Agreement.",
          "Nothing in this DPA excludes or limits liability where liability cannot lawfully be excluded or limited.",
        ],
      },
      {
        heading: "42. Indemnity",
        paragraphs: [
          "Customer agrees to defend, indemnify and hold harmless the Protected Parties from and against all claims, demands, investigations, complaints, proceedings, losses, liabilities, damages, penalties, fines, settlements, costs and expenses, including reasonable legal fees, arising from or related to:",
          "We may control the defence of any matter subject to indemnity. Customer must cooperate with us and must not settle any claim in a way that imposes obligations, admissions, restrictions or liability on any Protected Party without prior written consent.",
          "Customer’s indemnity obligations apply to the maximum extent permitted by law, including where claims are brought by customers, end clients, recipients, users, regulators, authorities, payment providers, email providers, AI providers, data subjects, rights holders, competitors or third-party platforms.",
        ],
        bullets: [
          "Customer Personal Data;",
          "Customer instructions;",
          "Customer’s breach of this DPA;",
          "Customer’s breach of Data Protection Laws;",
          "unlawful data collection;",
          "unlawful data sourcing;",
          "unlawful outreach;",
          "lack of consent;",
          "lack of lawful basis;",
          "deficient privacy notices;",
          "suppression failures;",
          "opt-out failures;",
          "data subject requests;",
          "agency client data;",
          "end-client instructions;",
          "regulated data uploaded without authorization;",
          "prohibited data uploaded without authorization;",
          "Customer-side security incidents;",
          "Customer integrations;",
          "Customer exports;",
          "Customer AI deployment;",
          "Customer use of Generated Outputs;",
          "international transfer issues caused by Customer’s use case;",
          "regulatory complaints caused by Customer’s data, instructions, outreach or use.",
        ],
      },
      {
        heading: "43. Local Law and Mandatory Rights",
        paragraphs: [
          "This DPA applies to the maximum extent permitted by applicable law.",
          "If mandatory law in a jurisdiction gives Customer or data subjects rights that cannot be excluded, restricted or modified, this DPA does not exclude those rights, but all exclusions, limitations, disclaimers, indemnities and protections continue to apply to the maximum extent permitted by that law.",
          "Where a provision cannot be enforced as written in a particular jurisdiction, it will be interpreted, narrowed or reformed only to the minimum extent necessary to make it enforceable, and the rest of the provision and this DPA remain effective.",
        ],
      },
      {
        heading: "44. Governing Law and Jurisdiction",
        paragraphs: [
          "This DPA is governed by the laws of the State of Delaware, United States, except where Data Protection Laws, standard contractual clauses, UK transfer mechanisms, Swiss transfer mechanisms or mandatory law require another governing law for a specific data protection issue.",
          "Subject to any mandatory law that cannot be waived, the state and federal courts located in Delaware have exclusive jurisdiction over disputes arising out of or relating to this DPA.",
          "Despite the above, Global Solutions Management LLC may seek injunctive relief, debt recovery, payment enforcement, intellectual property enforcement, confidentiality enforcement, security enforcement, data protection enforcement or urgent protective relief in any court or forum of competent jurisdiction.",
        ],
      },
      {
        heading: "45. Order of Precedence for Data Transfer Terms",
        paragraphs: [
          "If standard contractual clauses, UK transfer mechanisms, Swiss transfer mechanisms or mandatory Data Protection Laws apply and conflict with this DPA, those transfer terms or mandatory laws control only to the extent required for the restricted transfer or specific mandatory data protection issue.",
          "All other terms of this DPA, the Platform Terms of Service and Customer Agreement continue to apply to the maximum extent permitted by law.",
        ],
      },
      {
        heading: "46. Changes to this DPA",
        paragraphs: [
          "Global Solutions Management LLC may update this DPA from time to time to reflect product changes, legal developments, security requirements, operational needs, Subprocessor changes, AI provider changes, infrastructure changes, transfer mechanism changes or changes to Data Protection Laws.",
          "Material updates may be notified by posting, email, platform notice or other reasonable method.",
          "Continued use of Velocity Vision after the effective date of an updated DPA means Customer accepts the updated DPA.",
          "If Customer does not agree to an updated DPA, Customer must stop using Velocity Vision and cancel the applicable account or plan.",
        ],
      },
      {
        heading: "47. Term and Survival",
        paragraphs: [
          "This DPA remains in effect for as long as Global Solutions Management LLC processes Customer Personal Data on behalf of Customer.",
          "Provisions that by their nature should survive termination will survive, including provisions relating to confidentiality, deletion, retention, audits, liability, indemnity, no personal liability, international transfers, records, regulatory requests, local law, governing law and enforcement.",
        ],
      },
      {
        heading: "48. Contact",
        paragraphs: [
          "For privacy and data processing inquiries, use the Contact page and select the route that matches your request.",
          "Global Solutions Management LLC Delaware, United States Operator of Velocity Vision",
        ],
      },
    ],
  },
  "privacy-policy": {
    title: "Privacy Policy",
    effectiveDate: "30 June 2026",
    lastUpdated: "1 July 2026",
    version: "8.0",
    sections: [
      {
        heading: "Overview",
        paragraphs: [
          `This Privacy Policy explains how Global Solutions Management LLC collects, uses, shares, stores, transfers and protects personal information in connection with the ${PLATFORM} website, platform, workspaces, Data Vault, AI-assisted tools, generated outputs, support, billing, communications, security, legal operations and related services.`,
          "This Privacy Policy should be read together with the Platform Terms of Service, Customer Agreement, Data Processing Agreement, Cookie Policy, Acceptable Use Policy, Marketing Compliance Policy, Platform Security Policy, Service Level Agreement and any other applicable platform terms.",
          "This document may be displayed in your browser's preferred language using an automated machine-translation layer, including GTranslate, Google Translate or similar translation technology. Translations are provided for convenience only. The English version controls if there is any conflict, ambiguity, inconsistency, error or difference between versions.",
        ],
      },
      {
        heading: "1. Who We Are",
        paragraphs: [
          `${PLATFORM} is operated by Global Solutions Management LLC, a company incorporated in the State of Delaware, United States.`,
          "For personal information that we process for our own business purposes, Global Solutions Management LLC may act as a controller, business or equivalent role under applicable privacy laws.",
          "For Customer Personal Data uploaded, imported, generated, activated, exported or otherwise processed by customers through the platform, the customer is usually the controller, business or equivalent determining party, and Global Solutions Management LLC usually acts as processor, service provider, contractor or equivalent processing party under the Data Processing Agreement.",
        ],
      },
      {
        heading: "2. Delaware Operator and Contract Stack",
        paragraphs: [
          `Global Solutions Management LLC is the operating entity for ${PLATFORM}.`,
          "The Platform Terms of Service, Customer Agreement and Data Processing Agreement contain important provisions on customer responsibility, disclaimers, liability limits, indemnities, no personal liability, director and staff protection, dispute resolution, governing law and enforcement.",
          `To the maximum extent permitted by law, claims relating to ${PLATFORM}, customer workspace data, customer instructions, customer outreach, customer AI use, customer exports, customer integrations or customer-controlled processing are governed by the Platform Terms of Service, Customer Agreement and Data Processing Agreement.`,
          "This Privacy Policy is a privacy notice. It does not create contractual rights beyond those required by applicable law.",
        ],
      },
      {
        heading: "3. Protected Parties",
        paragraphs: [
          "For clarity, references to protections for Global Solutions Management LLC in the contract stack are intended to protect Global Solutions Management LLC and its present and former owners, members, managers, directors, officers, employees, staff, contractors, consultants, agents, representatives, affiliates, licensors, suppliers, service providers, subprocessors, payment providers, infrastructure providers, AI providers, hosting providers, professional advisers, successors and assigns.",
          "Nothing in this Privacy Policy excludes rights or obligations that cannot legally be excluded.",
        ],
      },
      {
        heading: "4. Scope of this Privacy Policy",
        paragraphs: ["This Privacy Policy applies to personal information processed in connection with:"],
        bullets: [
          `the ${PLATFORM} website;`,
          "account creation;",
          "user login and authentication;",
          "customer workspaces;",
          "Data Vault features;",
          "AI-assisted features;",
          "generated outputs;",
          "billing and subscriptions;",
          "credits and plan usage;",
          "support communications;",
          "service communications;",
          "legal communications;",
          "security monitoring;",
          "product analytics;",
          "cookies and similar technologies;",
          "platform operations;",
          "compliance and enforcement.",
          "This Privacy Policy does not replace the privacy notices of our customers. Customers remain responsible for their own privacy notices, lawful bases, consent records, suppression records, outreach activity, recipient communications, customer lists, agency client data and use of generated outputs.",
        ],
      },
      {
        heading: "5. Business Use Only",
        paragraphs: [
          `${PLATFORM} is designed for lawful business, professional, commercial, agency, organizational and entrepreneurial use.`,
          "It is not intended for personal, household or consumer use.",
          "Customers must not upload personal, household, children's, sensitive, regulated or restricted data unless expressly authorized in writing and lawful for the intended processing.",
        ],
      },
      {
        heading: "6. Customer-Uploaded Data and Customer Responsibility",
        paragraphs: [
          `Customers control what data they upload, import, generate, activate, export and use through ${PLATFORM}.`,
          "Customer-uploaded data may include contact records, company records, names, email addresses, telephone numbers, job titles, notes, tags, source fields, replies, pipeline status, relationship history, suppression information, campaign records and other fields chosen by the customer.",
          "Where we process Customer Personal Data on behalf of a customer, we do so according to the customer's instructions and the Data Processing Agreement.",
          "Customers are responsible for:",
        ],
        bullets: [
          "deciding what data to upload;",
          "having a lawful basis for processing;",
          "providing privacy notices to their contacts;",
          "maintaining consent records where required;",
          "maintaining legitimate interest assessments where required;",
          "respecting opt-outs and unsubscribe requests;",
          "maintaining suppression records;",
          "ensuring outreach is lawful;",
          "reviewing AI-generated outputs;",
          "responding to data subject requests relating to their workspace data;",
          "complying with local privacy, marketing, AI, consumer protection, platform and sector-specific laws.",
          "If you are a contact, prospect, recipient, customer, employee, supplier or other person whose data appears in a customer workspace, please contact the relevant customer first. We may redirect requests relating to customer-controlled workspace data to that customer unless the law requires us to respond directly.",
        ],
      },
      {
        heading: "7. Information We Collect",
        paragraphs: ["We may collect the following categories of personal information."],
      },
      {
        heading: "7.1 Account and Identity Information",
        paragraphs: ["This may include:"],
        bullets: [
          "name;",
          "email address;",
          "company name;",
          "role;",
          "username;",
          "login information;",
          "authentication identifiers;",
          "workspace role;",
          "team membership;",
          "account settings;",
          "communication preferences.",
        ],
      },
      {
        heading: "7.2 Billing and Commercial Information",
        paragraphs: ["This may include:"],
        bullets: [
          "billing contact details;",
          "billing address;",
          "company details;",
          "tax information;",
          "VAT, GST or sales tax details;",
          "subscription plan;",
          "credit purchases;",
          "top-ups;",
          "invoices;",
          "payment status;",
          "transaction records;",
          "payment processor references;",
          "renewal records;",
          "cancellation records;",
          "dispute records;",
          "chargeback records.",
          "Payment card or bank details may be processed by payment providers. We do not intentionally store full payment card numbers where a payment processor provides tokenised payment processing.",
        ],
      },
      {
        heading: "7.3 Platform Usage Information",
        paragraphs: ["This may include:"],
        bullets: [
          "pages viewed;",
          "features used;",
          "uploads;",
          "exports;",
          "settings;",
          "workspace actions;",
          "activation actions;",
          "generation actions;",
          "AI-assisted feature usage;",
          "credit usage;",
          "plan limits;",
          "integrations;",
          "support requests;",
          "error logs;",
          "audit logs;",
          "security logs;",
          "timestamps;",
          "usage patterns.",
        ],
      },
      {
        heading: "7.4 Customer Workspace Data",
        paragraphs: ["This may include data uploaded, imported, entered, generated, activated or exported by customers, such as:"],
        bullets: [
          "contact records;",
          "company records;",
          "business contact details;",
          "professional profiles;",
          "tags;",
          "notes;",
          "source information;",
          "reply data;",
          "campaign data;",
          "pipeline information;",
          "suppression status;",
          "opt-out information;",
          "generated drafts;",
          "AI-assisted outputs;",
          "reports;",
          "customer-configured fields.",
        ],
      },
      {
        heading: "7.5 Technical and Device Information",
        paragraphs: ["This may include:"],
        bullets: [
          "IP address;",
          "device information;",
          "browser type;",
          "operating system;",
          "approximate location;",
          "language settings;",
          "cookie identifiers;",
          "session identifiers;",
          "diagnostics;",
          "log data;",
          "security signals;",
          "referral URLs;",
          "pages visited before or after using the website.",
        ],
      },
      {
        heading: "7.6 Support and Communications Information",
        paragraphs: ["This may include:"],
        bullets: [
          "messages sent to us;",
          "support tickets;",
          "contact form submissions;",
          "feedback;",
          "service inquiries;",
          "legal inquiries;",
          "privacy requests;",
          "attachments provided to support;",
          "communication history;",
          "call or meeting details where applicable.",
        ],
      },
      {
        heading: "7.7 Information from Third Parties",
        paragraphs: ["We may receive information from:"],
        bullets: [
          "payment providers;",
          "authentication providers;",
          "hosting providers;",
          "analytics providers;",
          "communications tools;",
          "security providers;",
          "connected integrations;",
          "customer administrators;",
          "agency customers;",
          "public sources;",
          "commercially available sources where lawful;",
          "professional advisers;",
          "regulators;",
          "law enforcement;",
          "third-party platforms where lawful and relevant.",
        ],
      },
      {
        heading: "8. Sources of Personal Information",
        paragraphs: ["We may collect personal information from:"],
        bullets: [
          "you directly;",
          "your employer or organization;",
          "customer administrators;",
          "agency customers;",
          "invited workspace users;",
          "customer uploads;",
          "support communications;",
          "billing interactions;",
          "website forms;",
          "cookies and similar technologies;",
          "connected services;",
          "payment providers;",
          "authentication providers;",
          "analytics providers;",
          "security tools;",
          "public or commercially available sources where lawful;",
          "regulators, law enforcement or professional advisers where relevant.",
        ],
      },
      {
        heading: "9. How We Use Personal Information",
        paragraphs: ["We may use personal information for the following purposes:"],
        bullets: [
          `to provide ${PLATFORM};`,
          "to create and manage accounts;",
          "to operate workspaces;",
          "to provide Data Vault features;",
          "to support data mapping, quality review, deduplication and segmentation;",
          "to provide AI-assisted tools and generated outputs;",
          "to support governed activation and workflow controls;",
          "to provide replies, follow-up, pipeline, exports and reports;",
          "to process subscriptions, credits, top-ups, renewals, invoices and taxes;",
          "to authenticate users;",
          "to manage permissions and workspace access;",
          "to provide support;",
          "to send service notices, billing notices, legal notices and security alerts;",
          "to detect fraud, abuse, policy breaches and security threats;",
          "to protect the platform, users, customers, recipients and third-party providers;",
          "to monitor reliability, performance and usage;",
          "to improve product quality, safety and usability;",
          "to comply with legal, tax, accounting, regulatory and sanctions obligations;",
          "to enforce agreements and platform policies;",
          "to respond to disputes, legal requests and regulatory inquiries;",
          "to manage corporate transactions;",
          "to protect our rights, business, infrastructure, personnel and legal position.",
        ],
      },
      {
        heading: "10. Lawful Bases for Processing",
        paragraphs: ["Where GDPR, UK GDPR or similar laws apply, our lawful bases may include:"],
        bullets: [
          "contract: to provide accounts, subscriptions, workspaces, support, billing and platform services;",
          `legitimate interests: to operate, secure, improve, monitor, support and protect ${PLATFORM}, prevent abuse, enforce terms, maintain records and communicate with business users;`,
          "legal obligation: to comply with tax, accounting, legal, sanctions, regulatory, security and law enforcement obligations;",
          "consent: where required for certain cookies, marketing communications or optional processing;",
          "vital interests or public interest: where applicable in exceptional circumstances.",
          "For Customer Personal Data processed on behalf of customers, the customer is responsible for identifying and maintaining the lawful basis for the customer's own processing.",
        ],
      },
      {
        heading: "11. AI-Assisted Features and Generated Outputs",
        paragraphs: [`${PLATFORM} may use AI-assisted tools and AI service providers to provide features such as:`],
        bullets: [
          "data quality review;",
          "segmentation support;",
          "template generation;",
          "outreach pack generation;",
          "social post drafting;",
          "press release drafting;",
          "video script drafting;",
          "follow-up suggestions;",
          "multilingual variants;",
          "summaries;",
          "reports;",
          "prompt handling;",
          "workflow recommendations.",
          "Inputs, prompts, context, Customer Personal Data and generated outputs may be processed by us and by service providers as necessary to provide, secure, monitor, troubleshoot and improve these features.",
          "Customers are responsible for deciding whether personal information may lawfully be used with AI-assisted features and for reviewing, approving and lawfully using generated outputs.",
          "AI-generated outputs are drafts. They may be inaccurate, incomplete, outdated, repetitive, biased, unsuitable or non-compliant. Customers should not rely on AI-generated outputs as legal, tax, accounting, financial, medical, employment, investment, regulatory or professional advice.",
          "We do not use Customer Personal Data to train a publicly available foundation model or general-purpose AI model unless the customer expressly authorizes that use or the data has been aggregated, anonymised or de-identified so that it no longer identifies the customer or data subjects.",
        ],
      },
      {
        heading: "12. AI Legal Role Allocation",
        paragraphs: [
          `Customers are responsible for determining whether their use of ${PLATFORM} makes them a deployer, user, provider, distributor, importer, operator, controller, business, employer, regulated entity or other legal role under applicable AI, privacy, marketing, consumer protection, employment, professional or sector-specific laws.`,
          `${PLATFORM} provides AI-assisted software tools and workflow support. Unless expressly agreed in writing, Global Solutions Management LLC does not assume the customer's legal role, regulated status, professional obligations, deployment obligations, customer-facing obligations, human oversight obligations, impact assessment obligations, transparency obligations, risk management obligations, monitoring obligations or recordkeeping obligations.`,
          `Customers must not use ${PLATFORM} in any way that would make Global Solutions Management LLC responsible for the customer's regulated AI system, high-risk AI deployment, automated decision system, profiling activity, employment decision, credit decision, housing decision, education decision, insurance decision, medical decision, legal decision or other regulated decision.`,
        ],
      },
      {
        heading: "13. Automated Decision-Making and Profiling",
        paragraphs: [
          `${PLATFORM} is designed as an AI-assisted, customer-controlled commercial workspace.`,
          "We do not intend to use customer-uploaded contact data to make legally significant automated decisions about individuals on our own behalf.",
          `Customers are responsible for determining whether their use of ${PLATFORM} involves profiling, automated decision-making, high-risk AI, regulated AI, targeted advertising, direct marketing, employment decisions, credit decisions, housing decisions, education decisions, insurance decisions or other regulated use.`,
          `Customers must not use ${PLATFORM} for legally significant automated decision-making or high-risk AI use unless expressly authorized in writing and lawful for the intended use.`,
        ],
      },
      {
        heading: "14. Customer Outreach and Marketing Compliance",
        paragraphs: [
          `${PLATFORM} may help customers prepare outreach assets, templates, follow-up drafts, pipeline notes and activation workflows.`,
          "Customers are responsible for all outreach decisions, recipient selection, sender identity, lawful basis, consent, opt-outs, unsubscribe handling, suppression lists, message content and compliance with marketing, privacy, consumer protection, platform and sector-specific laws.",
          `${PLATFORM} does not validate consent, certify recipient lists, guarantee compliance, act as sender of record, guarantee deliverability or assume responsibility for customer outreach.`,
        ],
      },
      {
        heading: "15. How We Share Personal Information",
        paragraphs: ["We may share personal information with:"],
        bullets: [
          "hosting providers;",
          "cloud infrastructure providers;",
          "authentication providers;",
          "payment processors;",
          "analytics providers;",
          "monitoring providers;",
          "logging providers;",
          "security providers;",
          "communications tools;",
          "support tools;",
          "AI providers;",
          "translation providers;",
          "email connectivity providers;",
          "CRM and integration providers;",
          "professional advisers;",
          "auditors;",
          "tax advisers;",
          "legal advisers;",
          "insurers;",
          "regulators;",
          "courts;",
          "law enforcement;",
          "payment networks;",
          "banks;",
          "acquirers, investors, successors or transaction parties in connection with corporate transactions;",
          "affiliates, contractors, service providers and subprocessors where necessary and lawful.",
          "We require service providers that process personal information on our behalf to process it for authorized purposes and to use appropriate confidentiality and data protection measures.",
        ],
      },
      {
        heading: "16. Customer-Uploaded Contact Data",
        paragraphs: [
          "We do not sell customer-uploaded contact data.",
          "We do not use customer-uploaded contact lists to market to those contacts for our own purposes while acting as processor or service provider for the customer.",
          `We process customer-uploaded contact data to provide, secure, support, operate and improve ${PLATFORM} in accordance with the Data Processing Agreement, applicable platform terms and customer instructions.`,
          "Customers remain responsible for the legality, source, accuracy, quality, permissions, notices, consents, suppression status, outreach use, retention and export of customer-uploaded contact data.",
        ],
      },
      {
        heading: "17. Sale, Sharing and Targeted Advertising",
        paragraphs: [
          "We do not sell Customer Personal Data processed on behalf of customers.",
          "We do not share Customer Personal Data processed on behalf of customers for cross-context behavioral advertising.",
          "We may use cookies, analytics or advertising tools on our website where lawful and subject to applicable consent or opt-out requirements.",
          "If our practices change in a way that requires additional notice, opt-out controls or consent, we will update this Privacy Policy and provide required choices.",
        ],
      },
      {
        heading: "18. Cookies and Similar Technologies",
        paragraphs: ["We use cookies and similar technologies for:"],
        bullets: [
          "essential site functions;",
          "login sessions;",
          "authentication;",
          "security;",
          "fraud prevention;",
          "preferences;",
          "performance;",
          "analytics;",
          "product improvement;",
          "service diagnostics;",
          "lawful marketing where applicable.",
          "The Cookie Policy explains cookie categories and how users can manage preferences.",
          "Depending on your location, non-essential cookies may require consent or may be subject to opt-out rights.",
          "You can also manage some cookies through browser settings. Blocking cookies may affect website or platform functionality.",
        ],
      },
      {
        heading: "19. Global Privacy Control and Browser Signals",
        paragraphs: [
          "Where required by applicable law and technically feasible, we may recognize legally required opt-out preference signals, such as Global Privacy Control, for website-level selling, sharing or targeted advertising choices.",
          `Because ${PLATFORM} is primarily a business software platform and customer-uploaded data is controlled by customers, some requests or signals relating to customer workspace data may need to be directed to the relevant customer.`,
          "We do not guarantee that all browser-based signals will apply to authenticated workspace activity, customer-controlled processing or third-party services.",
        ],
      },
      {
        heading: "20. Marketing Communications from Us",
        paragraphs: [
          "We may send administrative, transactional, billing, support, security and service messages that are necessary for operating the platform.",
          `Where lawful, we may also send business marketing communications about ${PLATFORM}. You may opt out of marketing emails by using the unsubscribe link or contacting us through the relevant route.`,
          "Opting out of marketing communications does not stop service, billing, legal, security or transactional messages.",
        ],
      },
      {
        heading: "21. International Transfers",
        paragraphs: [
          "Global Solutions Management LLC is incorporated in Delaware, United States.",
          "Personal information may be processed in the United States, United Kingdom, European Economic Area and other jurisdictions where we or our service providers operate.",
          "These jurisdictions may have data protection laws that differ from those in your location.",
          "Where required by applicable law, we use recognized transfer mechanisms such as adequacy decisions, standard contractual clauses, UK transfer mechanisms, Swiss transfer mechanisms, data privacy frameworks or other lawful safeguards.",
          `Customers remain responsible for determining whether their own use of ${PLATFORM}, including customer-uploaded data, outreach activity, AI-assisted features and integrations, is lawful in each relevant jurisdiction.`,
        ],
      },
      {
        heading: "22. Data Privacy Framework",
        paragraphs: [
          "Where we or a relevant service provider participates in an applicable data privacy framework, including the EU-U.S. Data Privacy Framework, UK Extension or Swiss-U.S. Data Privacy Framework, transfers may rely on that framework where legally available.",
          "If a data privacy framework is invalidated, suspended, unavailable or no longer applicable, transfers may rely on standard contractual clauses, UK transfer mechanisms, Swiss transfer mechanisms or another lawful transfer mechanism.",
          "Nothing in this Privacy Policy requires Global Solutions Management LLC to maintain certification under any data privacy framework unless expressly stated by us.",
        ],
      },
      {
        heading: "23. EU, UK and International Representative Information",
        paragraphs: [
          "Where required by applicable law, we may appoint a representative, data protection contact or other local contact point and publish the relevant details.",
          "If no separate representative details are published, privacy inquiries should be submitted through the Contact page using the route that matches the request.",
          "Nothing in this section creates a representative appointment, data protection officer appointment or local establishment unless expressly stated by Global Solutions Management LLC.",
        ],
      },
      {
        heading: "24. Retention",
        paragraphs: ["We retain personal information for as long as reasonably necessary for the purposes described in this Privacy Policy, including to:"],
        bullets: [
          "provide the platform;",
          "manage accounts;",
          "process billing;",
          "maintain tax and accounting records;",
          "provide support;",
          "maintain security;",
          "prevent fraud and abuse;",
          "resolve disputes;",
          "enforce agreements;",
          "comply with legal obligations;",
          "maintain audit records;",
          "support business operations;",
          "preserve evidence where necessary;",
          "comply with sanctions, regulatory, legal or law enforcement obligations.",
          "Customer workspace data may be retained until deleted by the customer, the account is terminated, deletion is requested and verified, or retention is no longer required, subject to backups, logs, legal retention, security retention, billing retention, fraud prevention, dispute handling and compliance needs.",
          "Backups, logs and archival copies may persist for a limited period before ordinary deletion cycles complete.",
          "We may retain aggregated, anonymised or de-identified data where it no longer identifies an individual.",
        ],
      },
      {
        heading: "25. Security",
        paragraphs: [
          "We use reasonable technical and organizational measures designed to protect personal information.",
          "Measures may include encrypted transmission, hosted access controls, role-based permissions, authentication controls, monitoring, logging, backups, vulnerability management, provider controls, confidentiality obligations and operational procedures.",
          "No internet service, SaaS platform, AI service, email integration, hosting provider or cloud environment is completely secure, error-free or immune from attack.",
          "Customers and users are responsible for:",
        ],
        bullets: [
          "using strong credentials;",
          "protecting accounts;",
          "managing workspace permissions;",
          "restricting user access;",
          "protecting connected email accounts;",
          "securing devices;",
          "securing exports;",
          "training users;",
          "reporting suspected compromise promptly.",
          "We are not responsible for security issues caused by customer-side systems, weak passwords, shared credentials, compromised devices, unsafe exports, unauthorized internal users, customer integrations, customer misconfiguration or third-party account compromise.",
        ],
      },
      {
        heading: "26. Children's Data",
        paragraphs: [
          `${PLATFORM} is not directed to children and is not intended for use by children.`,
          `Customers and users must not upload, import, process, generate, activate, export or store children's data through ${PLATFORM} unless expressly authorized in a separate written agreement and lawful for the intended use.`,
          "If we become aware that children's data has been uploaded without authorization, we may delete, restrict, quarantine or suspend processing.",
        ],
      },
      {
        heading: "27. Sensitive and Restricted Data",
        paragraphs: [
          `Customers must not upload, import, process, generate, activate, export or store sensitive or restricted data through ${PLATFORM} unless expressly authorized in writing and lawful for the intended use.`,
          "Restricted data includes:",
        ],
        bullets: [
          "health data;",
          "biometric data;",
          "genetic data;",
          "criminal offense data;",
          "financial account data;",
          "payment card data;",
          "government identifiers;",
          "precise geolocation data;",
          "special category data under GDPR or UK GDPR;",
          "sensitive personal information under CCPA or similar laws;",
          "children's data;",
          "data subject to professional secrecy or privilege;",
          "data subject to sector-specific restrictions;",
          "data that cannot lawfully be processed in a SaaS platform.",
          "We may delete, restrict, quarantine, block, refuse or report processing of restricted data where we reasonably believe it creates legal, regulatory, security, operational or reputational risk.",
        ],
      },
      {
        heading: "28. Your Privacy Rights",
        paragraphs: ["Depending on your location and relationship with us, you may have rights to:"],
        bullets: [
          "access personal information;",
          "correct personal information;",
          "delete personal information;",
          "restrict processing;",
          "object to processing;",
          "request portability;",
          "withdraw consent;",
          "opt out of certain marketing;",
          "opt out of sale or sharing where applicable;",
          "limit use of sensitive personal information where applicable;",
          "appeal certain rights decisions where applicable;",
          "lodge a complaint with a regulator.",
          "These rights may be limited by law, identity verification, security, legal privilege, customer-controller instructions, contractual obligations, fraud prevention, tax obligations, dispute handling, technical limitations or our legal obligations.",
          "Requests about customer-uploaded workspace data should normally be directed to the customer that controls the workspace.",
          "Requests about your account, website, billing, support or direct relationship with Global Solutions Management LLC may be sent to us through the Contact page.",
        ],
      },
      {
        heading: "29. California and US State Privacy Rights",
        paragraphs: [
          "If California or other US state privacy laws apply, you may have additional rights, including rights to know, access, correct, delete, opt out of sale or sharing, opt out of targeted advertising, limit certain uses of sensitive personal information, portability, non-discrimination and appeal.",
          "We do not sell Customer Personal Data processed on behalf of customers.",
          "We do not share Customer Personal Data processed on behalf of customers for cross-context behavioral advertising.",
          "If we use website analytics or advertising tools in a way that is considered selling, sharing or targeted advertising under applicable law, we will provide required notices and choices.",
          "We will not discriminate against you for exercising privacy rights, but some features may be unavailable where information is necessary to provide the platform.",
        ],
      },
      {
        heading: "30. Identity Verification and Authorized Agents",
        paragraphs: [
          "We may need to verify your identity before responding to a privacy request.",
          "Verification may require information that reasonably matches our records, account access, email confirmation, billing confirmation or other reasonable steps.",
          "If you use an authorized agent, we may require proof of authority and may also require you to verify your identity directly unless prohibited by law.",
          "We may refuse requests that are fraudulent, unverifiable, excessive, repetitive, abusive, technically infeasible, legally restricted or outside the scope of applicable rights.",
        ],
      },
      {
        heading: "31. Agency Customers and End Clients",
        paragraphs: [
          `If an agency customer uses ${PLATFORM} for its own clients, the agency remains responsible for its client data, client privacy notices, client lawful basis, client consents, client suppression lists, client opt-outs, client rights requests, client outreach compliance and client AI use.`,
          "Global Solutions Management LLC does not contract directly with agency end clients unless expressly agreed in writing.",
          "End clients, recipients and contacts should normally direct privacy requests to the agency or customer controlling the relevant workspace.",
        ],
      },
      {
        heading: "32. Legal, Regulatory and Third-Party Requests",
        paragraphs: ["We may disclose or preserve personal information where we believe it is necessary or appropriate to:"],
        bullets: [
          "comply with law;",
          "respond to court orders;",
          "respond to subpoenas;",
          "respond to regulator requests;",
          "respond to law enforcement;",
          "respond to payment-provider requests;",
          "respond to email-provider requests;",
          "respond to AI-provider or infrastructure-provider requests;",
          "investigate fraud;",
          "investigate abuse;",
          "protect rights;",
          "protect safety;",
          "protect the platform;",
          "enforce agreements;",
          "defend legal claims;",
          "prevent harm.",
          "Where legally permitted and commercially reasonable, we may notify affected customers or users of requests relating to them.",
          "We are not required to challenge legal, regulatory, payment-provider, email-provider, AI-provider or third-party requests unless required by law or expressly agreed in writing.",
        ],
      },
      {
        heading: "33. Monitoring, Abuse Prevention and Platform Protection",
        paragraphs: [
          "We may monitor, scan, log, review, analyse, investigate, preserve or restrict accounts, workspaces, Customer Personal Data, generated outputs, usage patterns, sender connections, integrations, activation activity, exports, billing activity and platform signals to operate the platform, improve safety, prevent abuse, enforce platform terms, comply with law, respond to complaints, protect infrastructure, protect recipients, protect third-party providers and protect our business.",
          "No monitoring, review, automated check, manual check, warning, flag, hold, approval flow or failure to act makes us responsible for customer data, customer outputs, customer campaigns, customer compliance, customer outreach or customer legal obligations.",
        ],
      },
      {
        heading: "34. Corporate Transactions",
        paragraphs: [
          "We may disclose or transfer personal information in connection with a merger, acquisition, financing, restructuring, sale of assets, transfer of business, insolvency, due diligence process, corporate reorganization or change of control.",
          "Any successor or acquiring entity may continue to process personal information in accordance with this Privacy Policy or provide a replacement policy where required.",
        ],
      },
      {
        heading: "35. De-Identified and Aggregated Data",
        paragraphs: [
          `We may use aggregated, anonymised or de-identified data to maintain, secure, analyse, improve and develop ${PLATFORM}.`,
          "Where required by applicable law, we will take reasonable measures to ensure de-identified data cannot reasonably be used to infer information about, or otherwise be linked to, a particular individual, and we will not attempt to re-identify it except as permitted by law.",
          "Aggregated, anonymised and de-identified data is not personal information where it no longer identifies or reasonably relates to an identifiable individual.",
        ],
      },
      {
        heading: "36. Third-Party Websites and Services",
        paragraphs: [
          `${PLATFORM} may link to or integrate with third-party websites, tools, services, platforms or providers.`,
          "Third-party services are not controlled by Global Solutions Management LLC and may have their own privacy policies, terms, cookies, security practices and data handling rules.",
          "We are not responsible for third-party privacy practices, security practices, terms, policies, outages, errors, account restrictions or data handling outside our reasonable control.",
          "Customers and users are responsible for reviewing third-party terms and privacy policies before connecting or using third-party services.",
        ],
      },
      {
        heading: "37. Limitation of This Privacy Policy",
        paragraphs: [
          "This Privacy Policy is intended to explain our privacy practices. It does not create contractual rights beyond those required by applicable law.",
          "The Platform Terms of Service, Customer Agreement and Data Processing Agreement contain important terms on customer responsibility, disclaimers, liability limits, indemnities, no personal liability, director and staff protection, dispute resolution, governing law and customer-controlled processing.",
          `To the maximum extent permitted by law, claims relating to ${PLATFORM}, customer workspace data, customer instructions, customer outreach, customer AI use, customer exports, customer integrations or customer-controlled processing are governed by the Platform Terms of Service, Customer Agreement and Data Processing Agreement.`,
          "Nothing in this Privacy Policy excludes rights or obligations that cannot legally be excluded.",
        ],
      },
      {
        heading: "38. Changes to this Privacy Policy",
        paragraphs: [
          "We may update this Privacy Policy from time to time to reflect product changes, legal developments, security requirements, operational needs, service provider changes, AI provider changes, infrastructure changes, transfer mechanism changes or changes to our business.",
          "Material updates may be notified by posting, email, platform notice or other reasonable method.",
          `The version posted on the ${PLATFORM} website or platform applies from its effective date unless otherwise stated.`,
        ],
      },
      {
        heading: "39. Contact",
        paragraphs: [
          "For privacy rights, data inquiries, security concerns or questions about this Privacy Policy, use the Contact page and select the route that matches your request.",
          `Global Solutions Management LLC — Delaware, United States — Operator of ${PLATFORM}.`,
        ],
      },
    ],
  },
  "acceptable-use-policy": {
    title: "Acceptable Use Policy",
    effectiveDate: "30 June 2026",
    lastUpdated: "1 July 2026",
    version: "8.0",
    sections: [
      {
        heading: "Overview",
        paragraphs: [
          `This Acceptable Use Policy governs permitted and prohibited use of ${PLATFORM}.`,
          "It applies to all accounts, workspaces, users, customers, agencies, client workspaces, uploads, imports, prompts, generated outputs, AI-assisted features, activations, exports, sender connections, integrations, support interactions, billing activity and platform use.",
          "This Acceptable Use Policy forms part of the Platform Terms of Service, Customer Agreement, Data Processing Agreement, Privacy Policy, Marketing Compliance Policy, Platform Security Policy, Service Level Agreement and all other incorporated platform terms.",
          "This document may be displayed in your browser's preferred language using an automated machine-translation layer, including GTranslate, Google Translate or similar translation technology. Translations are provided for convenience only. The English version controls if there is any conflict, ambiguity, inconsistency, error or difference between versions.",
        ],
      },
      {
        heading: "1. Purpose",
        paragraphs: [
          `${PLATFORM} is a self-serve, AI-powered commercial workspace for organizing contact data, preparing AI-assisted outreach assets, supporting governed activation, managing follow-up and moving warm contacts into pipeline.`,
          `This Acceptable Use Policy protects ${PLATFORM}, customers, users, recipients, senders, data subjects, rights holders, regulators, infrastructure, payment systems, AI systems, third-party providers, the business of Global Solutions Management LLC and all Protected Parties.`,
          `Customers and users must use ${PLATFORM} lawfully, safely, honestly and responsibly.`,
          `${PLATFORM} may refuse, restrict or terminate any use that creates legal, regulatory, security, payment, sanctions, AI, privacy, marketing, deliverability, reputational, operational or third-party provider risk.`,
        ],
      },
      {
        heading: "2. Delaware Operator and Protected Parties",
        paragraphs: [
          `${PLATFORM} is operated by Global Solutions Management LLC, a Delaware limited liability company.`,
          "For purposes of this Acceptable Use Policy, Protected Parties means Global Solutions Management LLC and its present and former owners, members, managers, directors, officers, employees, staff, contractors, consultants, agents, representatives, affiliates, licensors, suppliers, service providers, subprocessors, payment providers, infrastructure providers, AI providers, hosting providers, professional advisers, successors and assigns.",
          "To the maximum extent permitted by law, users and customers must not attempt to hold any Protected Party personally responsible for customer data, customer outreach, customer instructions, generated outputs, activation decisions, sender activity, agency client activity, prohibited use, platform misuse, customer-controlled processing, third-party provider actions or enforcement decisions.",
          "The Protected Parties are intended beneficiaries of the protections, restrictions, enforcement rights, indemnities, limitations, disclaimers and no-personal-liability provisions in this Acceptable Use Policy, the Platform Terms of Service, Customer Agreement and Data Processing Agreement.",
          "Nothing in this Acceptable Use Policy excludes liability where liability cannot lawfully be excluded.",
        ],
      },
      {
        heading: "3. Business Use Only",
        paragraphs: [
          `${PLATFORM} may be used only for lawful business, professional, commercial, agency, organizational and entrepreneurial purposes.`,
          `${PLATFORM} must not be used for personal, household, consumer, unlawful, abusive, deceptive, exploitative, high-risk or unsafe purposes.`,
          `You must not use ${PLATFORM} if you are not authorized to accept and comply with this Acceptable Use Policy.`,
        ],
      },
      {
        heading: "4. Customer Responsibility",
        paragraphs: [
          "Customers are responsible for all activity under their accounts, workspaces, users, client workspaces, sender connections, integrations, payment methods, uploads, generated outputs, exports and activation decisions.",
          "This includes activity by employees, contractors, agencies, clients, administrators, invited users, outsourced providers, consultants and anyone using customer credentials, workspace access, integration access, sender connections or payment methods.",
          "Customers must maintain appropriate internal approvals, user permissions, compliance records, suppression records, opt-out records, lawful-basis records, sender records, client authority and evidence of permission where required.",
          `No platform prompt, quality flag, compliance prompt, activation gate, AI output, support response, review step, warning or failure to warn transfers responsibility from Customer to ${PLATFORM}.`,
        ],
      },
      {
        heading: "5. Lawful Use Only",
        paragraphs: [
          `You may use ${PLATFORM} only in compliance with all applicable laws, regulations, sanctions, export controls, platform policies, provider rules, contracts, industry standards and third-party rights.`,
          `You must not use ${PLATFORM} for:`,
        ],
        bullets: [
          "unlawful data collection;",
          "unlawful data processing;",
          "unlawful data enrichment;",
          "unlawful data sale;",
          "unlawful outreach;",
          "unlawful exports;",
          "unlawful profiling;",
          "unlawful automated decision-making;",
          "unlawful AI deployment;",
          "unlawful marketing;",
          "fraud;",
          "scams;",
          "deception;",
          "infringement;",
          "harassment;",
          "discrimination;",
          "abuse;",
          "exploitation;",
          "activity that would cause Global Solutions Management LLC, any Protected Party, provider, customer, recipient or third party to breach law, contract, regulation, platform rule or policy.",
          `If your intended use is restricted, regulated, unclear or high-risk, you are responsible for obtaining legal advice before using ${PLATFORM}.`,
        ],
      },
      {
        heading: "6. Know Your Use and Verification Rights",
        paragraphs: [
          "We may require identity, business, payment, sender, domain, data-source, lawful-basis, consent, client-authority, agency-authority or regulated-sector verification at any time.",
          "We may require evidence of:",
        ],
        bullets: [
          "business identity;",
          "user authority;",
          "payment authority;",
          "domain ownership;",
          "sender authority;",
          "email account authority;",
          "client authority;",
          "data source;",
          "lawful basis;",
          "consent;",
          "suppression handling;",
          "opt-out handling;",
          "recipient permission;",
          "campaign approval;",
          "regulated-sector permission;",
          "sanctions compliance;",
          "export control compliance;",
          "AI deployment compliance.",
          "Failure to provide satisfactory evidence may result in upload blocks, activation holds, export restrictions, sender holds, workspace restrictions, account suspension, termination, deletion, quarantine or refusal of future service.",
          `A request for evidence does not mean that ${PLATFORM} approves, validates, certifies or assumes responsibility for your data, outreach, lawful basis, sender setup, campaign, AI use or generated outputs.`,
        ],
      },
      {
        heading: "7. Data and List Restrictions",
        paragraphs: [
          "You must not upload, import, paste, generate, process, activate, export, sell, share, enrich or use data that you cannot lawfully process for the intended purpose.",
          `You must not use ${PLATFORM} with:`,
        ],
        bullets: [
          "illegally obtained data;",
          "stolen data;",
          "leaked data;",
          "breached data;",
          "unlawfully scraped data;",
          "hacked data;",
          "purchased lists without lawful basis;",
          "rented lists without lawful basis;",
          "brokered lists without lawful basis;",
          "harvested lists without lawful basis;",
          "enriched data without lawful basis;",
          "suppressed records;",
          "do-not-contact records;",
          "opted-out contacts;",
          "unsubscribed contacts;",
          "data subject to contractual restrictions;",
          "data subject to platform restrictions;",
          "data subject to professional secrecy restrictions;",
          "data subject to sector-specific restrictions;",
          "data that cannot lawfully be transferred to, stored in or processed through a SaaS platform;",
          "data that cannot lawfully be used for outreach, activation, export, analysis, AI-assisted generation or follow-up.",
          `You are responsible for knowing the source, lawful basis, permission status, consent status, suppression status and usage restrictions for all data you place into ${PLATFORM}.`,
        ],
      },
      {
        heading: "8. No List Washing or Suppression Abuse",
        paragraphs: [
          `You must not use ${PLATFORM} to clean, validate, enrich, reactivate, recycle, test, suppress, evade, match, compare or "wash" contact lists in order to contact people who have opted out, unsubscribed, objected, complained, suppressed their details or otherwise restricted contact.`,
          "You must not upload suppression lists, unsubscribe lists, objection lists or do-not-contact records except where you are authorized to process them and only for lawful suppression and compliance purposes.",
          "You must not use suppression data to target, profile, reverse engineer, enrich, resell, share or contact individuals.",
        ],
      },
      {
        heading: "9. Sensitive, Restricted and Prohibited Data",
        paragraphs: [`You must not upload, import, process, generate, activate, export or store through ${PLATFORM}:`],
        bullets: [
          "children's data;",
          "health data;",
          "biometric data;",
          "genetic data;",
          "criminal offense data;",
          "payment card data;",
          "financial account data;",
          "government identifier data;",
          "precise geolocation data;",
          "special category data under GDPR or UK GDPR;",
          "sensitive personal information under CCPA or similar laws;",
          "data subject to professional secrecy or privilege unless authorized in writing;",
          "data subject to confidentiality obligations that prohibit SaaS processing;",
          "data subject to export, sanctions, data localization or secrecy restrictions;",
          "data that creates legal, regulatory, operational, reputational or security risk.",
          "Restricted data may only be processed if expressly authorized in a separate signed written agreement accepted by Global Solutions Management LLC and lawful for the intended use.",
          "We may delete, restrict, quarantine, block, refuse or report processing of restricted data where we reasonably believe it creates risk.",
        ],
      },
      {
        heading: "10. Outreach and Sender Restrictions",
        paragraphs: [
          `You must not use ${PLATFORM} for spam, unlawful unsolicited communications, unlawful cold outreach, unlawful SMS, unlawful electronic mail, unlawful messaging, unlawful social outreach, unlawful telephone outreach or any communication that violates recipient rights, platform rules or applicable law.`,
          "You must not:",
        ],
        bullets: [
          "use deceptive headers;",
          "use misleading subject lines;",
          "use false sender details;",
          "impersonate another person or organization;",
          "spoof domains;",
          "spoof email addresses;",
          "hide your identity;",
          "misrepresent sender authority;",
          "misrepresent affiliation;",
          "misrepresent purpose;",
          "evade opt-outs;",
          "evade unsubscribe requests;",
          "ignore suppression lists;",
          "contact do-not-contact records;",
          "send to recipients without required consent or lawful basis;",
          "use invalid physical address details where required;",
          "omit required unsubscribe mechanisms;",
          "omit required sender identification;",
          "continue contacting recipients who have opted out;",
          "generate or send messages that would mislead a recipient about who is contacting them or why.",
          `${PLATFORM} may provide governance controls, sender prompts, quality flags or activation controls, but these do not make your outreach lawful by themselves.`,
          `You remain responsible for all outreach activity, whether sent through ${PLATFORM}, exported from ${PLATFORM}, copied from ${PLATFORM}, adapted from ${PLATFORM} or prepared using generated outputs.`,
        ],
      },
      {
        heading: "11. Global Marketing Compliance",
        paragraphs: [
          "You are responsible for complying with all laws and rules that apply to outreach, prospecting, marketing, advertising, email, SMS, messaging, social outreach, telephone contact, retargeting, follow-up, lead generation and pipeline activity in every relevant jurisdiction.",
          "This includes rules relating to:",
        ],
        bullets: [
          "consent;",
          "soft opt-in;",
          "legitimate interest;",
          "unsubscribe links;",
          "opt-outs;",
          "suppression lists;",
          "sender identity;",
          "subject lines;",
          "header information;",
          "physical address disclosure;",
          "recipient location;",
          "commercial claims;",
          "advertising standards;",
          "platform terms;",
          "vulnerable recipients;",
          "regulated sectors;",
          "B2B marketing;",
          "B2C marketing;",
          "agency-managed campaigns;",
          "cross-border outreach.",
          `${PLATFORM} does not validate consent, approve lawful basis, certify recipient lists, guarantee compliance, act as sender of record, guarantee deliverability or assume responsibility for customer outreach.`,
        ],
      },
      {
        heading: "12. High-Volume, Automated and Bulk Activation",
        paragraphs: [
          `You must not use ${PLATFORM} for high-volume, automated, bulk, aggressive, deceptive, evasive or uncontrolled activation unless your plan expressly permits the relevant use and your activity complies with all applicable laws, provider rules and platform controls.`,
          "We may impose activation limits, sender limits, export limits, generation limits, rate limits, workspace limits, review gates, approval steps or manual checks at any time.",
          "You must not split campaigns, duplicate workspaces, rotate senders, create multiple accounts, manipulate domains, use multiple clients or otherwise structure usage to avoid platform limits, provider limits, lawful-basis requirements, unsubscribe obligations, suppression requirements or enforcement controls.",
        ],
      },
      {
        heading: "13. Deliverability, Domain and Sender Reputation Abuse",
        paragraphs: [
          `You must not use ${PLATFORM} to manipulate, deceive or abuse email providers, spam filters, inbox providers, recipient systems, sender reputation systems or domain reputation systems.`,
          `You must not use ${PLATFORM} for:`,
        ],
        bullets: [
          "domain spoofing;",
          "sender spoofing;",
          "deceptive domain rotation;",
          "deceptive inbox rotation;",
          "reputation laundering;",
          "unlawful domain warming;",
          "misleading sender identities;",
          "mailbox abuse;",
          "mass testing of emails against spam filters;",
          "evasion of provider restrictions;",
          "reactivation of blocked senders;",
          "use of compromised mailboxes;",
          "use of unauthorized mailboxes;",
          "use of mailboxes without sender authority.",
          "We may suspend, restrict or disconnect senders, domains, integrations or workspaces if we detect bounce risk, complaint risk, spam risk, blacklist risk, sender reputation risk, domain reputation risk, provider risk or unlawful outreach risk.",
        ],
      },
      {
        heading: "14. AI-Assisted Use Restrictions",
        paragraphs: [
          `${PLATFORM} may include AI-assisted tools for drafting, structuring, summarising, classifying, translating, segmenting, reviewing, suggesting or generating content and workflow actions.`,
          "Generated outputs are drafts only.",
          "You must review, edit, approve and verify generated outputs before using, sending, publishing, exporting, relying on or activating them.",
          "You must not use AI-assisted features to:",
        ],
        bullets: [
          "generate unlawful content;",
          "generate deceptive content;",
          "generate harmful content;",
          "generate discriminatory content;",
          "generate abusive content;",
          "generate infringing content;",
          "generate regulated advice;",
          "generate fraudulent claims;",
          "impersonate people;",
          "create deceptive synthetic identities;",
          "create deepfake-style deception;",
          "misrepresent authority;",
          "misrepresent qualifications;",
          "misrepresent endorsements;",
          "manipulate vulnerable people;",
          "evade legal obligations;",
          "make legally significant decisions about individuals;",
          "create high-risk AI use without written authorization;",
          "automate outreach in a way that removes required human review;",
          "bypass customer-controlled activation;",
          "create content that violates this Acceptable Use Policy.",
          `You are responsible for determining whether your use of ${PLATFORM} creates obligations under AI, privacy, employment, consumer protection, marketing, professional, sector-specific or automated-decision laws.`,
        ],
      },
      {
        heading: "15. High-Risk AI and Automated Decision Restrictions",
        paragraphs: [
          `You must not use ${PLATFORM} for high-risk AI, prohibited AI practices, automated decision-making, profiling, eligibility decisions, employment decisions, credit decisions, housing decisions, education access decisions, insurance decisions, healthcare decisions, criminal justice decisions, biometric identification, vulnerable-person targeting or legally significant decision-making unless expressly authorized in a separate signed written agreement accepted by Global Solutions Management LLC and lawful for the intended use.`,
          `You must not use ${PLATFORM} to create or operate any AI system that is prohibited, restricted or high-risk under applicable law unless expressly authorized in writing and fully compliant.`,
          "We may suspend, restrict, refuse, delete or terminate any AI-related use that we reasonably believe creates legal, regulatory, ethical, reputational, operational, security or data protection risk.",
        ],
      },
      {
        heading: "16. Content Restrictions",
        paragraphs: ["You must not create, upload, import, generate, send, publish, export, activate or promote content that is:"],
        bullets: [
          "illegal;",
          "fraudulent;",
          "deceptive;",
          "misleading;",
          "defamatory;",
          "harassing;",
          "threatening;",
          "abusive;",
          "exploitative;",
          "hateful;",
          "discriminatory;",
          "sexually exploitative;",
          "harmful to minors;",
          "violent;",
          "extremist;",
          "malicious;",
          "infringing;",
          "privacy-invasive;",
          "unsafe;",
          "designed to manipulate;",
          "designed to evade law;",
          "targeted at vulnerable people in an inappropriate or unlawful way.",
          "You must not use generated outputs to misrepresent:",
          "facts;",
          "qualifications;",
          "endorsements;",
          "pricing;",
          "scarcity;",
          "legal rights;",
          "regulatory status;",
          "product capabilities;",
          "customer results;",
          "case studies;",
          "testimonials;",
          "sender identity;",
          "affiliations;",
          "investment opportunity;",
          "health outcomes;",
          "financial outcomes;",
          "compliance status;",
          "AI capability.",
        ],
      },
      {
        heading: "17. Regulated and Restricted Sectors",
        paragraphs: [
          "Certain sectors may be restricted, blocked or subject to additional requirements.",
          `You must not use ${PLATFORM} for regulated, sensitive or high-risk sectors unless you have all required permissions, licenses, legal bases, approvals and compliance processes.`,
          "Restricted or high-risk sectors include:",
        ],
        bullets: [
          "health;",
          "medical services;",
          "pharmaceuticals;",
          "finance;",
          "investment;",
          "credit;",
          "lending;",
          "insurance;",
          "legal services;",
          "immigration;",
          "employment;",
          "recruitment;",
          "education access;",
          "housing;",
          "politics;",
          "political campaigning;",
          "public-sector influence;",
          "gambling;",
          "alcohol;",
          "tobacco;",
          "nicotine;",
          "controlled substances;",
          "weapons;",
          "adult content;",
          "dating;",
          "surveillance;",
          "cybersecurity tools;",
          "debt collection;",
          "charities and fundraising where regulated;",
          "vulnerable-person services;",
          "children's products or services;",
          "data brokerage;",
          "lead resale;",
          "telemarketing;",
          "call-centre operations;",
          "any other sector that creates legal, regulatory, reputational, safety or platform risk.",
          `We may refuse, restrict or suspend use in any sector we consider unsuitable for ${PLATFORM}.`,
        ],
      },
      {
        heading: "18. Political, Public-Issue and Sensitive Influence",
        paragraphs: [
          `You must not use ${PLATFORM} for political campaigning, voter targeting, public-issue persuasion, lobbying, government influence, public-sector influence, fundraising for political or sensitive causes, or targeting based on sensitive characteristics unless expressly authorized in writing and lawful in every relevant jurisdiction.`,
          `You must not use ${PLATFORM} to target individuals based on sensitive personal data, protected characteristics, vulnerability, financial distress, health status, political views, religious beliefs, trade union status, sexuality, ethnicity or similar sensitive traits.`,
          "We may refuse or restrict any use involving political, public-issue, sensitive influence or vulnerable-person targeting.",
        ],
      },
      {
        heading: "19. Dangerous, Illegal and Harmful Activity",
        paragraphs: [`You must not use ${PLATFORM} to promote, facilitate, organize, support or conceal:`],
        bullets: [
          "illegal goods or services;",
          "terrorism;",
          "violent extremism;",
          "organized crime;",
          "human trafficking;",
          "sexual exploitation;",
          "child exploitation;",
          "self-harm;",
          "fraud;",
          "scams;",
          "phishing;",
          "malware;",
          "credential theft;",
          "unauthorized surveillance;",
          "doxxing;",
          "harassment campaigns;",
          "evasion of law enforcement;",
          "evasion of sanctions;",
          "cyber abuse;",
          "unauthorized access;",
          "blackmail;",
          "extortion;",
          "money laundering;",
          "bribery;",
          "corruption;",
          "tax evasion;",
          "export control evasion.",
          "We may report suspected unlawful activity to relevant providers, platforms, regulators, law enforcement or other appropriate parties where we consider it necessary or appropriate.",
        ],
      },
      {
        heading: "20. Platform Abuse and Security",
        paragraphs: [
          `You must not interfere with, probe, scan, disrupt, overload, damage, reverse engineer, scrape, copy, frame, mirror, decompile, disassemble, extract, bypass, resell, sublicense or misuse ${PLATFORM}, its infrastructure, source code, APIs, billing systems, safety checks, sender verification, quality holds, plan limits, credit logic, security controls, prompts, workflows, templates, outputs or product logic.`,
          "You must not:",
        ],
        bullets: [
          "attempt unauthorized access;",
          "test vulnerabilities without permission;",
          "bypass authentication;",
          "share credentials;",
          "abuse invitations;",
          "abuse workspaces;",
          "overload systems;",
          "perform denial-of-service attacks;",
          "introduce malware;",
          "upload malicious files;",
          "bypass rate limits;",
          "bypass usage limits;",
          "bypass billing;",
          "create multiple accounts to evade restrictions;",
          "exploit bugs;",
          "conceal abuse;",
          "interfere with logs;",
          "interfere with monitoring;",
          "compromise other users or customers.",
          "Security testing is allowed only with prior written permission from Global Solutions Management LLC.",
        ],
      },
      {
        heading: "21. Competitor, Benchmarking and Model Training Restrictions",
        paragraphs: [
          `You must not use ${PLATFORM} to build, train, fine-tune, benchmark, evaluate, test or improve any competing product, AI model, dataset, platform, workflow, template library, automation system, commercial database or outreach tool without our prior written consent.`,
          "You must not scrape, copy, extract, monitor, harvest or use platform content, prompts, templates, workflows, outputs, interface patterns, scoring logic, safety logic, product logic or system behavior for competitive analysis, model training, dataset creation, product replication, reverse engineering or benchmarking.",
        ],
      },
      {
        heading: "22. Account, Billing and Credit Abuse",
        paragraphs: [
          "You must not abuse accounts, trials, promotions, founder offers, credits, referrals, billing systems, payment methods, refunds, chargebacks or plan limits.",
          "You must not:",
        ],
        bullets: [
          "create duplicate accounts to obtain extra credits;",
          "misuse promotional credits;",
          "sell or transfer credits;",
          "exploit pricing errors;",
          "abuse refund processes;",
          "initiate fraudulent chargebacks;",
          "use stolen payment methods;",
          "evade billing controls;",
          "misrepresent tax or billing information;",
          "hide payment risk;",
          "use the platform after failed payment without authorization.",
          "We may correct credit errors, reverse abusive credits, suspend accounts, restrict features, recover unpaid amounts and refuse future service.",
        ],
      },
      {
        heading: "23. Agency and Client Workspace Conduct",
        paragraphs: [
          "Agencies are responsible for all client workspaces, invited users, client data, sender setup, permissions, approvals, generated outputs, exports, activation decisions, compliance records and outreach activity.",
          "Agencies must not:",
        ],
        bullets: [
          "mix client data without lawful permission;",
          "use one client's data for another client's benefit without lawful permission;",
          "misrepresent client authority;",
          "upload client data without authorization;",
          "activate outreach without client approval where approval is required;",
          `conceal ${PLATFORM}'s role;`,
          `represent that ${PLATFORM} has approved or legally cleared client campaigns;`,
          "expose client data to unauthorized users;",
          "ignore client opt-outs or suppression records;",
          `shift client compliance responsibility to ${PLATFORM}.`,
          `${PLATFORM} does not contract directly with agency end clients unless expressly agreed in writing.`,
          "The agency account holder is responsible for all client workspace activity and must indemnify the Protected Parties for claims arising from client data, client outreach, client instructions, client users, client disputes or agency-managed use.",
        ],
      },
      {
        heading: "24. Client Flow-Down Obligations",
        paragraphs: [
          `If you use ${PLATFORM} for clients, customers, brands, subsidiaries, portfolio companies, franchisees, partners, campaigns or third parties, you must ensure that those parties comply with this Acceptable Use Policy, the Platform Terms of Service, Customer Agreement, Data Processing Agreement and all applicable laws.`,
          "You remain responsible even if the relevant client, customer, brand, subsidiary, portfolio company, franchisee, partner or third party gave the instruction that caused the violation.",
          "You must not rely on a client instruction as a defence to breaching this Acceptable Use Policy.",
        ],
      },
      {
        heading: "25. Sanctions, Export Controls and Restricted Jurisdictions",
        paragraphs: [
          "You must comply with all applicable sanctions, export control, trade control, anti-bribery, anti-corruption and anti-money laundering laws.",
          `You must not use ${PLATFORM} if you or any user is located in, organized under the laws of, ordinarily resident in, owned or controlled by, or acting on behalf of any person or entity subject to sanctions, embargoes or trade restrictions that would prohibit access to or use of ${PLATFORM}.`,
          `You must not use ${PLATFORM} to support prohibited transactions, restricted parties, sanctioned entities, embargoed territories, unlawful exports, unlawful re-exports, unlawful circumvention of trade controls, bribery, corruption, money laundering or sanctions evasion.`,
          "We may restrict, suspend, block or terminate access where sanctions, export control, trade compliance, payment or jurisdictional risk is identified or suspected.",
        ],
      },
      {
        heading: "26. Data Localization, Secrecy and Regulated Transfer Restrictions",
        paragraphs: [
          `You must not use ${PLATFORM} where applicable law, contract, professional duty, sector rule, data localization law, secrecy obligation, export control law or confidentiality restriction prohibits the transfer, storage, access, processing, AI processing, support access or use of data through the platform.`,
          `You are responsible for determining whether your data may lawfully be processed in the United States or any other jurisdiction where ${PLATFORM} or its providers operate.`,
          "We may refuse, restrict, suspend or terminate processing where we reasonably believe data localization, secrecy, transfer, confidentiality or jurisdictional restrictions create risk.",
        ],
      },
      {
        heading: "27. Third-Party Provider Rules",
        paragraphs: [
          `${PLATFORM} may rely on third-party providers for hosting, payments, AI processing, analytics, email connectivity, communications, authentication, logging, monitoring, translation, support and infrastructure.`,
          `You must not use ${PLATFORM} in a way that violates third-party provider terms, platform rules, acceptable use policies, AI provider policies, email provider policies, payment provider rules, hosting provider rules or integration provider rules.`,
          "We may restrict, suspend, remove or modify access where required by third-party provider rules or where continued use could create provider, infrastructure, payment, deliverability, legal or operational risk.",
        ],
      },
      {
        heading: "28. No Evasion of Controls",
        paragraphs: ["You must not bypass, disable, manipulate, avoid, conceal or interfere with:"],
        bullets: [
          "sender verification;",
          "data quality holds;",
          "suppression prompts;",
          "opt-out controls;",
          "compliance prompts;",
          "activation gates;",
          "plan limits;",
          "credit consumption;",
          "rate limits;",
          "risky-record flags;",
          "billing controls;",
          "security controls;",
          "account restrictions;",
          "monitoring;",
          "enforcement actions;",
          "suspension measures;",
          "identity checks;",
          "verification requests;",
          "provider restrictions;",
          "jurisdiction restrictions.",
          "Attempts to evade controls may result in immediate suspension or termination.",
        ],
      },
      {
        heading: "29. No Circumvention After Suspension",
        paragraphs: [
          "If we suspend, restrict, terminate, block or refuse service, you must not attempt to regain access by creating another account, using another entity, using another email address, using another domain, using a client account, using an agency account, using another payment method, masking identity or otherwise circumventing enforcement.",
          "We may treat circumvention as a material breach and may block related accounts, workspaces, users, domains, senders, payment methods, integrations and clients.",
        ],
      },
      {
        heading: "30. Evidence and Audit Logs",
        paragraphs: [
          "Platform records, usage logs, billing logs, support records, security logs, audit logs, activation logs, export logs, acceptance logs, sender logs, integration logs and enforcement records may be used as evidence of activity, acceptance, breach, risk, investigation, payment status, activation, export, compliance review or enforcement action.",
          "You agree that such records may be relied upon to the maximum extent permitted by law.",
        ],
      },
      {
        heading: "31. Monitoring and Investigation",
        paragraphs: [
          "We may, but are not required to, monitor, scan, log, review, analyse, investigate, preserve or restrict accounts, workspaces, data, generated outputs, usage patterns, sender connections, integrations, activation activity, exports, billing activity and platform signals.",
          "We may do this to:",
        ],
        bullets: [
          `operate ${PLATFORM};`,
          "improve safety;",
          "prevent abuse;",
          "enforce this Acceptable Use Policy;",
          "comply with law;",
          "respond to complaints;",
          "protect infrastructure;",
          "protect recipients;",
          "protect customers;",
          "protect third-party providers;",
          "protect payment systems;",
          "protect AI systems;",
          "protect the business;",
          "protect Protected Parties.",
          "No monitoring, review, automated check, manual check, warning, flag, hold, approval flow or failure to act makes us responsible for customer data, customer outputs, customer campaigns, customer compliance, customer outreach or customer legal obligations.",
        ],
      },
      {
        heading: "32. Enforcement Rights",
        paragraphs: [
          "If we believe this Acceptable Use Policy has been breached, or if we believe there is legal, regulatory, security, payment, provider, AI, deliverability, reputational, operational or platform risk, we may take any action we consider appropriate, including:",
        ],
        bullets: [
          "blocking uploads;",
          "holding records;",
          "quarantining data;",
          "deleting data;",
          "pausing activation;",
          "restricting exports;",
          "disabling generated outputs;",
          "removing content;",
          "throttling usage;",
          "requiring verification;",
          "requiring compliance evidence;",
          "suspending senders;",
          "disconnecting integrations;",
          "disabling workspaces;",
          "restricting credits;",
          "canceling credits;",
          "downgrading plans;",
          "suspending accounts;",
          "terminating accounts;",
          "refusing future service;",
          "notifying providers;",
          "notifying customers;",
          "notifying affected parties;",
          "preserving records;",
          "reporting unlawful activity where appropriate.",
          `We may act immediately and without prior notice where needed to protect ${PLATFORM}, customers, users, recipients, infrastructure, third-party providers, payment systems, AI systems, sender systems, legal compliance, platform integrity, Protected Parties or our business.`,
          "We are not liable for loss arising from enforcement action taken in good faith to protect the platform, customers, recipients, third-party providers, legal compliance or the business.",
        ],
      },
      {
        heading: "33. No Refund for Enforcement",
        paragraphs: [
          "Enforcement action under this Acceptable Use Policy does not create any right to a refund, credit, service credit, damages, compensation, chargeback, termination payment or extension unless expressly required by law.",
          "You remain responsible for fees, taxes, usage charges, credit usage, chargebacks, indemnity obligations, confidentiality obligations, data compliance obligations, acceptable use obligations and outreach compliance obligations already incurred.",
        ],
      },
      {
        heading: "34. Reporting and Cooperation",
        paragraphs: [
          "You must promptly report suspected misuse, security issues, unlawful activity, unauthorized access, compromised credentials, data misuse, sender abuse, client workspace misuse or policy violations.",
          "You must cooperate with reasonable investigations, verification requests, legal requests, security reviews, payment reviews, compliance reviews and provider inquiries.",
          "Failure to cooperate may result in restriction, suspension or termination.",
        ],
      },
      {
        heading: "35. Legal, Regulatory and Third-Party Requests",
        paragraphs: [
          "If we receive a subpoena, court order, regulator request, law enforcement request, data subject request, rights-holder complaint, payment-provider request, email-provider request, AI-provider request, third-party platform request, security complaint or legal notice relating to your account, workspace, users, data, outreach, outputs, clients or integrations, we may respond, disclose information, preserve records, restrict access, notify you or take other action as we consider legally required or reasonably appropriate.",
          "We are not required to challenge any legal, regulatory, payment-provider, email-provider, AI-provider or third-party request unless required by law or expressly agreed in writing.",
        ],
      },
      {
        heading: "36. Indemnity",
        paragraphs: [
          "You agree to defend, indemnify and hold harmless the Protected Parties from and against all claims, demands, investigations, complaints, proceedings, losses, liabilities, damages, penalties, fines, settlements, costs and expenses, including reasonable legal fees, arising from or related to:",
        ],
        bullets: [
          "your breach of this Acceptable Use Policy;",
          "your data;",
          "your contact lists;",
          "your prompts;",
          "your generated outputs;",
          "your outreach;",
          "your sender identity;",
          "your client activity;",
          "your agency workspaces;",
          "your unlawful marketing;",
          "your data protection violation;",
          "your privacy violation;",
          "your AI misuse;",
          "your regulated-sector activity;",
          "your security misuse;",
          "your payment abuse;",
          "your platform abuse;",
          "your third-party provider violation;",
          "your user activity;",
          "your customer-side systems;",
          "your breach of law;",
          "your breach of third-party rights;",
          "your evasion of platform controls;",
          "your sanctions or export control breach;",
          "your data localization or secrecy breach;",
          "your deliverability or sender reputation abuse;",
          "your list washing or suppression abuse.",
          "We may control the defence of any matter subject to indemnity. You must cooperate with us and must not settle any claim in a way that imposes obligations, admissions, restrictions or liability on any Protected Party without our prior written consent.",
          "This indemnity survives suspension, termination, cancellation, account closure and deletion of data.",
        ],
      },
      {
        heading: "37. Relationship with Other Terms",
        paragraphs: [
          "This Acceptable Use Policy supplements the Platform Terms of Service, Customer Agreement, Data Processing Agreement, Marketing Compliance Policy, Privacy Policy, Platform Security Policy and Service Level Agreement.",
          "If there is any conflict, the stricter or more protective provision applies to the maximum extent permitted by law.",
          "Nothing in this Acceptable Use Policy limits our rights under any other agreement, policy, law or equitable remedy.",
        ],
      },
      {
        heading: "38. Local Law and Mandatory Rights",
        paragraphs: [
          "This Acceptable Use Policy applies to the maximum extent permitted by applicable law.",
          "If mandatory law in a jurisdiction gives rights that cannot be excluded, restricted or modified, this Acceptable Use Policy does not exclude those rights, but all exclusions, limitations, restrictions, indemnities and protections continue to apply to the maximum extent permitted by that law.",
          "Where a provision cannot be enforced as written in a particular jurisdiction, it will be interpreted, narrowed or reformed only to the minimum extent necessary to make it enforceable, and the rest of this Acceptable Use Policy remains effective.",
        ],
      },
      {
        heading: "39. Changes to this Acceptable Use Policy",
        paragraphs: [
          "We may update this Acceptable Use Policy from time to time to reflect product changes, legal developments, security requirements, operational needs, provider requirements, AI provider requirements, payment provider requirements, infrastructure changes, abuse patterns or changes to our business.",
          `Continued use of ${PLATFORM} after the effective date of an updated Acceptable Use Policy means you accept the updated policy.`,
          `If you do not agree to an updated Acceptable Use Policy, you must stop using ${PLATFORM}.`,
        ],
      },
      {
        heading: "40. Survival",
        paragraphs: [
          "Any provision that by its nature should survive suspension, termination, cancellation, account closure or deletion of data will survive, including provisions relating to customer responsibility, prohibited use, evidence, monitoring, enforcement, no refund, indemnity, Protected Parties, local law, relationship with other terms and legal requests.",
        ],
      },
      {
        heading: "41. Contact",
        paragraphs: [
          "For acceptable use, abuse reports, policy concerns, compliance concerns or security concerns, use the Contact page and select the route that matches your request.",
          `Global Solutions Management LLC — Delaware, United States — Operator of ${PLATFORM}.`,
        ],
      },
    ],
  },
  "marketing-compliance-policy": {
    title: "Marketing Compliance Policy",
    effectiveDate: "30 June 2026",
    lastUpdated: "1 July 2026",
    version: "10.0",
    sections: [
      {
        heading: `1. Purpose`,
        paragraphs: [
          `Velocity Vision provides self-serve software tools that may help customers organize contact data, prepare AI-assisted outreach assets, review data readiness, manage templates, support governed activation, manage follow-up and move warm contacts into pipeline.`,
          `This Marketing Compliance Policy protects Velocity Vision, customers, users, recipients, senders, data subjects, rights holders, regulators, infrastructure, payment systems, AI systems, email providers, hosting providers, integration providers, third-party platforms, the business of Global Solutions Management LLC and all Protected Parties.`,
          `Velocity Vision does not provide legal advice, compliance sign-off, deliverability consulting, sender-of-record services, advertiser-of-record services, managed campaign services, marketing agency services, professional review, regulatory clearance, list-brokerage services or guaranteed lead-generation services.`,
          `Customers are responsible for ensuring that every outreach activity, contact source, message, sender identity, advertiser identity, opt-out process, suppression process, follow-up action, generated output, export, activation and campaign complies with applicable law.`,
        ],
      },
      {
        heading: `2. Delaware Operator and Protected Parties`,
        paragraphs: [
          `Velocity Vision is operated by Global Solutions Management LLC, a Delaware limited liability company.`,
          `For purposes of this Marketing Compliance Policy, **Protected Parties** means Global Solutions Management LLC and its present and former owners, members, managers, directors, officers, employees, staff, contractors, consultants, agents, representatives, affiliates, licensors, suppliers, service providers, subprocessors, payment providers, infrastructure providers, AI providers, hosting providers, email providers, integration providers, professional advisers, successors and assigns.`,
          `To the maximum extent permitted by law, customers and users must not attempt to hold any Protected Party personally responsible for customer data, customer outreach, customer instructions, generated outputs, activation decisions, sender activity, advertiser activity, agency client activity, prohibited marketing, platform misuse, customer-controlled processing, third-party provider actions or enforcement decisions.`,
          `The Protected Parties are intended beneficiaries of the protections, restrictions, enforcement rights, indemnities, limitations, disclaimers and no-personal-liability provisions in this Marketing Compliance Policy, the Platform Terms of Service, Customer Agreement, Data Processing Agreement and Acceptable Use Policy.`,
          `Nothing in this Marketing Compliance Policy excludes liability where liability cannot lawfully be excluded.`,
        ],
      },
      {
        heading: `3. No Personal Claims Against Directors, Managers, Staff or Providers`,
        paragraphs: [
          `To the maximum extent permitted by law, Customer agrees that any claim, complaint, demand, dispute, proceeding or allegation relating to Customer’s marketing, outreach, data, sender identity, generated outputs, activation, exports, agency client activity, provider enforcement, recipient complaints or compliance failure must be brought only against Global Solutions Management LLC where a claim against Velocity Vision is legally permitted, and not personally against any owner, member, manager, director, officer, employee, staff member, contractor, consultant, adviser, supplier, service provider, subprocessor, AI provider, hosting provider, payment provider, email provider, integration provider or infrastructure provider.`,
          `Customer must not encourage, assist, fund, support, threaten or pursue personal claims against Protected Parties arising from Customer’s own data, outreach, instructions, campaigns, clients, senders, exports, generated outputs, integrations or use of Velocity Vision.`,
          `Customer must not make personal allegations against Protected Parties in complaints, chargebacks, provider reports, regulator submissions, client disputes or third-party proceedings where the issue arises from Customer-controlled marketing activity, Customer data, Customer instructions or Customer use of the platform.`,
          `This section does not limit rights or liabilities that cannot legally be excluded, but it applies to the maximum extent permitted by law in every jurisdiction.`,
        ],
      },
      {
        heading: `4. Customer as Sender, Advertiser and Decision-Maker`,
        paragraphs: [
          `Customer is the sender, advertiser, marketer, campaign owner and decision-maker for all outreach, marketing, advertising, exports, generated outputs and activation decisions made through or using Velocity Vision, unless a separate signed written agreement expressly states otherwise.`,
          `Customer is solely responsible for:`,
          `* recipient selection;`,
          `* data source;`,
          `* lawful basis;`,
          `* consent;`,
          `* opt-outs;`,
          `* suppression;`,
          `* sender identity;`,
          `* advertiser identity;`,
          `* message content;`,
          `* subject lines;`,
          `* headers;`,
          `* claims;`,
          `* disclosures;`,
          `* timing;`,
          `* frequency;`,
          `* channel selection;`,
          `* jurisdictional analysis;`,
          `* client approvals;`,
          `* AI-output review;`,
          `* activation decisions;`,
          `* exports;`,
          `* follow-up;`,
          `* complaint handling;`,
          `* provider-policy compliance;`,
          `* downstream use.`,
          `Velocity Vision does not become the sender of record, advertiser of record, campaign owner, compliance approver, legal reviewer, consent validator, data broker, list owner, regulated adviser, professional adviser or managed marketing provider by making software tools available.`,
        ],
      },
      {
        heading: `5. Customer Responsibility`,
        paragraphs: [
          `Customer is solely responsible for all marketing, outreach, prospecting, lead generation, follow-up, sender setup, recipient selection, message content, lawful basis, consent, unsubscribe handling, suppression handling, customer-controlled activation, exports, generated outputs and campaign decisions.`,
          `This responsibility applies whether activity is:`,
          `* sent through Velocity Vision;`,
          `* prepared through Velocity Vision;`,
          `* exported from Velocity Vision;`,
          `* copied from Velocity Vision;`,
          `* adapted from Velocity Vision;`,
          `* generated using Velocity Vision;`,
          `* activated using Velocity Vision;`,
          `* managed through an agency workspace;`,
          `* performed by a user, employee, contractor, client, administrator, consultant or outsourced provider;`,
          `* performed through a third-party service after export;`,
          `* performed manually using Velocity Vision outputs.`,
          `Customer must not represent that Velocity Vision has approved, reviewed, certified, legally cleared, compliance-cleared or guaranteed any contact list, lawful basis, consent record, sender setup, message, campaign, generated output, follow-up, activation, export, deliverability result or commercial outcome.`,
        ],
      },
      {
        heading: `6. Business Use Only`,
        paragraphs: [
          `Velocity Vision is designed for lawful business, professional, commercial, agency, organizational and entrepreneurial use only.`,
          `Velocity Vision is not intended for personal, household or consumer use.`,
          `Customer must not use Velocity Vision for personal spam, consumer scams, household messaging, personal contact-list exploitation, unlawful consumer targeting or any marketing activity that is inconsistent with the Platform Terms of Service, Customer Agreement, Acceptable Use Policy or this Marketing Compliance Policy.`,
        ],
      },
      {
        heading: `7. Applicable Marketing Laws`,
        paragraphs: [
          `Customer is responsible for identifying and complying with all laws, regulations, guidance, regulator expectations, platform rules, provider rules, industry standards and contractual restrictions that apply to Customer’s marketing activity.`,
          `Relevant laws may include, depending on the jurisdiction and use case:`,
          `* GDPR;`,
          `* ePrivacy rules;`,
          `* CAN-SPAM;`,
          `* CASL;`,
          `* Australian Spam Act;`,
          `* CCPA/CPRA;`,
          `* US state privacy laws;`,
          `* consumer protection laws;`,
          `* unfair commercial practices laws;`,
          `* advertising standards;`,
          `* telemarketing laws;`,
          `* do-not-call laws;`,
          `* sector-specific marketing laws;`,
          `* financial promotion rules;`,
          `* healthcare marketing rules;`,
          `* political marketing rules;`,
          `* charity fundraising rules;`,
          `* AI transparency rules;`,
          `* automated decision-making rules;`,
          `* platform terms;`,
          `* provider rules;`,
          `* local laws in the recipient’s jurisdiction.`,
          `Customer must apply the rule required for the recipient, channel, sender, advertiser, location, message type, purpose, sector and jurisdiction.`,
          `Where rules differ, conflict or overlap, Customer is responsible for applying the stricter rule where needed to avoid unlawful marketing, platform risk, provider risk, recipient harm, data-subject harm or regulatory exposure.`,
        ],
      },
      {
        heading: `8. Recipient Location and Global Compliance`,
        paragraphs: [
          `Marketing rules differ by country, state, province, territory, recipient type, channel, sector and message purpose.`,
          `Customer is responsible for determining where recipients are located and which laws apply before activating, exporting, contacting or following up with any recipient.`,
          `Customer must not assume that a message is lawful merely because:`,
          `* the recipient is a business contact;`,
          `* the email address is a work address;`,
          `* the contact appears on a website;`,
          `* the contact is publicly listed;`,
          `* the contact is on a social network;`,
          `* the contact is in a directory;`,
          `* the contact attended an event;`,
          `* the contact downloaded material;`,
          `* the contact works for a company;`,
          `* the contact is a director or officer;`,
          `* the data was purchased;`,
          `* the data was supplied by a client;`,
          `* a third-party provider claimed the data was compliant;`,
          `* the platform did not block the record;`,
          `* an AI output was generated;`,
          `* a template was available;`,
          `* a sender was verified;`,
          `* a quality score was positive.`,
          `Customer remains responsible for local-law compliance in every relevant jurisdiction.`,
        ],
      },
      {
        heading: `9. Strictest Applicable Standard`,
        paragraphs: [
          `Where Customer is uncertain which law applies, where recipient location is unclear, where a recipient may be in multiple jurisdictions, where data source is unclear, where consent is unclear, where a channel is regulated, or where the message could be received in multiple jurisdictions, Customer must apply the strictest rule reasonably necessary to avoid unlawful marketing and recipient harm.`,
          `Customer must not use uncertainty as a reason to proceed with activation.`,
          `Customer must not shift uncertainty, ambiguity, conflict-of-law risk or local-law interpretation risk to Velocity Vision.`,
        ],
      },
      {
        heading: `10. Jurisdiction Blocking and Refusal Rights`,
        paragraphs: [
          `Velocity Vision may refuse, restrict, geoblock, suspend, throttle, limit or terminate marketing activity, exports, activation, sender connections, campaigns, workspaces, jurisdictions, sectors, recipient types, channels, features or accounts where we reasonably believe there is legal, regulatory, sanctions, payment, provider, privacy, AI, deliverability, reputational, operational or platform risk.`,
          `We are not required to support marketing activity in every country, language, sector, recipient category, channel or jurisdiction.`,
          `We may refuse support for any jurisdiction, sector, use case, data type, message type or customer instruction where continued support would increase risk to Velocity Vision, Protected Parties, recipients, data subjects, providers, infrastructure or the business.`,
        ],
      },
      {
        heading: `11. B2B, B2C and Individual Business Contacts`,
        paragraphs: [
          `Rules differ between business-to-business and business-to-consumer marketing.`,
          `A business email address, corporate contact, director email, employee email, professional profile or role-based contact may still be personal data if it identifies or relates to an individual.`,
          `Customer is responsible for determining whether each recipient is:`,
          `* an individual subscriber;`,
          `* corporate subscriber;`,
          `* consumer;`,
          `* business contact;`,
          `* employee;`,
          `* sole trader;`,
          `* partner;`,
          `* director;`,
          `* officer;`,
          `* client contact;`,
          `* agency client contact;`,
          `* regulated recipient;`,
          `* vulnerable recipient;`,
          `* person protected by local marketing rules.`,
          `Customer must not treat B2B marketing as automatically lawful. Customer must assess lawful basis, consent, legitimate interest, soft opt-in, opt-out, suppression and local requirements before activation.`,
        ],
      },
      {
        heading: `12. Lawful Basis and Consent`,
        paragraphs: [
          `Before activating, exporting, sending or using any contact for outreach, Customer must confirm that it has a lawful basis, consent or other required permission for the message, recipient, channel, jurisdiction and purpose.`,
          `Customer must maintain records sufficient to evidence, where applicable:`,
          `* consent;`,
          `* implied consent;`,
          `* express consent;`,
          `* legitimate interest;`,
          `* soft opt-in;`,
          `* existing customer relationship;`,
          `* contract necessity;`,
          `* conspicuous publication;`,
          `* referral source;`,
          `* client instruction;`,
          `* opt-out status;`,
          `* unsubscribe status;`,
          `* suppression status;`,
          `* recipient location;`,
          `* notice provided;`,
          `* date collected;`,
          `* source of data;`,
          `* permitted purpose;`,
          `* expiry or staleness of consent;`,
          `* applicable restrictions.`,
          `Customer must not activate, export, contact or follow up with a recipient where Customer cannot evidence the required lawful basis, consent or permission.`,
        ],
      },
      {
        heading: `13. Consent Quality and Proof`,
        paragraphs: [
          `Where consent is required, Customer is responsible for ensuring that consent is valid, specific, informed, freely given, unambiguous, current and capable of being evidenced where required by law.`,
          `Customer must not rely on consent that is:`,
          `* vague;`,
          `* bundled;`,
          `* coerced;`,
          `* stale;`,
          `* expired;`,
          `* withdrawn;`,
          `* unverifiable;`,
          `* obtained by deception;`,
          `* obtained by a third party without sufficient records;`,
          `* obtained for a different purpose;`,
          `* obtained for a different sender;`,
          `* obtained for a different advertiser;`,
          `* obtained for a different brand;`,
          `* obtained for a different channel;`,
          `* obtained for a different jurisdiction;`,
          `* invalid under applicable law.`,
          `Customer is responsible for proving consent if challenged by a recipient, regulator, provider, platform, client or third party.`,
        ],
      },
      {
        heading: `14. Consent Chain of Title`,
        paragraphs: [
          `Customer must be able to prove the full chain of authority for any contact data used through Velocity Vision.`,
          `This includes data obtained from:`,
          `* Customer’s own collection;`,
          `* client lists;`,
          `* agency client lists;`,
          `* affiliates;`,
          `* group companies;`,
          `* partners;`,
          `* referrals;`,
          `* events;`,
          `* webinars;`,
          `* lead magnets;`,
          `* downloaded materials;`,
          `* CRM imports;`,
          `* data brokers;`,
          `* lead vendors;`,
          `* public directories;`,
          `* social platforms;`,
          `* enrichment providers;`,
          `* purchased lists;`,
          `* rented lists;`,
          `* licensed datasets;`,
          `* third-party databases.`,
          `Customer must ensure that each collection, transfer, import, upload, enrichment, use, export and activation is lawful, permitted by the original notice or consent, and not prohibited by contract, platform rules, privacy law, marketing law or recipient restriction.`,
          `Customer must not rely on a vendor, client, affiliate, group company, partner or data broker’s generic assurance unless Customer has sufficient records to prove lawful use for the specific sender, advertiser, brand, channel, purpose, jurisdiction and recipient.`,
        ],
      },
      {
        heading: `15. Publicly Available Data`,
        paragraphs: [
          `Public availability does not automatically make data lawful for marketing.`,
          `Customer must not assume that a publicly listed email address, website contact, directory listing, social profile, company officer record, conference attendee list or public post may be used for outreach.`,
          `Customer is responsible for assessing whether use of publicly available data is lawful, fair, transparent, permitted by local law, consistent with the context of publication and not restricted by opt-out, objection, platform terms, professional rules, sector rules or privacy expectations.`,
        ],
      },
      {
        heading: `16. Legitimate Interest and Soft Opt-In`,
        paragraphs: [
          `Where Customer relies on legitimate interest, soft opt-in, implied consent, existing relationship or similar basis, Customer is responsible for ensuring that all legal conditions are satisfied.`,
          `Customer must consider:`,
          `* relationship with recipient;`,
          `* reasonable expectations of the recipient;`,
          `* context of data collection;`,
          `* product or service similarity;`,
          `* notice provided at collection;`,
          `* opportunity to opt out at collection;`,
          `* opportunity to opt out in each message;`,
          `* impact on the recipient;`,
          `* recipient vulnerability;`,
          `* frequency of contact;`,
          `* recipient location;`,
          `* local-law restrictions;`,
          `* sector-specific restrictions;`,
          `* whether the basis applies to prospects, customers or bought-in lists.`,
          `Customer must not rely on soft opt-in, legitimate interest or implied consent where local law does not allow it or where the conditions are not met.`,
        ],
      },
      {
        heading: `17. Pre-Activation Duties`,
        paragraphs: [
          `Before activating, exporting, sending, scheduling, copying, publishing or using any marketing content or recipient data, Customer must verify:`,
          `* recipient lawful basis;`,
          `* recipient location;`,
          `* sender identity;`,
          `* advertiser identity;`,
          `* message purpose;`,
          `* consent or lawful basis;`,
          `* suppression status;`,
          `* opt-out status;`,
          `* client approval where applicable;`,
          `* regulated-sector requirements;`,
          `* required disclosures;`,
          `* required unsubscribe mechanism;`,
          `* claim substantiation;`,
          `* AI-output accuracy;`,
          `* AI transparency requirements;`,
          `* channel rules;`,
          `* provider rules;`,
          `* frequency and timing;`,
          `* jurisdiction-specific restrictions;`,
          `* downstream-use restrictions.`,
          `Customer must not activate or export where any required compliance check is incomplete, uncertain, failed, stale, unsupported or disputed.`,
        ],
      },
      {
        heading: `18. Email and Message Requirements`,
        paragraphs: [
          `Commercial messages must comply with all applicable identification, transparency, sender, advertiser, unsubscribe, recordkeeping and content requirements.`,
          `Customer is responsible for ensuring that each message:`,
          `* uses accurate sender information;`,
          `* uses accurate “From” information;`,
          `* uses accurate “Reply-To” information;`,
          `* uses accurate routing information;`,
          `* does not use deceptive headers;`,
          `* does not use deceptive subject lines;`,
          `* does not use misleading preview text;`,
          `* identifies the sender or promoted organization where required;`,
          `* identifies the advertiser or beneficiary where required;`,
          `* identifies the commercial nature of the message where required;`,
          `* includes a valid opt-out or unsubscribe mechanism where required;`,
          `* includes a valid postal address or business contact details where required;`,
          `* includes required advertising disclosures where required;`,
          `* includes required regulated-sector disclosures where required;`,
          `* includes required terms, conditions or risk warnings where required;`,
          `* does not misrepresent identity, affiliation, purpose, authority, endorsement, offer, price, scarcity, outcome, legal rights or product capability.`,
          `Templates, AI-assisted drafts, workspace fields or platform prompts may assist with message preparation, but Customer remains responsible for final compliance.`,
        ],
      },
      {
        heading: `19. Unsubscribe, Opt-Out and Suppression Requirements`,
        paragraphs: [
          `Customer must maintain and honour unsubscribe requests, opt-out requests, objections, consent withdrawals, suppression requests, do-not-contact requests and similar restrictions.`,
          `Customer must ensure opt-outs are:`,
          `* easy to use where required;`,
          `* available in each message where required;`,
          `* honoured promptly;`,
          `* honoured within legally required timeframes;`,
          `* applied to the correct sender, brand, client, workspace, channel and purpose;`,
          `* recorded accurately;`,
          `* not ignored, bypassed or reactivated unlawfully.`,
          `Customer must not contact individuals who have opted out, withdrawn consent, objected, unsubscribed, complained or otherwise requested no further marketing where that request applies.`,
          `Customer must not charge a fee, require unnecessary information, force account login, require excessive steps, use dark patterns or create unreasonable friction for opt-out where prohibited by law.`,
        ],
      },
      {
        heading: `20. Duty to Stop`,
        paragraphs: [
          `Customer must immediately pause, suppress, stop or review marketing activity where:`,
          `* a recipient opts out;`,
          `* a recipient objects;`,
          `* a recipient complains;`,
          `* consent is withdrawn;`,
          `* lawful basis becomes uncertain;`,
          `* suppression status changes;`,
          `* bounce rates increase materially;`,
          `* complaint rates increase materially;`,
          `* spam reports increase materially;`,
          `* a sender is blocked;`,
          `* a domain is blacklisted;`,
          `* an email provider warns or restricts;`,
          `* an integration provider warns or restricts;`,
          `* a regulator contacts Customer;`,
          `* a client withdraws approval;`,
          `* a data source is challenged;`,
          `* a data breach affects the relevant data;`,
          `* a provider policy may have been breached;`,
          `* any law, rule or platform policy may have been breached.`,
          `Customer must not continue, repeat, reroute, relabel, export, reupload, resend, re-segment, rebrand or move the activity into another workspace, sender, domain, account, client or channel to avoid stopping.`,
        ],
      },
      {
        heading: `21. Suppression Data Restrictions`,
        paragraphs: [
          `Suppression data may only be used for lawful compliance and suppression purposes.`,
          `Customer must not sell, rent, share, activate, enrich, profile, target, monetise, reverse engineer, reuse or exploit suppression lists, opt-out lists, unsubscribe lists, objection lists or do-not-contact lists for unrelated marketing.`,
          `Customer must not upload suppression data unless it is authorized to process it and does so for lawful suppression and compliance purposes.`,
          `Customer must not use suppression data to identify people for renewed targeting, alternative-channel outreach, client resale, enrichment or campaign recycling.`,
        ],
      },
      {
        heading: `22. No List Washing`,
        paragraphs: [
          `Customer must not use Velocity Vision to clean, validate, enrich, reactivate, recycle, test, suppress, evade, match, compare or “wash” contact lists in order to contact people who have opted out, unsubscribed, objected, complained, suppressed their details or otherwise restricted contact.`,
          `Customer must not use Velocity Vision to test whether suppressed, old, invalid, stale, risky or unlawfully obtained contacts can be made contactable again.`,
          `Customer must not use alternate senders, alternate brands, alternate clients, alternate domains, alternate workspaces, alternate channels or agency accounts to bypass suppression or opt-out obligations.`,
        ],
      },
      {
        heading: `23. Data Source and List Hygiene`,
        paragraphs: [
          `Customer must maintain records showing:`,
          `* where contact data came from;`,
          `* when it was collected;`,
          `* who collected it;`,
          `* what notices were provided;`,
          `* what consent was obtained;`,
          `* what lawful basis applies;`,
          `* what purpose was disclosed;`,
          `* whether the recipient opted out;`,
          `* whether the contact has been suppressed;`,
          `* whether jurisdiction-specific restrictions apply;`,
          `* whether third-party restrictions apply;`,
          `* whether client authority exists;`,
          `* whether data may be used with AI-assisted features;`,
          `* whether data may be transferred internationally;`,
          `* whether downstream marketing use is permitted.`,
          `Customer must not use:`,
          `* harvested addresses;`,
          `* dictionary-generated addresses;`,
          `* guessed addresses;`,
          `* scraped data without lawful basis;`,
          `* purchased lists without lawful basis;`,
          `* rented lists without lawful basis;`,
          `* brokered lists without lawful basis;`,
          `* old lists without current permission;`,
          `* stale lists without validation of legal permission;`,
          `* lists obtained through deception;`,
          `* lists obtained through breach;`,
          `* lists subject to suppression or opt-out;`,
          `* lists from clients without lawful authority.`,
          `Quality flags, deduplication, verification, risk labels, enrichment, AI suggestions or platform readiness checks do not prove legal permission.`,
        ],
      },
      {
        heading: `24. AI-Assisted Marketing Outputs`,
        paragraphs: [
          `Velocity Vision may generate AI-assisted marketing drafts, outreach templates, follow-up messages, social posts, press releases, video scripts, summaries, segmentation suggestions, campaign variants and other generated outputs.`,
          `Generated outputs are drafts only.`,
          `Customer must review, edit, approve and verify generated outputs before sending, publishing, exporting, relying on or activating them.`,
          `Customer is responsible for ensuring generated outputs are:`,
          `* accurate;`,
          `* lawful;`,
          `* non-deceptive;`,
          `* non-infringing;`,
          `* appropriate for the recipient;`,
          `* appropriate for the jurisdiction;`,
          `* appropriate for the channel;`,
          `* appropriate for the sector;`,
          `* consistent with Customer’s brand;`,
          `* supported by evidence;`,
          `* compliant with applicable marketing, privacy, AI, consumer protection and sector-specific laws.`,
          `Customer must not use AI-generated outputs to misrepresent facts, qualifications, endorsements, pricing, scarcity, urgency, legal rights, regulatory status, product capability, customer results, case studies, testimonials, financial outcomes, health outcomes, investment outcomes, compliance status or AI capability.`,
        ],
      },
      {
        heading: `25. AI Transparency, Synthetic Content and Deepfake Restrictions`,
        paragraphs: [
          `Customer is responsible for determining whether AI-generated or AI-assisted marketing content must be labelled, disclosed, marked or otherwise identified under applicable AI, consumer protection, advertising, platform or sector-specific laws.`,
          `Customer must not use Velocity Vision to create or distribute:`,
          `* deceptive synthetic content;`,
          `* undisclosed deepfake-style content;`,
          `* fake endorsements;`,
          `* fake testimonials;`,
          `* fake user reviews;`,
          `* fake social proof;`,
          `* fake scarcity;`,
          `* fake urgency;`,
          `* fake identity;`,
          `* fake authority;`,
          `* misleading AI-generated publications;`,
          `* content that misleads recipients into believing they are interacting with a human where disclosure is required.`,
          `Customer is responsible for all required AI transparency notices, human oversight, approval steps, records and disclosures.`,
        ],
      },
      {
        heading: `26. AI Personalization and Manipulation`,
        paragraphs: [
          `Customer must not use Velocity Vision or generated outputs for deceptive personalization, manipulative targeting, fake familiarity, false relationship claims, synthetic identity deception, deepfake-style deception, fake endorsements, fake testimonials, fake scarcity, fake urgency, vulnerable-person manipulation or pressure tactics that are unlawful, misleading or inappropriate.`,
          `Customer is responsible for ensuring that any personalization based on Customer Personal Data is lawful, fair, transparent and appropriate.`,
          `Customer must not use AI-assisted features to create marketing that unlawfully exploits vulnerability, protected characteristics, health status, financial distress, age, disability, ethnicity, religion, political views, sexuality, trade union status or other sensitive traits.`,
        ],
      },
      {
        heading: `27. Regulated Claims and Substantiation`,
        paragraphs: [
          `Customer must ensure all claims in marketing content are truthful, accurate, substantiated and lawful.`,
          `Customer must not make claims that are false, misleading, unverified, exaggerated or unsupported, including claims about:`,
          `* revenue;`,
          `* profit;`,
          `* savings;`,
          `* investment returns;`,
          `* funding;`,
          `* health outcomes;`,
          `* legal outcomes;`,
          `* tax outcomes;`,
          `* compliance status;`,
          `* qualifications;`,
          `* customer results;`,
          `* case studies;`,
          `* testimonials;`,
          `* guarantees;`,
          `* AI capability;`,
          `* automation;`,
          `* deliverability;`,
          `* inbox placement;`,
          `* endorsements;`,
          `* certifications;`,
          `* regulatory approval.`,
          `Customer is responsible for retaining evidence supporting marketing claims where required by law, advertising standards, platform rules or regulator expectations.`,
        ],
      },
      {
        heading: `28. Regulated and Restricted Sectors`,
        paragraphs: [
          `Customer must not use Velocity Vision for marketing in regulated, sensitive or high-risk sectors unless Customer has all required permissions, licenses, legal bases, approvals, disclosures, warnings and compliance processes.`,
          `Restricted or high-risk sectors include:`,
          `* health;`,
          `* medical services;`,
          `* pharmaceuticals;`,
          `* finance;`,
          `* investment;`,
          `* credit;`,
          `* lending;`,
          `* insurance;`,
          `* legal services;`,
          `* immigration;`,
          `* employment;`,
          `* recruitment;`,
          `* education access;`,
          `* housing;`,
          `* politics;`,
          `* political campaigning;`,
          `* public-sector influence;`,
          `* gambling;`,
          `* alcohol;`,
          `* tobacco;`,
          `* nicotine;`,
          `* controlled substances;`,
          `* weapons;`,
          `* adult content;`,
          `* dating;`,
          `* surveillance;`,
          `* cybersecurity tools;`,
          `* debt collection;`,
          `* charities and fundraising where regulated;`,
          `* vulnerable-person services;`,
          `* children’s products or services;`,
          `* data brokerage;`,
          `* lead resale;`,
          `* telemarketing;`,
          `* call-centre operations;`,
          `* any other sector that creates legal, regulatory, reputational, safety or platform risk.`,
          `We may refuse, restrict, suspend or terminate marketing activity in any sector we consider unsuitable for Velocity Vision.`,
        ],
      },
      {
        heading: `29. Political, Public-Issue and Sensitive Influence`,
        paragraphs: [
          `Customer must not use Velocity Vision for political campaigning, voter targeting, public-issue persuasion, lobbying, government influence, public-sector influence, fundraising for political or sensitive causes, or targeting based on sensitive characteristics unless expressly authorized in writing and lawful in every relevant jurisdiction.`,
          `Customer must not use Velocity Vision to target individuals based on sensitive personal data, protected characteristics, vulnerability, financial distress, health status, political views, religious beliefs, trade union status, sexuality, ethnicity or similar sensitive traits.`,
          `We may refuse or restrict any use involving political, public-issue, sensitive influence or vulnerable-person targeting.`,
        ],
      },
      {
        heading: `30. High-Volume, Automated and Bulk Outreach`,
        paragraphs: [
          `Customer must not use Velocity Vision for high-volume, automated, bulk, aggressive, deceptive, evasive or uncontrolled outreach unless Customer’s plan expressly permits the relevant use and Customer’s activity complies with all applicable laws, provider rules and platform controls.`,
          `We may impose activation limits, sender limits, export limits, generation limits, rate limits, workspace limits, review gates, approval steps or manual checks at any time.`,
          `Customer must not split campaigns, duplicate workspaces, rotate senders, create multiple accounts, manipulate domains, use multiple clients or otherwise structure usage to avoid platform limits, provider limits, lawful-basis requirements, unsubscribe obligations, suppression requirements or enforcement controls.`,
        ],
      },
      {
        heading: `31. Deliverability and Sender Reputation`,
        paragraphs: [
          `Customer is responsible for sender reputation, domain reputation, message quality, complaint rates, bounce rates, spam classification, blacklist status, recipient response, email provider enforcement and inbox placement.`,
          `Customer must not use Velocity Vision to manipulate, deceive or abuse email providers, spam filters, inbox providers, recipient systems, sender reputation systems or domain reputation systems.`,
          `Customer must not use Velocity Vision for:`,
          `* domain spoofing;`,
          `* sender spoofing;`,
          `* deceptive domain rotation;`,
          `* deceptive inbox rotation;`,
          `* reputation laundering;`,
          `* unlawful domain warming;`,
          `* misleading sender identities;`,
          `* mailbox abuse;`,
          `* mass testing against spam filters;`,
          `* evasion of provider restrictions;`,
          `* reactivation of blocked senders;`,
          `* use of compromised mailboxes;`,
          `* use of unauthorized mailboxes.`,
          `We may suspend, restrict or disconnect senders, domains, integrations or workspaces if we detect bounce risk, complaint risk, spam risk, blacklist risk, sender reputation risk, domain reputation risk, provider risk or unlawful outreach risk.`,
        ],
      },
      {
        heading: `32. Channel-Specific Compliance`,
        paragraphs: [
          `Customer is responsible for complying with channel-specific requirements for:`,
          `* email;`,
          `* SMS;`,
          `* MMS;`,
          `* messaging apps;`,
          `* social media direct messages;`,
          `* social selling;`,
          `* telephone calls;`,
          `* voicemail;`,
          `* retargeting;`,
          `* advertising audiences;`,
          `* custom audiences;`,
          `* lookalike audiences;`,
          `* website tracking;`,
          `* cookies;`,
          `* pixels;`,
          `* CRM exports;`,
          `* third-party campaign tools.`,
          `Customer must not export or use data from Velocity Vision in another platform in a way that would violate this Marketing Compliance Policy, the Acceptable Use Policy, Data Processing Agreement, Privacy Policy, platform rules or applicable law.`,
        ],
      },
      {
        heading: `33. Telephone, SMS and Messaging Restrictions`,
        paragraphs: [
          `Customer must not use Velocity Vision to prepare, export, organize or support telephone, SMS, MMS, messaging-app or similar outreach unless Customer has all required consent, lawful basis, notices, opt-out processes, sender identification, do-not-call screening, timing controls, recordkeeping and channel-specific compliance.`,
          `Customer is responsible for compliance with calling, texting, messaging, do-not-call, quiet-hours, consent, revocation and recording laws where applicable.`,
          `Velocity Vision does not approve or certify any telephone, SMS, messaging-app or telemarketing use.`,
        ],
      },
      {
        heading: `34. Retargeting, Custom Audiences and Platform Uploads`,
        paragraphs: [
          `Customer must not export or upload Customer Personal Data, contact lists, suppression lists, lookalike audiences, custom audiences, hashed identifiers, email lists, phone numbers or other identifiers to advertising platforms, social platforms, CRM tools, retargeting tools or third-party marketing systems unless Customer has all required rights, notices, consents, lawful bases, platform permissions and contractual authority.`,
          `Customer is responsible for complying with the terms of any third-party platform used for retargeting, custom audiences, lookalike audiences, advertising, tracking, attribution or campaign measurement.`,
          `Velocity Vision does not approve or certify any export, audience upload, retargeting activity or downstream third-party marketing use.`,
        ],
      },
      {
        heading: `35. Agency Responsibilities`,
        paragraphs: [
          `Agency customers are responsible for each client workspace, client campaign, client list, client sender, client approval, client lawful basis, client privacy notice, client consent record, client suppression record, client opt-out process, client generated output, client activation and client follow-up activity.`,
          `Agencies must:`,
          `* obtain client authority before uploading data;`,
          `* obtain client authority before activation;`,
          `* verify client lawful basis;`,
          `* maintain client approvals;`,
          `* prevent cross-client data mixing;`,
          `* prevent cross-client suppression misuse;`,
          `* prevent unauthorized client access;`,
          `* ensure client senders are authorized;`,
          `* ensure client claims are substantiated;`,
          `* ensure client campaigns are lawful;`,
          `* flow down relevant obligations to clients and users.`,
          `Agencies must not represent that Velocity Vision has approved, reviewed, endorsed, guaranteed, legally cleared or compliance-cleared any client campaign, data source, outreach list, message, template, generated output, sender setup, deliverability result, recipient group or activation.`,
          `Velocity Vision does not contract directly with agency end clients unless expressly agreed in writing.`,
          `The agency account holder is responsible for all client workspace activity and must indemnify the Protected Parties for claims arising from client data, client outreach, client instructions, client users, client disputes or agency-managed use.`,
        ],
      },
      {
        heading: `36. Client Flow-Down Obligations`,
        paragraphs: [
          `If Customer uses Velocity Vision for clients, customers, brands, subsidiaries, portfolio companies, franchisees, partners, campaigns or third parties, Customer must ensure that those parties comply with this Marketing Compliance Policy, the Acceptable Use Policy, Platform Terms of Service, Customer Agreement, Data Processing Agreement and all applicable laws.`,
          `Customer remains responsible even if the relevant client, customer, brand, subsidiary, portfolio company, franchisee, partner or third party gave the instruction that caused the violation.`,
          `Customer must not rely on a client instruction as a defence to breaching this Marketing Compliance Policy.`,
        ],
      },
      {
        heading: `37. Platform Controls Are Not Legal Approval`,
        paragraphs: [
          `Sender verification, data quality checks, quality scores, readiness labels, risky-record flags, send limits, cadence controls, suppression prompts, activation gates, template fields, AI suggestions, support responses and platform warnings are operational safeguards only.`,
          `They are not:`,
          `* legal advice;`,
          `* compliance certification;`,
          `* consent validation;`,
          `* lawful-basis approval;`,
          `* list approval;`,
          `* sender approval;`,
          `* advertiser approval;`,
          `* message approval;`,
          `* campaign approval;`,
          `* deliverability guarantee;`,
          `* inbox placement guarantee;`,
          `* proof that a recipient may lawfully be contacted;`,
          `* proof that a message may lawfully be sent;`,
          `* proof that a generated output is compliant;`,
          `* proof that an export is lawful;`,
          `* proof that downstream use is lawful.`,
          `Customer remains responsible for final review, approval and compliance.`,
        ],
      },
      {
        heading: `38. No Waiver from Availability or Non-Blocking`,
        paragraphs: [
          `The fact that a feature, template, AI output, export, activation path, sender option, integration, field, workflow, segment, quality status or platform function is available does not mean that Customer’s intended use is lawful, approved, reviewed, certified or compliant.`,
          `The fact that Velocity Vision does not block, flag, warn, pause, restrict or reject an action does not mean that Customer’s action is lawful or permitted.`,
          `Customer remains responsible for compliance even where the platform allows a technical action.`,
        ],
      },
      {
        heading: `39. Compliance Evidence and Verification`,
        paragraphs: [
          `We may require Customer to provide evidence of lawful basis, consent, source, suppression handling, sender authority, advertiser authority, recipient permission, client authority, data ownership, opt-out processing, campaign approval, regulated-sector permission, AI transparency review, claim substantiation, provider permission or other compliance evidence where we reasonably believe there is legal, regulatory, security, payment, abuse, deliverability, reputational or operational risk.`,
          `Failure to provide satisfactory evidence may result in upload blocks, activation holds, export restrictions, sender holds, workspace restrictions, account review, suspension, termination, deletion, quarantine or refusal of future service.`,
          `A request for evidence does not mean that Velocity Vision approves, validates, certifies or assumes responsibility for Customer’s data, outreach, lawful basis, sender setup, campaign, AI use or generated outputs.`,
        ],
      },
      {
        heading: `40. Monitoring and Review`,
        paragraphs: [
          `We may, but are not required to, monitor, scan, log, review, analyse, investigate, preserve or restrict accounts, workspaces, data, generated outputs, usage patterns, sender connections, integrations, activation activity, exports, billing activity and platform signals.`,
          `We may do this to:`,
          `* operate Velocity Vision;`,
          `* improve safety;`,
          `* prevent abuse;`,
          `* enforce this Marketing Compliance Policy;`,
          `* comply with law;`,
          `* respond to complaints;`,
          `* protect infrastructure;`,
          `* protect recipients;`,
          `* protect customers;`,
          `* protect third-party providers;`,
          `* protect payment systems;`,
          `* protect AI systems;`,
          `* protect the business;`,
          `* protect Protected Parties.`,
          `No monitoring, review, automated check, manual check, warning, flag, hold, approval flow or failure to act makes us responsible for customer data, customer outputs, customer campaigns, customer compliance, customer outreach or customer legal obligations.`,
        ],
      },
      {
        heading: `41. Enforcement`,
        paragraphs: [
          `If we believe this Marketing Compliance Policy has been breached, or if we believe there is legal, regulatory, security, payment, provider, AI, deliverability, reputational, operational or platform risk, we may take any action we consider appropriate, including:`,
          `* blocking uploads;`,
          `* holding records;`,
          `* quarantining data;`,
          `* deleting data;`,
          `* pausing activation;`,
          `* restricting exports;`,
          `* disabling generated outputs;`,
          `* removing content;`,
          `* throttling usage;`,
          `* requiring verification;`,
          `* requiring compliance evidence;`,
          `* suspending senders;`,
          `* disconnecting integrations;`,
          `* disabling workspaces;`,
          `* restricting credits;`,
          `* canceling credits;`,
          `* downgrading plans;`,
          `* suspending accounts;`,
          `* terminating accounts;`,
          `* refusing future service;`,
          `* notifying providers;`,
          `* notifying customers;`,
          `* notifying affected parties;`,
          `* preserving records;`,
          `* reporting unlawful activity where appropriate.`,
          `We may act immediately and without prior notice where needed to protect Velocity Vision, customers, users, recipients, infrastructure, third-party providers, payment systems, AI systems, sender systems, legal compliance, platform integrity, Protected Parties or our business.`,
          `We are not liable for loss arising from enforcement action taken in good faith to protect the platform, customers, recipients, third-party providers, legal compliance or the business.`,
        ],
      },
      {
        heading: `42. No Refund for Enforcement`,
        paragraphs: [
          `Enforcement action under this Marketing Compliance Policy does not create any right to a refund, credit, service credit, damages, compensation, chargeback, termination payment or extension unless expressly required by law.`,
          `Customer remains responsible for fees, taxes, usage charges, credit usage, chargebacks, indemnity obligations, confidentiality obligations, data compliance obligations, acceptable use obligations and outreach compliance obligations already incurred.`,
        ],
      },
      {
        heading: `43. Third-Party Provider Rules`,
        paragraphs: [
          `Velocity Vision may rely on third-party providers for hosting, payments, AI processing, analytics, email connectivity, communications, authentication, logging, monitoring, translation, support and infrastructure.`,
          `Customer must not use Velocity Vision in a way that violates third-party provider terms, platform rules, acceptable use policies, AI provider policies, email provider policies, payment provider rules, hosting provider rules or integration provider rules.`,
          `We may restrict, suspend, remove or modify access where required by third-party provider rules or where continued use could create provider, infrastructure, payment, deliverability, legal or operational risk.`,
        ],
      },
      {
        heading: `44. Legal, Regulatory and Third-Party Requests`,
        paragraphs: [
          `If we receive a subpoena, court order, regulator request, law enforcement request, data subject request, rights-holder complaint, payment-provider request, email-provider request, AI-provider request, third-party platform request, security complaint, spam complaint, abuse report, deliverability complaint or legal notice relating to Customer’s account, workspace, users, data, outreach, outputs, clients or integrations, we may respond, disclose information, preserve records, restrict access, notify Customer or take other action as we consider legally required or reasonably appropriate.`,
          `We are not required to challenge any legal, regulatory, payment-provider, email-provider, AI-provider or third-party request unless required by law or expressly agreed in writing.`,
          `Customer is responsible for costs, losses, fees, penalties, legal fees and operational burden arising from requests, complaints, investigations or claims caused by Customer’s data, outreach, instructions, users, clients, integrations, sender activity or use of Velocity Vision.`,
        ],
      },
      {
        heading: `45. Regulator, Provider and Recipient Complaints`,
        paragraphs: [
          `Customer must promptly notify us if Customer receives a regulator enquiry, provider warning, spam complaint, legal notice, data subject complaint, recipient complaint, email provider enforcement notice, blacklist notice, platform enforcement notice or similar issue relating to activity conducted through or prepared using Velocity Vision.`,
          `Customer must cooperate with reasonable investigation, mitigation, preservation, evidence, suppression, suspension, provider, legal or regulatory steps.`,
          `Customer must not conceal complaints, split campaigns, move activity to another workspace, use another sender, use another domain, use another account or continue substantially similar activity to avoid enforcement.`,
        ],
      },
      {
        heading: `46. Records and Evidence`,
        paragraphs: [
          `Platform records, usage logs, billing logs, support records, security logs, audit logs, activation logs, export logs, acceptance logs, sender logs, integration logs and enforcement records may be used as evidence of activity, acceptance, breach, risk, investigation, payment status, activation, export, compliance review or enforcement action.`,
          `Customer is responsible for maintaining its own consent records, lawful-basis records, suppression records, opt-out records, sender records, advertiser records, client-approval records, claim substantiation records, AI review records and campaign records.`,
          `Customer agrees that Velocity Vision records may be relied upon to the maximum extent permitted by law.`,
        ],
      },
      {
        heading: `47. Customer Warranties`,
        paragraphs: [
          `Customer represents and warrants that:`,
          `* it has authority to use all data processed through Velocity Vision;`,
          `* it has all required lawful bases, consents, permissions, notices and approvals;`,
          `* its marketing activity complies with applicable law;`,
          `* its senders and advertisers are accurately identified;`,
          `* its claims are truthful and substantiated;`,
          `* its opt-out and suppression processes are effective;`,
          `* its users are authorized and trained;`,
          `* its client instructions are lawful;`,
          `* its agency client activity is authorized;`,
          `* its AI-assisted marketing use is lawful;`,
          `* its exports and downstream use are lawful;`,
          `* it will not use Velocity Vision to evade legal, platform, provider or recipient restrictions.`,
          `Customer acknowledges that Velocity Vision relies on these warranties when providing access to the platform.`,
        ],
      },
      {
        heading: `48. Indemnity`,
        paragraphs: [
          `Customer agrees to defend, indemnify and hold harmless the Protected Parties from and against all claims, demands, investigations, complaints, proceedings, losses, liabilities, damages, penalties, fines, settlements, costs and expenses, including reasonable legal fees, arising from or related to:`,
          `* Customer’s breach of this Marketing Compliance Policy;`,
          `* Customer’s data;`,
          `* Customer’s contact lists;`,
          `* Customer’s prompts;`,
          `* Customer’s generated outputs;`,
          `* Customer’s outreach;`,
          `* Customer’s sender identity;`,
          `* Customer’s advertiser identity;`,
          `* Customer’s client activity;`,
          `* Customer’s agency workspaces;`,
          `* Customer’s unlawful marketing;`,
          `* Customer’s data protection violation;`,
          `* Customer’s privacy violation;`,
          `* Customer’s AI misuse;`,
          `* Customer’s regulated-sector activity;`,
          `* Customer’s suppression failure;`,
          `* Customer’s opt-out failure;`,
          `* Customer’s consent failure;`,
          `* Customer’s lawful-basis failure;`,
          `* Customer’s list washing;`,
          `* Customer’s deliverability or sender reputation abuse;`,
          `* Customer’s high-volume activation;`,
          `* Customer’s third-party provider violation;`,
          `* Customer’s user activity;`,
          `* Customer’s customer-side systems;`,
          `* Customer’s breach of law;`,
          `* Customer’s breach of third-party rights;`,
          `* Customer’s failure to stop;`,
          `* Customer’s failure to keep records;`,
          `* Customer’s failure to substantiate claims;`,
          `* Customer’s export or downstream use;`,
          `* Customer’s retargeting or custom-audience use;`,
          `* Customer’s provider-policy breach;`,
          `* Customer’s client instruction;`,
          `* Customer’s agency client dispute;`,
          `* Customer’s recipient complaints;`,
          `* Customer’s regulator complaints;`,
          `* Customer’s data subject complaints.`,
          `We may control the defence of any matter subject to indemnity. Customer must cooperate with us and must not settle any claim in a way that imposes obligations, admissions, restrictions or liability on any Protected Party without our prior written consent.`,
          `This indemnity survives suspension, termination, cancellation, account closure and deletion of data.`,
        ],
      },
      {
        heading: `49. Relationship with Other Terms`,
        paragraphs: [
          `This Marketing Compliance Policy supplements the Platform Terms of Service, Customer Agreement, Data Processing Agreement, Acceptable Use Policy, Privacy Policy, Cookie Policy, Platform Security Policy and Service Level Agreement.`,
          `If there is any conflict, the stricter or more protective provision applies to the maximum extent permitted by law.`,
          `Nothing in this Marketing Compliance Policy limits our rights under any other agreement, policy, law or equitable remedy.`,
        ],
      },
      {
        heading: `50. Local Law and Mandatory Rights`,
        paragraphs: [
          `This Marketing Compliance Policy applies to the maximum extent permitted by applicable law.`,
          `If mandatory law in a jurisdiction gives rights that cannot be excluded, restricted or modified, this Marketing Compliance Policy does not exclude those rights, but all exclusions, limitations, restrictions, indemnities and protections continue to apply to the maximum extent permitted by that law.`,
          `Where a provision cannot be enforced as written in a particular jurisdiction, it will be interpreted, narrowed or reformed only to the minimum extent necessary to make it enforceable, and the rest of this Marketing Compliance Policy remains effective.`,
        ],
      },
      {
        heading: `51. No Assumption of Customer Legal Duties`,
        paragraphs: [
          `Nothing in this Marketing Compliance Policy, the platform, product interface, support, documentation, templates, prompts, generated outputs, activation controls or compliance reminders causes Global Solutions Management LLC or any Protected Party to assume Customer’s legal, regulatory, contractual, marketing, privacy, AI, advertising, sender, advertiser, client, agency, recordkeeping, suppression or opt-out obligations.`,
          `Customer remains responsible even where Velocity Vision provides tools, checks, prompts, settings, warnings, templates, AI-assisted outputs or operational safeguards.`,
        ],
      },
      {
        heading: `52. Changes to this Marketing Compliance Policy`,
        paragraphs: [
          `We may update this Marketing Compliance Policy from time to time to reflect product changes, legal developments, security requirements, operational needs, provider requirements, AI provider requirements, payment provider requirements, infrastructure changes, abuse patterns, marketing-law developments or changes to our business.`,
          `Continued use of Velocity Vision after the effective date of an updated Marketing Compliance Policy means Customer accepts the updated policy.`,
          `If Customer does not agree to an updated Marketing Compliance Policy, Customer must stop using Velocity Vision.`,
        ],
      },
      {
        heading: `53. Survival`,
        paragraphs: [
          `Any provision that by its nature should survive suspension, termination, cancellation, account closure or deletion of data will survive, including provisions relating to customer responsibility, lawful basis, consent, suppression, opt-outs, prohibited marketing, evidence, monitoring, enforcement, no refund, indemnity, Protected Parties, no personal claims, local law, relationship with other terms, records, warranties and legal requests.`,
        ],
      },
      {
        heading: `54. Contact`,
        paragraphs: [
          `For marketing compliance questions, abuse reports, policy concerns or compliance concerns, use the Contact page and select the route that matches your request.`,
          `Global Solutions Management LLC`,
          `Delaware, United States`,
          `Operator of Velocity Vision`,
        ],
      },
    ],
  },
  "cookie-policy": {
    title: "Cookie Policy",
    effectiveDate: "30 June 2026",
    lastUpdated: "1 July 2026",
    version: "8.0",
    sections: [
      {
        heading: `1. Purpose`,
        paragraphs: [
          `Cookies and similar technologies help Velocity Vision operate, secure, improve and support the website and platform.`,
          `They may be used to keep users signed in, authenticate sessions, remember preferences, protect accounts, process billing flows, detect abuse, prevent fraud, analyse usage, monitor performance, support integrations, manage consent choices, understand lawful campaign effectiveness and improve reliability.`,
          `This Cookie Policy gives users information about the categories of cookies and similar technologies we may use and how users can manage them.`,
        ],
      },
      {
        heading: `2. Delaware Operator`,
        paragraphs: [
          `Velocity Vision is operated by Global Solutions Management LLC, a Delaware limited liability company.`,
          `Global Solutions Management LLC is the operator of the Velocity Vision website and platform.`,
          `The Platform Terms of Service, Customer Agreement, Privacy Policy and Data Processing Agreement contain important provisions on customer responsibility, disclaimers, limitations of liability, indemnities, no personal liability, director and staff protection, governing law and customer-controlled processing.`,
        ],
      },
      {
        heading: `3. Protected Parties`,
        paragraphs: [
          `For purposes of this Cookie Policy, Protected Parties means Global Solutions Management LLC and its present and former owners, members, managers, directors, officers, employees, staff, contractors, consultants, agents, representatives, affiliates, shareholders or equityholders where applicable, licensors, suppliers, service providers, subprocessors, payment providers, infrastructure providers, AI providers, hosting providers, analytics providers, cookie providers, consent-management providers, security providers, professional advisers, successors and assigns.`,
          `To the maximum extent permitted by law, users and customers must not attempt to hold any Protected Party personally responsible for customer-controlled cookie choices, customer-controlled integrations, customer websites, customer marketing tools, customer exports, customer downstream use, customer pixels, customer analytics, customer advertising platforms, browser settings, device settings, third-party provider actions or enforcement decisions.`,
          `The Protected Parties are intended beneficiaries of the protections, restrictions, enforcement rights, indemnities, limitations, disclaimers and no-personal-liability provisions in this Cookie Policy and the wider platform terms.`,
          `Nothing in this Cookie Policy excludes liability where liability cannot lawfully be excluded.`,
        ],
      },
      {
        heading: `4. No Personal Claims Against Owners, Directors, Managers, Staff or Providers`,
        paragraphs: [
          `To the maximum extent permitted by law, any claim, complaint, demand, dispute, proceeding or allegation relating to cookies, tracking technologies, consent records, analytics, advertising technologies, customer-controlled tracking, customer websites, customer integrations, customer exports or downstream customer use must be brought only against Global Solutions Management LLC where a claim against Velocity Vision is legally permitted, and not personally against any owner, member, manager, director, officer, employee, staff member, contractor, consultant, adviser, supplier, service provider, subprocessor, AI provider, hosting provider, payment provider, analytics provider, cookie provider, consent-management provider, integration provider or infrastructure provider.`,
          `Customers must not encourage, assist, fund, support, threaten or pursue personal claims against Protected Parties arising from customer-controlled cookies, customer websites, customer campaigns, customer tracking tools, customer pixels, customer analytics, customer integrations, customer exports, customer advertising platforms, customer consent failures or customer downstream use.`,
          `This section applies to the maximum extent permitted by law in every jurisdiction and does not limit rights or liabilities that cannot legally be excluded.`,
        ],
      },
      {
        heading: `5. Scope`,
        paragraphs: [
          `This Cookie Policy applies to cookies and similar technologies used in connection with:`,
          `This Cookie Policy does not govern cookies or tracking technologies used independently by customers on their own websites, landing pages, email campaigns, advertising accounts, domains, CRM systems, analytics tools, client websites or third-party systems.`,
          `Customers are responsible for their own cookie notices, consent tools, tracking controls, analytics settings, advertising tags, pixels, email tracking, retargeting, custom audiences and downstream use of exported data.`,
        ],
        bullets: [
          `the Velocity Vision public website`,
          `account login`,
          `authentication`,
          `user sessions`,
          `platform workspaces`,
          `customer dashboards`,
          `billing and checkout flows`,
          `support tools`,
          `security tools`,
          `analytics tools`,
          `performance monitoring`,
          `preference management`,
          `consent management`,
          `language display`,
          `embedded content`,
          `integrations`,
          `lawful marketing measurement where enabled`,
          `related platform operations.`,
        ],
      },
      {
        heading: `6. Cookies and Similar Technologies`,
        paragraphs: [
          `Cookies are small text files that may be stored on a browser or device.`,
          `Similar technologies may include:`,
          `References to “cookies” in this Cookie Policy include similar technologies where the context allows.`,
        ],
        bullets: [
          `local storage`,
          `session storage`,
          `pixels`,
          `tags`,
          `beacons`,
          `SDKs`,
          `scripts`,
          `device identifiers`,
          `browser identifiers`,
          `log files`,
          `server-side logs`,
          `tracking URLs`,
          `analytics identifiers`,
          `consent records`,
          `security tokens`,
          `embedded content signals`,
          `server-side tracking`,
          `fingerprinting-like signals where lawful and enabled.`,
        ],
      },
      {
        heading: `7. Essential Cookies`,
        paragraphs: [
          `Essential cookies are necessary for the website or platform to function properly.`,
          `They may be used for:`,
          `Essential cookies may be set without consent where permitted by law because they are needed to provide a service requested by the user, protect accounts, secure transactions or operate the platform.`,
          `If essential cookies are blocked, Velocity Vision may not work properly or may not work at all.`,
        ],
        bullets: [
          `login`,
          `authentication`,
          `account security`,
          `session management`,
          `fraud prevention`,
          `abuse prevention`,
          `routing`,
          `load balancing`,
          `consent recording`,
          `billing flow security`,
          `checkout security`,
          `workspace access`,
          `role permissions`,
          `platform integrity`,
          `legal or security logging`,
          `remembering privacy choices.`,
        ],
      },
      {
        heading: `8. Preference Cookies`,
        paragraphs: [
          `Preference cookies may remember choices such as:`,
          `Preference cookies help improve user experience but may not be strictly necessary.`,
          `Where required by law, preference cookies may be subject to consent.`,
        ],
        bullets: [
          `language`,
          `region`,
          `currency`,
          `interface settings`,
          `accessibility settings`,
          `display preferences`,
          `cookie choices`,
          `workspace preferences.`,
        ],
      },
      {
        heading: `9. Analytics Cookies`,
        paragraphs: [
          `Analytics cookies help us understand how users interact with the website and platform.`,
          `They may help us measure:`,
          `Analytics may be aggregated, de-identified, pseudonymised or limited where appropriate.`,
          `Where required by law, analytics cookies are used only with consent unless an applicable exemption applies.`,
        ],
        bullets: [
          `pages viewed`,
          `features used`,
          `visit frequency`,
          `referral sources`,
          `device type`,
          `browser type`,
          `approximate location`,
          `product usage`,
          `errors`,
          `performance issues`,
          `user journeys`,
          `conversion paths`,
          `feature adoption.`,
        ],
      },
      {
        heading: `10. Performance and Diagnostics Cookies`,
        paragraphs: [
          `Performance and diagnostics cookies may help us:`,
          `Some performance technologies may be essential where they are necessary for security, reliability or service delivery. Others may require consent depending on their purpose, configuration and applicable law.`,
        ],
        bullets: [
          `detect technical errors`,
          `measure page speed`,
          `identify broken flows`,
          `diagnose outages`,
          `monitor reliability`,
          `improve platform performance`,
          `understand failed requests`,
          `investigate bugs`,
          `improve security and stability.`,
        ],
      },
      {
        heading: `11. Security, Fraud and Abuse-Prevention Technologies`,
        paragraphs: [
          `We may use cookies and similar technologies to:`,
          `Security and fraud-prevention technologies may be used without consent where permitted by law because they are necessary to protect the platform, users, accounts, payments and infrastructure.`,
        ],
        bullets: [
          `detect suspicious activity`,
          `prevent account takeover`,
          `prevent credential abuse`,
          `prevent spam`,
          `prevent bot activity`,
          `prevent payment fraud`,
          `detect unauthorized access`,
          `enforce rate limits`,
          `preserve audit logs`,
          `support legal compliance`,
          `protect users, customers, recipients, providers and the platform.`,
        ],
      },
      {
        heading: `12. Billing and Payment Cookies`,
        paragraphs: [
          `Billing and payment providers may use cookies and similar technologies to:`,
          `Payment providers may process information under their own terms and privacy notices.`,
          `Velocity Vision does not intentionally store full payment card numbers where payment processors provide tokenised payment processing.`,
        ],
        bullets: [
          `process payments`,
          `secure checkout`,
          `prevent fraud`,
          `remember billing sessions`,
          `support tax calculations`,
          `support payment authentication`,
          `detect chargeback risk`,
          `comply with payment network rules`,
          `comply with legal obligations.`,
        ],
      },
      {
        heading: `13. Authentication and Workspace Cookies`,
        paragraphs: [
          `Authentication and workspace cookies may be used to:`,
          `These cookies are usually essential to provide the platform.`,
          `Blocking them may prevent login, workspace access, billing access, exports, activation or support functions.`,
        ],
        bullets: [
          `keep users signed in`,
          `verify identity`,
          `manage sessions`,
          `apply workspace permissions`,
          `remember roles`,
          `protect restricted areas`,
          `prevent unauthorized workspace access`,
          `support account recovery`,
          `support security logs.`,
        ],
      },
      {
        heading: `14. Marketing Cookies`,
        paragraphs: [
          `Marketing cookies may be used only where enabled and lawful.`,
          `They may help us:`,
          `Where required by law, marketing cookies are used only with consent or other required permission.`,
          `We do not use customer-uploaded contact data to market to those contacts for our own purposes while acting as processor or service provider for the customer.`,
        ],
        bullets: [
          `understand campaign effectiveness`,
          `measure visits from campaigns`,
          `avoid showing irrelevant communications`,
          `improve business communications`,
          `measure conversion from lawful ads`,
          `understand website engagement`,
          `support lawful remarketing where enabled.`,
        ],
      },
      {
        heading: `15. Advertising, Sale, Sharing and Cross-Context Tracking`,
        paragraphs: [
          `If advertising or cross-context tracking technologies are used, they may involve third-party providers that measure interactions across websites, apps or services.`,
          `Where such use is considered “sale”, “sharing”, targeted advertising, behavioral advertising, profiling or similar activity under applicable law, we will provide required notices and choices where those laws apply.`,
          `We do not sell Customer Personal Data processed on behalf of customers.`,
          `We do not share Customer Personal Data processed on behalf of customers for cross-context behavioral advertising.`,
          `If our practices change in a way that requires additional notice, opt-out controls or consent, we will update the relevant policy and provide required choices.`,
        ],
      },
      {
        heading: `16. Pixels, Tags and Tracking URLs`,
        paragraphs: [
          `We may use pixels, tags or tracking URLs where lawful to:`,
          `Where pixels, tags or tracking URLs require consent or opt-out rights, we will apply the applicable consent or opt-out framework where required.`,
          `Customers are responsible for any pixels, tags, tracking URLs, retargeting scripts, custom audiences or similar technologies they use outside Velocity Vision or through their own websites, campaigns or third-party tools.`,
        ],
        bullets: [
          `measure website performance`,
          `understand campaign effectiveness`,
          `measure email or service-message performance`,
          `diagnose deliverability issues`,
          `monitor security`,
          `detect abuse`,
          `support analytics`,
          `improve product communications.`,
        ],
      },
      {
        heading: `17. AI, Analytics and Product Improvement`,
        paragraphs: [
          `Cookie and usage data may help us understand how users interact with AI-assisted features, generated outputs, workflows, templates, exports and activation controls.`,
          `We may use operational data, usage data, analytics data, aggregated data, anonymised data or de-identified data to maintain, secure, analyse, improve and develop Velocity Vision.`,
          `We do not use Customer Personal Data to train a publicly available foundation model or general-purpose AI model unless the customer expressly authorizes that use or the data has been aggregated, anonymised or de-identified so that it no longer identifies the customer or data subjects.`,
        ],
      },
      {
        heading: `18. Third-Party Technologies`,
        paragraphs: [
          `Third-party providers may set or access cookies or similar technologies when they provide services such as:`,
          `Third-party technologies may be governed by the relevant third party’s own privacy policy, cookie policy, terms and technical configuration as well as our agreements with them.`,
          `Some third-party providers may act as independent controllers, businesses or equivalent roles for certain processing under their own policies.`,
          `We are not responsible for third-party practices outside our reasonable control.`,
        ],
        bullets: [
          `hosting`,
          `authentication`,
          `payments`,
          `analytics`,
          `performance monitoring`,
          `security`,
          `fraud prevention`,
          `support`,
          `communications`,
          `embedded tools`,
          `consent management`,
          `language translation`,
          `AI processing`,
          `email connectivity`,
          `CRM integrations`,
          `error logging`,
          `diagnostics.`,
        ],
      },
      {
        heading: `19. Machine Translation Cookies`,
        paragraphs: [
          `Velocity Vision may use automated machine-translation technology, including GTranslate, Google Translate or similar services, to display public legal pages or interface text in a user’s preferred browser language.`,
          `Machine translation is provided for convenience only. The English version of legal documents controls.`,
          `Translation providers may use cookies or similar technologies to provide language display, remember preferences or operate the translation tool.`,
          `Machine translation is not intended to translate Customer Personal Data for storage in the platform database unless a specific translation feature is made available and Customer chooses to use it.`,
        ],
      },
      {
        heading: `20. Embedded Content and External Links`,
        paragraphs: [
          `Velocity Vision may include embedded content, links, forms, widgets, videos, documents, support tools, payment windows or third-party interfaces.`,
          `Embedded content or external services may set their own cookies or similar technologies.`,
          `We do not control all cookies or tracking technologies used by third-party websites or services once a user leaves Velocity Vision or interacts with a third-party service.`,
          `Users should review the cookie and privacy policies of third-party services they choose to use.`,
        ],
      },
      {
        heading: `21. Customer-Controlled Integrations`,
        paragraphs: [
          `Customers may connect third-party integrations, email accounts, CRM systems, analytics tools, advertising tools, support tools or other services to Velocity Vision.`,
          `Those services may use cookies, tokens, logs, tracking identifiers or similar technologies.`,
          `Customers are responsible for ensuring that their connected services, client tools, agency tools, sender accounts, analytics settings, advertising settings and downstream integrations comply with applicable law.`,
          `Velocity Vision does not approve, certify or assume responsibility for customer-controlled third-party tools, customer settings, customer exports, customer advertising accounts, customer analytics tools or downstream tracking.`,
        ],
      },
      {
        heading: `22. Consent Management`,
        paragraphs: [
          `Where required by applicable law, we may use a consent banner, cookie settings tool, preference centre or similar mechanism to request, record and manage cookie choices.`,
          `Consent tools may allow users to accept, reject or manage categories of non-essential cookies.`,
          `The availability and design of consent tools may vary by region, website area, product area, account status, browser, device, feature, provider and legal requirement.`,
          `Essential cookies may remain active even if non-essential cookies are rejected.`,
        ],
      },
      {
        heading: `23. Consent Records`,
        paragraphs: [
          `We may keep records of cookie choices, consent status, opt-out choices, preference selections, banner interactions, timestamps, device identifiers, browser identifiers, region signals and related logs.`,
          `These records may be used to:`,
          `Consent records may be retained for as long as reasonably necessary for legal, compliance, security, audit, dispute, fraud-prevention and operational purposes.`,
        ],
        bullets: [
          `remember user choices`,
          `comply with law`,
          `demonstrate consent`,
          `support audits`,
          `troubleshoot consent tools`,
          `prevent repeated prompts`,
          `enforce privacy choices`,
          `investigate disputes`,
          `defend legal claims.`,
        ],
      },
      {
        heading: `24. No Dark Patterns`,
        paragraphs: [
          `Where a consent or preference tool is used, we aim to avoid designs that are intended to unlawfully mislead users, prevent meaningful choice or make refusal materially harder than acceptance where applicable law prohibits that practice.`,
          `Users remain responsible for their own browser, device, privacy-extension and account settings.`,
          `Customers are responsible for consent tools, cookie banners, tracking disclosures and privacy choices used on their own websites, campaigns, landing pages and third-party tools.`,
        ],
      },
      {
        heading: `25. Managing Cookies`,
        paragraphs: [
          `Users can control cookies through:`,
          `Blocking or deleting cookies may affect website or platform functionality.`,
          `Some cookies are necessary for login, account security, billing, workspace access, authentication, fraud prevention, consent records, preferences or platform operations.`,
        ],
        bullets: [
          `website cookie tools where available`,
          `browser settings`,
          `device settings`,
          `privacy extensions`,
          `operating system controls`,
          `third-party provider controls`,
          `opt-out tools where available`,
          `Global Privacy Control or similar signals where legally required and technically feasible.`,
        ],
      },
      {
        heading: `26. Browser Settings`,
        paragraphs: [
          `Most browsers allow users to block, delete or restrict cookies.`,
          `Browser settings may need to be managed separately for each browser, device and profile.`,
          `If cookies are deleted, previous choices may be lost and the user may need to set preferences again.`,
          `Blocking all cookies may prevent Velocity Vision from functioning correctly.`,
        ],
      },
      {
        heading: `27. Global Privacy Control and Opt-Out Signals`,
        paragraphs: [
          `Where required by applicable law and technically feasible, we may recognize legally required opt-out preference signals, such as Global Privacy Control, for website-level sale, sharing or targeted advertising choices.`,
          `Because Velocity Vision is primarily a business software platform and customer-uploaded data is controlled by customers, some requests or signals relating to customer workspace data may need to be directed to the relevant customer.`,
          `We do not guarantee that all browser-based signals will apply to authenticated workspace activity, customer-controlled processing, customer integrations, third-party services or downstream use.`,
        ],
      },
      {
        heading: `28. Do Not Track`,
        paragraphs: [
          `Some browsers may offer “Do Not Track” signals.`,
          `There is not always a consistent industry standard for responding to Do Not Track signals.`,
          `We may not respond to Do Not Track signals unless required by applicable law.`,
          `Where legally required opt-out preference signals apply, we will handle them as described in this Cookie Policy and the Privacy Policy.`,
        ],
      },
      {
        heading: `29. Sale, Sharing and Targeted Advertising Choices`,
        paragraphs: [
          `If we use cookies or similar technologies in a way that is considered sale, sharing, targeted advertising, behavioral advertising or similar activity under applicable privacy laws, we will provide required notices and choices where those laws apply.`,
          `Choices may include a cookie settings tool, opt-out link, preference centre, Global Privacy Control recognition or other legally required mechanism.`,
          `We do not sell Customer Personal Data processed on behalf of customers.`,
          `We do not share Customer Personal Data processed on behalf of customers for cross-context behavioral advertising.`,
        ],
      },
      {
        heading: `30. Regional Differences`,
        paragraphs: [
          `Cookie and tracking rules differ by country, state, province, territory and region.`,
          `Some regions require opt-in consent for non-essential cookies. Some regions require opt-out rights for sale, sharing, targeted advertising or behavioral advertising. Some regions apply different rules to analytics, security, advertising, embedded content, device identifiers or similar technologies.`,
          `We may provide different cookie banners, settings, disclosures, opt-out tools or consent mechanisms depending on region, product area, account status, provider configuration, technical feasibility or legal requirement.`,
        ],
      },
      {
        heading: `31. Customer Websites and Customer Campaigns`,
        paragraphs: [
          `This Cookie Policy applies to Velocity Vision’s website and platform. It does not apply to customer websites, customer landing pages, customer email campaigns, customer advertising accounts, customer pixels, customer retargeting tools, customer analytics, customer CRM exports or customer-controlled third-party systems.`,
          `Customers are responsible for their own cookie policies, consent banners, tracking disclosures, analytics settings, pixel controls, email tracking, advertising settings, retargeting settings and legal compliance.`,
          `Customers must not use Velocity Vision exports, generated outputs, contact records or integrations in a way that violates cookie, tracking, privacy, marketing, advertising, platform or provider laws.`,
        ],
      },
      {
        heading: `32. Email Tracking and Service Communications`,
        paragraphs: [
          `Velocity Vision may use tracking technologies in service communications, billing messages, support messages, security notices or lawful marketing communications where permitted by law.`,
          `These technologies may help us understand whether messages were delivered, opened, clicked, blocked, bounced or interacted with.`,
          `Users may manage marketing communications through unsubscribe links where available, but service, billing, legal and security communications may still be sent where necessary.`,
          `Customers are responsible for any tracking used in their own outreach, exported campaigns, third-party email tools or connected sender systems.`,
        ],
      },
      {
        heading: `33. Data Collected Through Cookies`,
        paragraphs: [
          `Cookies and similar technologies may collect or involve:`,
          `The specific data collected depends on the cookie category, provider, feature, configuration, browser, device and applicable settings.`,
        ],
        bullets: [
          `IP address`,
          `device identifiers`,
          `browser type`,
          `operating system`,
          `approximate location`,
          `language settings`,
          `referral URLs`,
          `pages viewed`,
          `links clicked`,
          `session identifiers`,
          `login status`,
          `account status`,
          `workspace interactions`,
          `feature usage`,
          `timestamps`,
          `error data`,
          `diagnostic data`,
          `security signals`,
          `consent status`,
          `campaign source information`,
          `analytics identifiers.`,
        ],
      },
      {
        heading: `34. How Cookie Data Is Used`,
        paragraphs: [
          `Cookie and tracking data may be used to:`,
        ],
        bullets: [
          `operate the platform`,
          `authenticate users`,
          `maintain sessions`,
          `remember preferences`,
          `process billing flows`,
          `prevent fraud`,
          `prevent abuse`,
          `secure accounts`,
          `detect suspicious activity`,
          `improve performance`,
          `diagnose errors`,
          `measure usage`,
          `improve user experience`,
          `manage consent`,
          `analyse product performance`,
          `measure lawful campaign effectiveness`,
          `comply with law`,
          `enforce platform terms`,
          `protect the business and Protected Parties.`,
        ],
      },
      {
        heading: `35. Sharing Cookie Data`,
        paragraphs: [
          `Cookie and tracking data may be shared with or accessed by:`,
          `Where providers process data on our behalf, we use contractual and operational controls appropriate to the service.`,
          `Some third-party providers may act as independent controllers or businesses for certain processing under their own policies.`,
        ],
        bullets: [
          `hosting providers`,
          `analytics providers`,
          `authentication providers`,
          `payment providers`,
          `security providers`,
          `fraud-prevention providers`,
          `monitoring providers`,
          `logging providers`,
          `support providers`,
          `communication providers`,
          `AI providers where relevant to feature operation`,
          `translation providers`,
          `consent-management providers`,
          `professional advisers`,
          `regulators, courts or authorities where required`,
          `corporate transaction parties where lawful.`,
        ],
      },
      {
        heading: `36. International Transfers`,
        paragraphs: [
          `Global Solutions Management LLC is incorporated in Delaware, United States.`,
          `Cookie and tracking data may be processed in the United States, United Kingdom, European Economic Area and other jurisdictions where we or our providers operate.`,
          `Where required by applicable law, we use recognized transfer mechanisms such as adequacy decisions, standard contractual clauses, UK transfer mechanisms, Swiss transfer mechanisms, data privacy frameworks or other lawful safeguards.`,
          `Users should review the Privacy Policy for more information about international transfers.`,
        ],
      },
      {
        heading: `37. Retention`,
        paragraphs: [
          `Cookies may last for different periods depending on their purpose and configuration.`,
          `Some cookies are deleted when a browser session ends. Others may remain for longer to remember preferences, maintain security, record consent, support analytics, detect fraud or support lawful business purposes.`,
          `We retain cookie-related records for as long as reasonably necessary for the purposes described in this Cookie Policy, including security, fraud prevention, consent management, legal compliance, analytics, dispute handling, audit and platform operations.`,
          `Users may delete cookies through browser settings, but deletion may not remove server-side logs, consent records, billing records, security logs, fraud-prevention records, audit logs or records we are legally or operationally permitted to retain.`,
        ],
      },
      {
        heading: `38. Security`,
        paragraphs: [
          `Cookies and similar technologies may help protect Velocity Vision, but no system is completely secure.`,
          `Users are responsible for:`,
          `We are not responsible for security issues caused by customer-side systems, compromised devices, browser extensions, shared devices, weak passwords, unauthorized internal users, unsafe exports, third-party account compromise or user-controlled settings.`,
        ],
        bullets: [
          `using secure devices`,
          `protecting account credentials`,
          `managing browser settings`,
          `avoiding shared devices where inappropriate`,
          `logging out on shared devices`,
          `keeping browsers updated`,
          `protecting connected accounts`,
          `reporting suspected compromise promptly.`,
        ],
      },
      {
        heading: `39. Customer Warranties`,
        paragraphs: [
          `Customers represent and warrant that:`,
          `Velocity Vision relies on these warranties when providing access to the platform.`,
        ],
        bullets: [
          `their own websites and campaigns comply with cookie and tracking laws`,
          `their own consent tools are lawful where required`,
          `their own analytics, pixels and advertising tools are configured lawfully`,
          `their own email tracking is lawful`,
          `their own exports and downstream use are lawful`,
          `their own integrations comply with provider terms`,
          `they will not use Velocity Vision exports, generated outputs, contact records or integrations to bypass privacy, cookie, advertising, marketing or tracking laws`,
          `they will maintain required notices, consent records, opt-out records and suppression records.`,
        ],
      },
      {
        heading: `40. No Legal, Compliance or Technical Guarantee`,
        paragraphs: [
          `This Cookie Policy explains our cookie and tracking practices. It does not guarantee that every cookie, provider, browser, device, integration, consent tool or third-party technology will behave identically in every jurisdiction, browser or technical environment.`,
          `Third-party providers, browser changes, device settings, legal developments, consent-tool changes, integration changes and platform updates may affect how cookies and similar technologies operate.`,
          `Nothing in this Cookie Policy creates a guarantee of cookie compliance for customer websites, customer campaigns, customer tracking, customer analytics, customer pixels, customer consent tools, customer third-party integrations, customer exports or downstream use.`,
        ],
      },
      {
        heading: `41. No Assumption of Customer Duties`,
        paragraphs: [
          `Nothing in this Cookie Policy, the platform, product interface, support, documentation, templates, prompts, generated outputs, activation controls, integrations or privacy reminders causes Global Solutions Management LLC or any Protected Party to assume customer legal, regulatory, contractual, cookie, tracking, analytics, advertising, marketing, privacy, consent, opt-out, pixel, retargeting, custom-audience or downstream-use obligations.`,
          `Customers remain responsible even where Velocity Vision provides tools, settings, warnings, integrations, templates, AI-assisted outputs or operational safeguards.`,
        ],
      },
      {
        heading: `42. Enforcement and Platform Protection`,
        paragraphs: [
          `We may restrict, disable, remove, block or modify cookies, integrations, scripts, tags, pixels, tracking technologies, embedded tools or third-party services where we reasonably believe there is legal, regulatory, security, provider, privacy, payment, operational or reputational risk.`,
          `We may also suspend or restrict accounts, workspaces, integrations or features where customer-controlled tracking, exports, integrations or downstream use creates risk to Velocity Vision, Protected Parties, providers, users, recipients or data subjects.`,
          `Enforcement action does not create any right to a refund, credit, service credit, damages, compensation, chargeback, termination payment or extension unless expressly required by law.`,
        ],
      },
      {
        heading: `43. Legal, Regulatory and Third-Party Requests`,
        paragraphs: [
          `If we receive a subpoena, court order, regulator request, law enforcement request, data subject request, rights-holder complaint, payment-provider request, analytics-provider request, advertising-provider request, security complaint, abuse report or legal notice relating to cookies, tracking technologies, consent records, website use, platform use, customer integrations or customer-controlled tracking, we may respond, disclose information, preserve records, restrict access, notify relevant parties or take other action as we consider legally required or reasonably appropriate.`,
          `We are not required to challenge any legal, regulatory, provider or third-party request unless required by law or expressly agreed in writing.`,
        ],
      },
      {
        heading: `44. Customer Indemnity`,
        paragraphs: [
          `Customers agree to defend, indemnify and hold harmless the Protected Parties from and against all claims, demands, investigations, complaints, proceedings, losses, liabilities, damages, penalties, fines, settlements, costs and expenses, including reasonable legal fees, arising from or related to:`,
          `This indemnity survives suspension, termination, cancellation, account closure and deletion of data.`,
        ],
        bullets: [
          `customer websites`,
          `customer landing pages`,
          `customer campaigns`,
          `customer tracking`,
          `customer cookies`,
          `customer pixels`,
          `customer analytics`,
          `customer retargeting`,
          `customer custom audiences`,
          `customer exports`,
          `customer downstream use`,
          `customer third-party integrations`,
          `customer email tracking`,
          `customer consent tools`,
          `customer failure to provide cookie notices`,
          `customer failure to obtain consent`,
          `customer failure to honour opt-outs`,
          `customer failure to maintain consent records`,
          `customer violation of cookie, tracking, privacy, marketing or advertising laws`,
          `customer breach of third-party provider rules`,
          `customer use of Velocity Vision outputs or exports in third-party tracking systems.`,
        ],
      },
      {
        heading: `45. Relationship with Other Terms`,
        paragraphs: [
          `This Cookie Policy supplements the Platform Terms of Service, Customer Agreement, Privacy Policy, Data Processing Agreement, Acceptable Use Policy, Marketing Compliance Policy, Platform Security Policy and Service Level Agreement.`,
          `If there is any conflict, the stricter or more protective provision applies to the maximum extent permitted by law.`,
          `Nothing in this Cookie Policy limits our rights under any other agreement, policy, law or equitable remedy.`,
        ],
      },
      {
        heading: `46. Local Law and Mandatory Rights`,
        paragraphs: [
          `This Cookie Policy applies to the maximum extent permitted by applicable law.`,
          `If mandatory law in a jurisdiction gives rights that cannot be excluded, restricted or modified, this Cookie Policy does not exclude those rights, but all exclusions, limitations, restrictions, indemnities and protections continue to apply to the maximum extent permitted by that law.`,
          `Where a provision cannot be enforced as written in a particular jurisdiction, it will be interpreted, narrowed or reformed only to the minimum extent necessary to make it enforceable, and the rest of this Cookie Policy remains effective.`,
        ],
      },
      {
        heading: `47. Changes to this Cookie Policy`,
        paragraphs: [
          `We may update this Cookie Policy from time to time to reflect changes in technology, providers, cookies, tracking tools, consent tools, legal requirements, product features, security needs, analytics configuration, advertising configuration, integrations or business operations.`,
          `Material updates may be notified by posting, email, platform notice, consent-tool notice or other reasonable method.`,
          `The version posted on the Velocity Vision website or platform applies from its effective date unless otherwise stated.`,
        ],
      },
      {
        heading: `48. Survival`,
        paragraphs: [
          `Any provision that by its nature should survive suspension, termination, cancellation, account closure or deletion of data will survive, including provisions relating to Protected Parties, no personal claims, customer responsibility, customer warranties, third-party technologies, customer websites, customer integrations, enforcement, legal requests, indemnity, relationship with other terms, local law and records.`,
        ],
      },
      {
        heading: `49. Contact`,
        paragraphs: [
          `For cookie, tracking technology, consent or privacy questions, use the Contact page and select the route that matches your request.`,
          `Global Solutions Management LLC`,
          `Delaware, United States`,
          `Operator of Velocity Vision`,
        ],
      },
    ],
  },
  "platform-security-policy": {
    title: "Platform Security Policy",
    effectiveDate: "30 June 2026",
    lastUpdated: "1 July 2026",
    version: "9.0",
    sections: [
      {
        heading: `1. Purpose`,
        paragraphs: [
          `Velocity Vision is designed to protect customer workspaces, uploaded data, sender controls, activation workflows, replies, pipeline records, billing flows, platform operations, AI-assisted features, integrations and platform integrity using reasonable technical and organizational measures appropriate to a self-serve SaaS platform.`,
          `Security is a shared responsibility.`,
          `Global Solutions Management LLC is responsible for reasonable security measures within the Velocity Vision platform environment under its reasonable control.`,
          `Customers are responsible for their own accounts, users, passwords, devices, browsers, networks, connected email accounts, sender accounts, integrations, exports, downloads, agency users, client workspaces, customer-side systems, internal approvals, data handling, business continuity and compliance processes.`,
        ],
      },
      {
        heading: `2. Delaware Operator and Contract Stack`,
        paragraphs: [
          `Velocity Vision is operated by Global Solutions Management LLC, a Delaware limited liability company.`,
          `The Platform Terms of Service, Customer Agreement, Data Processing Agreement, Privacy Policy and Acceptable Use Policy contain important provisions on customer responsibility, disclaimers, limitations of liability, indemnities, no personal liability, director and staff protection, governing law, customer-controlled processing and enforcement rights.`,
          `This Platform Security Policy is intended to supplement those terms and must be interpreted in the way that gives Global Solutions Management LLC and the Protected Parties the maximum protection permitted by law.`,
        ],
      },
      {
        heading: `3. Protected Parties`,
        paragraphs: [
          `For purposes of this Platform Security Policy, Protected Parties means Global Solutions Management LLC and its present and former owners, members, managers, directors, officers, employees, staff, contractors, consultants, agents, representatives, affiliates, shareholders or equityholders where applicable, licensors, suppliers, service providers, subprocessors, payment providers, infrastructure providers, AI providers, hosting providers, security providers, monitoring providers, analytics providers, email providers, integration providers, professional advisers, successors and assigns.`,
          `To the maximum extent permitted by law, customers and users must not attempt to hold any Protected Party personally responsible for customer-side security failures, customer-side compromise, weak credentials, shared credentials, compromised devices, unsafe exports, customer integrations, customer users, agency client users, connected sender accounts, third-party provider actions, enforcement decisions, platform restrictions, provider restrictions, cyberattacks, incidents outside our reasonable control or customer-controlled security events.`,
          `The Protected Parties are intended beneficiaries of the protections, restrictions, enforcement rights, indemnities, limitations, disclaimers and no-personal-liability provisions in this Platform Security Policy and the wider platform terms.`,
          `Nothing in this Platform Security Policy excludes liability where liability cannot lawfully be excluded.`,
        ],
      },
      {
        heading: `4. No Personal Claims Against Owners, Directors, Managers, Staff or Providers`,
        paragraphs: [
          `To the maximum extent permitted by law, any claim, complaint, demand, dispute, proceeding or allegation relating to platform security, customer data, customer-side compromise, connected accounts, integrations, exports, sender accounts, customer workspaces, agency workspaces, client workspaces, third-party providers, vulnerability reports, security investigations, enforcement decisions, cyber incidents or security events must be brought only against Global Solutions Management LLC where a claim against Velocity Vision is legally permitted, and not personally against any owner, member, manager, director, officer, employee, staff member, contractor, consultant, adviser, shareholder or equityholder where applicable, supplier, service provider, subprocessor, AI provider, hosting provider, payment provider, security provider, monitoring provider, email provider, integration provider or infrastructure provider.`,
          `Customers must not encourage, assist, fund, support, threaten or pursue personal claims against Protected Parties arising from customer-controlled systems, customer credentials, customer exports, customer devices, customer integrations, customer users, customer senders, customer clients, customer data handling, customer instructions, customer-side incidents or customer use of Velocity Vision.`,
          `This section applies to the maximum extent permitted by law in every jurisdiction and does not limit rights or liabilities that cannot legally be excluded.`,
        ],
      },
      {
        heading: `5. Security Standard`,
        paragraphs: [
          `Velocity Vision uses a risk-based security approach appropriate to the nature, size, maturity, architecture, providers, purpose and operational profile of the platform.`,
          `Security measures are designed to reduce risk. They do not eliminate all risk.`,
          `References to security measures in this policy do not guarantee that every measure applies to every feature, plan, customer, workspace, provider, integration, environment or configuration at all times.`,
          `Specific controls may change as the platform, providers, technology, risks, threat landscape, laws and business operations evolve.`,
        ],
      },
      {
        heading: `6. Security Governance`,
        paragraphs: [
          `We may maintain internal security, operational, provider-management, incident-response, access-control, logging, monitoring, support and abuse-prevention processes appropriate to the platform.`,
          `We are not required to publish detailed internal security controls, architecture, provider configurations, incident playbooks, vulnerability details, logs, detection rules, secrets, keys, infrastructure information or other non-public security information where disclosure could weaken security, create risk or expose confidential information.`,
          `Customer agrees that we may limit the security information we provide publicly or in response to customer requests where needed to protect the platform, providers, customers, users, data subjects, recipients, Protected Parties or the business.`,
        ],
      },
      {
        heading: `7. Shared Responsibility`,
        paragraphs: [
          `Security is shared between Velocity Vision and Customer.`,
          `Global Solutions Management LLC is responsible for reasonable security measures for the platform environment under its reasonable control.`,
          `Customer is responsible for:`,
          `No platform feature, warning, support message, audit log, permission setting, quality flag, security prompt, activation gate or failure to warn removes Customer’s responsibility for its own security.`,
        ],
        bullets: [
          `account security`,
          `user access`,
          `administrator permissions`,
          `password security`,
          `authentication settings`,
          `device security`,
          `browser security`,
          `network security`,
          `connected email accounts`,
          `sender accounts`,
          `billing account access`,
          `third-party integrations`,
          `exports`,
          `downloaded files`,
          `client workspaces`,
          `agency users`,
          `customer-side data handling`,
          `customer-side backups`,
          `internal policies`,
          `user training`,
          `legal and compliance obligations`,
          `business continuity planning`,
          `incident notification where required`,
          `security of downstream systems.`,
        ],
      },
      {
        heading: `8. Customer Account Security`,
        paragraphs: [
          `Customers and users must:`,
          `Customers must not share credentials, permit unauthorized access, bypass security controls, use another person’s account, create misleading user identities or allow unapproved users to access workspaces.`,
        ],
        bullets: [
          `use strong passwords`,
          `avoid password reuse`,
          `protect login credentials`,
          `keep account information current`,
          `use secure devices`,
          `keep browsers and operating systems updated`,
          `protect email accounts`,
          `protect connected sender accounts`,
          `restrict administrator access`,
          `invite only authorized users`,
          `remove users who no longer need access`,
          `monitor workspace activity`,
          `review client workspace access`,
          `protect exports and downloaded files`,
          `report suspected compromise promptly.`,
        ],
      },
      {
        heading: `9. Authentication and Verification`,
        paragraphs: [
          `Velocity Vision may use authentication controls, session controls, verification steps, password reset processes, email verification, provider authentication, workspace roles, administrative restrictions, security prompts and account restrictions.`,
          `The availability of particular authentication features may vary by plan, provider, product maturity, configuration, browser, device, workspace and feature.`,
          `Customers are responsible for enabling and using available authentication safeguards appropriate to their risk profile.`,
          `We may require verification, password reset, access review or additional security steps where we reasonably believe there is account, payment, abuse, legal, provider, data, sender or platform risk.`,
        ],
      },
      {
        heading: `10. Access Controls and Permissions`,
        paragraphs: [
          `Velocity Vision may use account-based access controls, workspace roles, permission settings, session controls, audit logs and provider-level controls to help manage access.`,
          `Customer is responsible for configuring user access appropriately and ensuring that each user has only the access needed for their role.`,
          `Customer is responsible for reviewing access after staff changes, client changes, contractor changes, project changes, role changes and suspected compromise.`,
          `Agency customers are responsible for separating client access, preventing cross-client access and removing client users when no longer authorized.`,
        ],
      },
      {
        heading: `11. Administrative Access`,
        paragraphs: [
          `Administrative access to platform systems is intended to be limited to authorized personnel, contractors or providers with a legitimate operational, support, security, billing, compliance, legal, abuse-prevention or maintenance need.`,
          `Administrative access may be used for:`,
          `Where appropriate, administrative access may be subject to internal controls, confidentiality obligations, logging, review or provider-level restrictions.`,
        ],
        bullets: [
          `support`,
          `troubleshooting`,
          `security monitoring`,
          `incident response`,
          `billing operations`,
          `abuse prevention`,
          `legal compliance`,
          `platform maintenance`,
          `provider management`,
          `enforcement of platform terms.`,
        ],
      },
      {
        heading: `12. Customer Workspace Separation`,
        paragraphs: [
          `Velocity Vision is designed to support separate customer workspaces and, where applicable, agency client workspaces.`,
          `Workspace separation is intended to reduce unauthorized access between customers, clients and workspaces.`,
          `Customer remains responsible for user invitations, role assignments, client access, agency access, exports, integrations and internal permission decisions.`,
          `Customer must not attempt to access another customer’s workspace, client workspace, data, reports, senders, replies, billing information, integrations or exports.`,
        ],
      },
      {
        heading: `13. Agency and Client Workspace Security`,
        paragraphs: [
          `Agency customers are responsible for all client workspaces under their account.`,
          `Agencies must:`,
          `Velocity Vision does not contract directly with agency end clients unless expressly agreed in writing.`,
          `The agency account holder is responsible for all security activity, misuse, compromise and access decisions in client workspaces.`,
        ],
        bullets: [
          `invite only authorized users`,
          `maintain client-specific access controls`,
          `prevent cross-client data exposure`,
          `prevent cross-client export misuse`,
          `remove users promptly when access is no longer authorized`,
          `avoid sharing credentials across clients`,
          `ensure client senders are authorized`,
          `protect client data`,
          `maintain client approvals`,
          `report suspected client workspace compromise promptly.`,
        ],
      },
      {
        heading: `14. Data Protection Measures`,
        paragraphs: [
          `Security measures may include, as appropriate:`,
          `Specific controls may change as the platform evolves.`,
          `A description of possible controls does not guarantee that every control applies to every feature, plan, provider, account, workspace, integration or environment at all times.`,
        ],
        bullets: [
          `encrypted transmission`,
          `secure hosting`,
          `permission controls`,
          `workspace access controls`,
          `authentication controls`,
          `operational logging`,
          `security logging`,
          `backup procedures`,
          `monitoring`,
          `vulnerability management`,
          `provider review`,
          `incident response processes`,
          `supplier controls`,
          `access restrictions`,
          `environment separation`,
          `abuse detection`,
          `fraud prevention`,
          `rate limits`,
          `audit records`,
          `account restrictions`,
          `export restrictions`,
          `administrative controls.`,
        ],
      },
      {
        heading: `15. Encryption and Transmission`,
        paragraphs: [
          `Velocity Vision may use encryption, secure transmission or provider-managed security methods where appropriate to protect data in transit.`,
          `Storage encryption, database encryption, provider-managed encryption or other protection measures may depend on hosting providers, infrastructure providers, product architecture, feature configuration and operational requirements.`,
          `Customer is responsible for protecting data after export, download, copy, transfer, synchronisation, screenshot, print, email, integration or use outside Velocity Vision.`,
          `Customer must not assume that exported files, copied outputs, downloaded reports, email attachments, screenshots, printed records or third-party transfers remain protected by Velocity Vision security controls.`,
        ],
      },
      {
        heading: `16. Secrets, Tokens, API Keys and Connected Accounts`,
        paragraphs: [
          `Customers are responsible for securing any passwords, API keys, tokens, credentials, OAuth connections, email accounts, sender accounts, CRM accounts, payment accounts, analytics accounts, advertising accounts, support accounts or other connected services they use with Velocity Vision.`,
          `Customers must not share, expose, publish, hard-code, email, reuse or store credentials insecurely.`,
          `Customer is responsible for revoking, rotating, removing or disabling credentials, tokens and integrations when access is no longer needed or when compromise is suspected.`,
          `We may disconnect integrations, invalidate sessions, restrict access or require re-authentication where we reasonably believe there is risk.`,
        ],
      },
      {
        heading: `17. Logs and Monitoring`,
        paragraphs: [
          `Velocity Vision may maintain platform logs, usage logs, access logs, security logs, billing logs, support logs, activation logs, export logs, integration logs, sender logs, audit records and enforcement records.`,
          `Logs may be used to:`,
          `Platform logs may be retained for as long as reasonably necessary for security, legal, operational, billing, fraud-prevention, compliance, audit, dispute-handling and enforcement purposes.`,
        ],
        bullets: [
          `operate the platform`,
          `investigate issues`,
          `detect abuse`,
          `prevent fraud`,
          `secure accounts`,
          `support billing`,
          `verify acceptance`,
          `investigate incidents`,
          `enforce platform terms`,
          `respond to legal requests`,
          `protect customers, users, recipients, providers and Protected Parties.`,
        ],
      },
      {
        heading: `18. Logs as Evidence`,
        paragraphs: [
          `Customer agrees that platform records, usage logs, billing logs, support records, security logs, audit logs, activation logs, export logs, acceptance logs, sender logs, integration logs, provider logs where available and enforcement records may be used as evidence of activity, access, acceptance, breach, risk, investigation, payment status, activation, export, compliance review, security review or enforcement action.`,
          `Such records may be relied upon to the maximum extent permitted by law.`,
        ],
      },
      {
        heading: `19. Backups and Resilience`,
        paragraphs: [
          `Velocity Vision may use backup, replication, restore or resilience processes appropriate to the platform and providers.`,
          `Backups are intended to support platform resilience and operational recovery. They are not a substitute for Customer’s own backup, recordkeeping, export or business continuity obligations.`,
          `Customer is responsible for maintaining independent copies of important records, exports, customer data, customer reports, legal records, consent records, suppression records, campaign records, accounting records and business-critical data.`,
          `We do not guarantee that all data can be restored, recovered, exported or reconstructed after deletion, corruption, customer error, provider failure, account closure, suspension, termination, cyberattack or incident.`,
        ],
      },
      {
        heading: `20. Change Management and Platform Updates`,
        paragraphs: [
          `We may update, patch, modify, replace, restrict, disable, migrate, reconfigure or remove platform features, integrations, providers, controls, workflows, security settings or technical architecture where reasonably necessary for security, compliance, reliability, provider requirements, legal obligations, abuse prevention, platform integrity or business operations.`,
          `Changes may affect features, exports, integrations, access, workflows, activation, AI tools, sender connections, logs or support processes.`,
          `We are not liable for loss arising from changes made in good faith to protect security, compliance, providers, customers, users, data subjects, recipients, Protected Parties or the business.`,
        ],
      },
      {
        heading: `21. Vulnerability Management`,
        paragraphs: [
          `We may use vulnerability-management processes appropriate to the platform, provider environment, risk profile, product maturity and operational needs.`,
          `This may include monitoring, provider updates, patching, review, remediation, configuration changes, access restriction, feature restriction or other reasonable steps.`,
          `We do not guarantee that all vulnerabilities will be discovered, prevented, remediated immediately or publicly disclosed.`,
          `We may prioritise remediation based on severity, exploitability, impact, provider dependency, customer risk, technical feasibility and operational requirements.`,
        ],
      },
      {
        heading: `22. Sender and Activation Governance`,
        paragraphs: [
          `Velocity Vision may use sender verification, activation gates, quality statuses, risky-record flags, plan limits, cadence controls, account restrictions, export restrictions, compliance prompts, suppression prompts and audit records to reduce operational, security, compliance and deliverability risk.`,
          `These safeguards are operational controls only.`,
          `They do not remove Customer’s responsibility for lawful outreach, customer-side security, sender authority, data accuracy, consent, opt-outs, suppression handling, campaign approvals or compliance with applicable law.`,
          `We may suspend, restrict or disconnect senders, integrations, activation, exports or workspaces where we reasonably believe there is security, abuse, compliance, deliverability, legal, provider or reputational risk.`,
        ],
      },
      {
        heading: `23. AI Security and Generated Outputs`,
        paragraphs: [
          `Velocity Vision may use AI-assisted tools and AI providers to support drafting, classification, segmentation, summarisation, quality review, templates, generated outputs, translations and workflow support.`,
          `Customer must not submit sensitive, regulated, prohibited or restricted data into AI-assisted features unless expressly authorized in writing and lawful for the intended use.`,
          `Customer must review generated outputs before use and must not rely on AI outputs as legal, compliance, security, professional or regulated advice.`,
          `We may restrict AI-assisted features where we reasonably believe there is security, privacy, legal, provider, abuse, data protection, ethical, reputational or operational risk.`,
        ],
      },
      {
        heading: `24. AI-Specific Security Risks`,
        paragraphs: [
          `Customer must not use Velocity Vision or AI-assisted features in a way that creates or increases security risk, including through:`,
          `We may block prompts, restrict AI features, suspend accounts, remove outputs, preserve logs or notify providers where we reasonably believe AI-assisted use creates security, legal, provider or abuse risk.`,
        ],
        bullets: [
          `prompt injection`,
          `malicious prompts`,
          `attempts to reveal system instructions`,
          `attempts to extract secrets`,
          `attempts to bypass safety controls`,
          `attempts to generate malware`,
          `attempts to generate phishing material`,
          `attempts to automate social engineering`,
          `attempts to identify vulnerabilities in third-party systems`,
          `attempts to exfiltrate data`,
          `attempts to bypass access controls`,
          `attempts to generate deceptive impersonation`,
          `attempts to misuse generated outputs for unlawful cyber activity.`,
        ],
      },
      {
        heading: `25. Third-Party Providers and Supply Chain`,
        paragraphs: [
          `Velocity Vision may rely on third-party providers for:`,
          `Third-party providers may have their own security practices, privacy notices, terms, policies, access controls, retention periods, incident processes, outages, suspensions and restrictions.`,
          `We are not responsible for third-party provider failures, outages, compromises, policy changes, account restrictions, data handling or security incidents outside our reasonable control.`,
          `Customer is responsible for complying with the terms and security requirements of any third-party services it connects to or uses with Velocity Vision.`,
        ],
        bullets: [
          `hosting`,
          `cloud infrastructure`,
          `authentication`,
          `payments`,
          `analytics`,
          `monitoring`,
          `logging`,
          `security`,
          `communications`,
          `support`,
          `email connectivity`,
          `AI processing`,
          `translation`,
          `CRM tools`,
          `domain services`,
          `backup`,
          `incident response`,
          `platform operations.`,
        ],
      },
      {
        heading: `26. Customer-Controlled Integrations`,
        paragraphs: [
          `Customers may connect email accounts, CRM systems, analytics tools, advertising tools, payment tools, communication tools, support tools or other third-party services to Velocity Vision.`,
          `Customer is responsible for:`,
          `Velocity Vision does not control customer-selected third-party integrations outside its own platform environment.`,
          `A compromise of a customer integration, customer email account, customer sender account, customer API key, customer token, customer device or customer-side system is Customer’s responsibility unless directly caused by our breach of this Platform Security Policy.`,
        ],
        bullets: [
          `authorizing integrations`,
          `securing connected accounts`,
          `reviewing integration permissions`,
          `removing unused integrations`,
          `protecting API keys`,
          `protecting tokens`,
          `managing connected sender accounts`,
          `complying with provider terms`,
          `monitoring downstream activity`,
          `handling incidents in connected services.`,
        ],
      },
      {
        heading: `27. Exports and Downloads`,
        paragraphs: [
          `Customer is responsible for securing any data exported, downloaded, copied, screenshotted, printed, emailed, transferred, synced, integrated or otherwise removed from Velocity Vision.`,
          `Once data leaves Velocity Vision, Customer is responsible for its storage, access controls, sharing, deletion, retention, security, transfer, downstream use and compliance.`,
          `Customer must not export or download data unless it has the legal right and security controls required for the intended use.`,
          `We may restrict exports where we reasonably believe there is legal, security, privacy, abuse, payment, sanctions, provider, customer, recipient or platform risk.`,
        ],
      },
      {
        heading: `28. Customer-Side Security Incidents`,
        paragraphs: [
          `Customer is responsible for security incidents caused by or relating to:`,
          `Customer must notify us promptly if it suspects any issue that may affect Velocity Vision, customer workspaces, connected integrations, sender accounts, platform security, other customers, recipients, data subjects, providers or Protected Parties.`,
        ],
        bullets: [
          `weak passwords`,
          `password reuse`,
          `shared credentials`,
          `compromised devices`,
          `compromised browsers`,
          `compromised email accounts`,
          `compromised sender accounts`,
          `compromised integrations`,
          `exposed API keys`,
          `exposed tokens`,
          `unsafe exports`,
          `user error`,
          `unauthorized internal users`,
          `agency client misuse`,
          `client workspace misuse`,
          `customer-side malware`,
          `customer-side phishing`,
          `customer-side social engineering`,
          `customer network compromise`,
          `customer misconfiguration`,
          `customer disclosure`,
          `customer failure to remove users`,
          `customer failure to secure downloaded files`,
          `customer failure to manage permissions`,
          `customer failure to follow security prompts`,
          `customer failure to maintain business continuity.`,
        ],
      },
      {
        heading: `29. Business Email Compromise and Social Engineering`,
        paragraphs: [
          `Customer is responsible for preventing, detecting and responding to business email compromise, impersonation, invoice fraud, social engineering, phishing, credential theft, unauthorized sender use and internal misuse affecting Customer’s users, senders, devices, accounts, clients or integrations.`,
          `Velocity Vision is not responsible for losses caused by customer-side business email compromise, fake instructions, compromised accounts, unauthorized payment instructions, user deception, client impersonation, provider impersonation or social engineering unless directly caused by our breach of this Platform Security Policy.`,
        ],
      },
      {
        heading: `30. Incident Response`,
        paragraphs: [
          `If we become aware of a confirmed security incident affecting Customer Personal Data or platform integrity within our reasonable control, we will investigate and take reasonable containment, mitigation and remediation steps.`,
          `Where required by law or contract, we will notify affected customers without undue delay after becoming aware of a confirmed incident affecting Customer Personal Data.`,
          `Notice may include, where available:`,
          `Notification of an incident is not an admission of fault, liability, unlawful conduct or breach.`,
          `Customer remains responsible for determining whether it must notify regulators, data subjects, clients, recipients, insurers, providers or other parties unless the law requires us to notify directly.`,
        ],
        bullets: [
          `nature of the incident`,
          `affected systems`,
          `affected data categories`,
          `likely consequences`,
          `mitigation steps`,
          `recommended customer actions`,
          `contact route for follow-up.`,
        ],
      },
      {
        heading: `31. Incident Exclusions`,
        paragraphs: [
          `The following are not security incidents caused by Global Solutions Management LLC unless directly caused by our breach of this Platform Security Policy:`,
          `Customer remains responsible for incidents arising from customer-side systems, users, instructions, exports, integrations, sender accounts and data handling.`,
        ],
        bullets: [
          `customer-side credential compromise`,
          `customer-side device compromise`,
          `customer email account compromise`,
          `customer sender account compromise`,
          `customer integration compromise`,
          `exposed customer API keys`,
          `exposed customer tokens`,
          `customer-side malware`,
          `customer-side phishing`,
          `customer-side social engineering`,
          `customer user error`,
          `customer administrator error`,
          `customer client error`,
          `customer agency error`,
          `unsafe exports`,
          `unauthorized internal users`,
          `compromised third-party tools selected by Customer`,
          `data disclosed by Customer`,
          `data shared by Customer`,
          `customer misconfiguration`,
          `breach caused by Customer instructions`,
          `provider incidents outside our reasonable control`,
          `browser or device compromise`,
          `network compromise outside our reasonable control.`,
        ],
      },
      {
        heading: `32. Vulnerability Reporting`,
        paragraphs: [
          `Customers, users, researchers and third parties must report security concerns responsibly.`,
          `Security concerns should be submitted through the Contact page using the route that matches the request.`,
          `Reports should include enough detail to understand and investigate the issue, such as affected feature, steps to reproduce, risk level, screenshots where appropriate and contact information.`,
          `Submitting a report does not create any entitlement to payment, bounty, reward, public credit, confidentiality exception, immunity or continued platform access.`,
        ],
      },
      {
        heading: `33. Prohibited Security Testing`,
        paragraphs: [
          `Unless expressly authorized in writing by Global Solutions Management LLC, you must not:`,
          `Unauthorized security testing may result in suspension, termination, legal action, provider notification or law enforcement notification where appropriate.`,
        ],
        bullets: [
          `conduct intrusive testing`,
          `conduct vulnerability scanning`,
          `conduct penetration testing`,
          `exploit vulnerabilities`,
          `access data that is not yours`,
          `access another customer’s workspace`,
          `attempt privilege escalation`,
          `bypass authentication`,
          `bypass rate limits`,
          `bypass security controls`,
          `perform denial-of-service testing`,
          `perform load testing`,
          `perform spam testing`,
          `perform social engineering`,
          `phish users or staff`,
          `test physical security`,
          `test third-party providers`,
          `deploy malware`,
          `exfiltrate data`,
          `modify data`,
          `destroy data`,
          `publish vulnerability details before remediation.`,
        ],
      },
      {
        heading: `34. Responsible Disclosure Conditions`,
        paragraphs: [
          `Where we choose to review a vulnerability report, the reporter must:`,
          `We may decline to engage with reports that are abusive, extortionate, threatening, duplicative, low-quality, generated by prohibited testing, unrelated to Velocity Vision or outside the platform scope.`,
        ],
        bullets: [
          `act in good faith`,
          `avoid privacy violations`,
          `avoid service disruption`,
          `avoid data destruction`,
          `avoid accessing other customers’ data`,
          `avoid persistence`,
          `avoid public disclosure before written permission`,
          `stop testing when asked`,
          `preserve confidentiality`,
          `comply with law`,
          `comply with this Platform Security Policy.`,
        ],
      },
      {
        heading: `35. Security Research Safe Boundaries`,
        paragraphs: [
          `Any security research must be limited to accounts, workspaces and data that the researcher owns or has express written authorization to test.`,
          `Researchers must not access, copy, retain, publish, transmit, alter, delete or disclose other users’ data, customer data, platform secrets, source code, credentials, tokens, keys, logs, non-public information or provider information.`,
          `If a researcher accidentally accesses data that is not theirs, they must stop immediately, preserve confidentiality and report the issue without copying, storing, sharing or using the data.`,
        ],
      },
      {
        heading: `36. Abuse, Fraud and Platform Protection`,
        paragraphs: [
          `We may monitor, investigate, restrict or take action against activity that may involve:`,
          `No monitoring, automated review, manual review, warning, flag, hold or failure to act makes us responsible for customer-side security, customer data, customer outputs, customer outreach, customer integrations or customer legal obligations.`,
        ],
        bullets: [
          `account takeover`,
          `credential abuse`,
          `fraud`,
          `payment abuse`,
          `phishing`,
          `spam`,
          `malware`,
          `scraping`,
          `rate-limit abuse`,
          `denial-of-service activity`,
          `unauthorized access`,
          `unsafe exports`,
          `suspicious integrations`,
          `suspicious sender activity`,
          `suspicious client workspace activity`,
          `policy evasion`,
          `sanctions risk`,
          `provider risk`,
          `legal risk.`,
        ],
      },
      {
        heading: `37. Enforcement Rights`,
        paragraphs: [
          `If we believe there is security, legal, regulatory, privacy, abuse, payment, sanctions, provider, AI, deliverability, reputational, operational or platform risk, we may take any action we consider appropriate, including:`,
          `We may act immediately and without prior notice where needed to protect Velocity Vision, customers, users, recipients, data subjects, infrastructure, third-party providers, payment systems, AI systems, sender systems, legal compliance, platform integrity, Protected Parties or our business.`,
        ],
        bullets: [
          `blocking access`,
          `requiring password reset`,
          `requiring verification`,
          `restricting users`,
          `restricting exports`,
          `restricting activation`,
          `disconnecting integrations`,
          `disconnecting senders`,
          `disabling API access`,
          `quarantining files`,
          `deleting harmful files`,
          `suspending workspaces`,
          `suspending accounts`,
          `terminating accounts`,
          `preserving records`,
          `notifying providers`,
          `notifying customers`,
          `notifying affected parties`,
          `reporting unlawful activity where appropriate`,
          `refusing future service.`,
        ],
      },
      {
        heading: `38. No Refund for Security Enforcement`,
        paragraphs: [
          `Security enforcement action does not create any right to a refund, credit, service credit, damages, compensation, chargeback, termination payment or extension unless expressly required by law.`,
          `Customer remains responsible for fees, taxes, usage charges, credit usage, chargebacks, indemnity obligations, confidentiality obligations, data compliance obligations, acceptable use obligations, marketing compliance obligations and security obligations already incurred.`,
        ],
      },
      {
        heading: `39. No Absolute Security Warranty`,
        paragraphs: [
          `No internet service, cloud service, SaaS platform, AI service, email integration, payment integration, hosting provider, browser, device, authentication process or software platform can guarantee absolute security.`,
          `We do not warrant that:`,
          `Security measures are designed to reduce risk, not eliminate all risk.`,
        ],
        bullets: [
          `unauthorized access can never occur`,
          `data loss can never occur`,
          `vulnerabilities can never exist`,
          `outages can never occur`,
          `attacks can never occur`,
          `provider failures can never occur`,
          `human error can never occur`,
          `customer-side compromise can never occur`,
          `integrations can never fail`,
          `exports can never be misused`,
          `all incidents can be prevented`,
          `all data can be recovered`,
          `all threats will be detected`,
          `all vulnerabilities will be fixed immediately`,
          `all security controls will function without interruption`,
          `all provider controls will remain unchanged`,
          `all cyberattacks will be blocked.`,
        ],
      },
      {
        heading: `40. No Security, Compliance, Certification or Business Continuity Guarantee`,
        paragraphs: [
          `Velocity Vision does not guarantee that the platform, controls, prompts, logs, backups, integrations, sender controls, security measures, provider controls or incident processes will satisfy every customer requirement, regulator expectation, industry standard, audit requirement, certification requirement, procurement requirement, insurance requirement or local-law requirement.`,
          `Unless expressly stated in a separate signed written agreement, Velocity Vision does not represent that it holds any particular certification, attestation, audit report, penetration-test report, security standard certification, SOC report, ISO certification, Cyber Essentials certification, industry accreditation or regulatory approval.`,
          `Customer is responsible for determining whether Velocity Vision is appropriate for its data, industry, sector, jurisdiction, client obligations, insurance obligations, business continuity requirements and risk appetite.`,
          `Customer should maintain its own backups, security controls, compliance processes, incident response plans, insurance, business continuity plans and disaster recovery arrangements.`,
        ],
      },
      {
        heading: `41. Service Availability and Security Events`,
        paragraphs: [
          `Security events, provider events, abuse events, maintenance, emergency fixes, third-party restrictions, legal requirements or suspected compromise may require temporary or permanent changes to platform availability, access, features, integrations, exports, sender connections, workspaces or support.`,
          `We may suspend, throttle, restrict, disable, reroute, modify or terminate features or access where reasonably necessary to protect security, availability, compliance, providers, customers, users, data subjects, recipients or Protected Parties.`,
          `Such action does not create a refund or service-credit right unless expressly required by law or expressly stated in a separate signed written agreement accepted by Global Solutions Management LLC.`,
        ],
      },
      {
        heading: `42. Legal, Regulatory and Third-Party Requests`,
        paragraphs: [
          `If we receive a subpoena, court order, regulator request, law enforcement request, data subject request, rights-holder complaint, payment-provider request, email-provider request, AI-provider request, hosting-provider request, security complaint, abuse report or legal notice relating to platform security, customer data, customer integrations, customer users, customer exports, customer workspaces, vulnerability reports or security events, we may respond, disclose information, preserve records, restrict access, notify relevant parties or take other action as we consider legally required or reasonably appropriate.`,
          `We are not required to challenge any legal, regulatory, provider or third-party request unless required by law or expressly agreed in writing.`,
        ],
      },
      {
        heading: `43. Customer Warranties`,
        paragraphs: [
          `Customer represents and warrants that:`,
          `Velocity Vision relies on these warranties when providing access to the platform.`,
        ],
        bullets: [
          `it will use reasonable security measures for its own accounts and users`,
          `its users are authorized`,
          `its administrators are authorized`,
          `its connected integrations are authorized`,
          `its sender accounts are authorized`,
          `its payment methods are authorized`,
          `its exports are lawful and secure`,
          `its client workspace access is authorized`,
          `its agency users are authorized`,
          `it will promptly remove unauthorized users`,
          `it will promptly report suspected compromise`,
          `it will not bypass platform security controls`,
          `it will not conduct unauthorized testing`,
          `it will not access other customers’ data`,
          `it will not use Velocity Vision for unlawful cyber activity`,
          `it will comply with applicable security, privacy, marketing and data protection laws.`,
        ],
      },
      {
        heading: `44. Customer Indemnity`,
        paragraphs: [
          `Customer agrees to defend, indemnify and hold harmless the Protected Parties from and against all claims, demands, investigations, complaints, proceedings, losses, liabilities, damages, penalties, fines, settlements, costs and expenses, including reasonable legal fees, arising from or related to:`,
          `We may control the defence of any matter subject to indemnity. Customer must cooperate with us and must not settle any claim in a way that imposes obligations, admissions, restrictions or liability on any Protected Party without prior written consent.`,
          `This indemnity survives suspension, termination, cancellation, account closure and deletion of data.`,
        ],
        bullets: [
          `customer-side security failures`,
          `weak credentials`,
          `shared credentials`,
          `compromised devices`,
          `compromised browsers`,
          `compromised email accounts`,
          `compromised sender accounts`,
          `compromised integrations`,
          `exposed API keys`,
          `exposed tokens`,
          `unsafe exports`,
          `customer-side malware`,
          `customer-side phishing`,
          `customer-side social engineering`,
          `business email compromise`,
          `unauthorized internal users`,
          `agency client misuse`,
          `client workspace misuse`,
          `customer misconfiguration`,
          `customer disclosure`,
          `customer failure to remove users`,
          `customer failure to manage permissions`,
          `customer failure to protect exports`,
          `customer unauthorized testing`,
          `customer misuse of AI features`,
          `customer cyber misuse`,
          `customer breach of this Platform Security Policy`,
          `customer breach of law`,
          `customer breach of third-party provider rules`,
          `customer data handling`,
          `customer instructions`,
          `customer connected systems`,
          `customer business continuity failures`,
          `customer incident notification failures`,
          `customer provider account compromise`,
          `customer downstream systems.`,
        ],
      },
      {
        heading: `45. Relationship with Other Terms`,
        paragraphs: [
          `This Platform Security Policy supplements the Platform Terms of Service, Customer Agreement, Data Processing Agreement, Privacy Policy, Acceptable Use Policy, Marketing Compliance Policy, Cookie Policy and Service Level Agreement.`,
          `If there is any conflict, the stricter or more protective provision applies to the maximum extent permitted by law.`,
          `Nothing in this Platform Security Policy limits our rights under any other agreement, policy, law or equitable remedy.`,
        ],
      },
      {
        heading: `46. Local Law and Mandatory Rights`,
        paragraphs: [
          `This Platform Security Policy applies to the maximum extent permitted by applicable law.`,
          `If mandatory law in a jurisdiction gives rights that cannot be excluded, restricted or modified, this Platform Security Policy does not exclude those rights, but all exclusions, limitations, restrictions, indemnities and protections continue to apply to the maximum extent permitted by that law.`,
          `Where a provision cannot be enforced as written in a particular jurisdiction, it will be interpreted, narrowed or reformed only to the minimum extent necessary to make it enforceable, and the rest of this Platform Security Policy remains effective.`,
        ],
      },
      {
        heading: `47. Changes to this Platform Security Policy`,
        paragraphs: [
          `We may update this Platform Security Policy from time to time to reflect changes in technology, providers, security controls, legal requirements, product features, security needs, infrastructure, integrations, threat patterns, abuse patterns or business operations.`,
          `Material updates may be notified by posting, email, platform notice or other reasonable method.`,
          `The version posted on the Velocity Vision website or platform applies from its effective date unless otherwise stated.`,
        ],
      },
      {
        heading: `48. Survival`,
        paragraphs: [
          `Any provision that by its nature should survive suspension, termination, cancellation, account closure or deletion of data will survive, including provisions relating to Protected Parties, no personal claims, customer responsibility, shared responsibility, customer warranties, prohibited testing, enforcement, legal requests, logs as evidence, indemnity, relationship with other terms, local law and records.`,
        ],
      },
      {
        heading: `49. Contact`,
        paragraphs: [
          `For security reports, vulnerability reports, suspected compromise or platform security questions, use the Contact page and select the route that matches your request.`,
          `Global Solutions Management LLC`,
          `Delaware, United States`,
          `Operator of Velocity Vision`,
        ],
      },
    ],
  },
  "service-level-agreement": {
    title: "Service Level Agreement",
    effectiveDate: "30 June 2026",
    lastUpdated: "1 July 2026",
    version: "9.0",
    sections: [
      {
        heading: "1. Purpose",
        paragraphs: [
          `This Service Level Agreement describes operational service targets for ${PLATFORM}. It is not a guarantee of uninterrupted access, continuous availability, error-free operation, support resolution time, restoration time, data recovery, AI-output availability, third-party provider performance, deliverability, cyber resilience or business outcome.`,
          `This Service Level Agreement forms part of the Platform Terms of Service, Customer Agreement, Data Processing Agreement, Privacy Policy, Acceptable Use Policy, Marketing Compliance Policy, Cookie Policy, Platform Security Policy and all other incorporated platform terms.`,
          `This document may be displayed in your browser's preferred language using an automated machine-translation layer, including GTranslate, Google Translate or similar translation technology. Translations are provided for convenience only. The English version controls if there is any conflict, ambiguity, inconsistency, error or difference between versions.`,
          `${PLATFORM} is a self-serve, AI-powered commercial workspace.`,
          `This Service Level Agreement sets out service targets, support targets, exclusions, customer responsibilities, service-credit restrictions and liability protections that apply to paid production workspaces unless a separate signed written agreement expressly states otherwise.`,
          `This Service Level Agreement is intended to protect the availability, stability, security, integrity and commercial operation of ${PLATFORM} while recognizing that SaaS platforms depend on customer systems, third-party providers, internet infrastructure, payment providers, email providers, AI providers, hosting providers, integrations, browsers, devices and external networks.`,
        ],
      },
      {
        heading: "2. Delaware Operator and Contract Stack",
        paragraphs: [
          `${PLATFORM} is operated by Global Solutions Management LLC, a Delaware limited liability company.`,
          `The Platform Terms of Service, Customer Agreement, Data Processing Agreement, Privacy Policy, Acceptable Use Policy, Marketing Compliance Policy, Cookie Policy and Platform Security Policy contain important provisions on customer responsibility, disclaimers, limitations of liability, indemnities, no personal liability, director and staff protection, governing law, customer-controlled processing, security responsibility and enforcement rights.`,
          `This Service Level Agreement supplements those terms and must be interpreted in the way that gives Global Solutions Management LLC and the Protected Parties the maximum protection permitted by law.`,
        ],
      },
      {
        heading: "3. Protected Parties",
        paragraphs: [
          `For purposes of this Service Level Agreement, Protected Parties means Global Solutions Management LLC and its present and former owners, members, managers, directors, officers, employees, staff, contractors, consultants, agents, representatives, affiliates, shareholders or equityholders where applicable, beneficial owners where applicable, licensors, suppliers, service providers, subprocessors, payment providers, infrastructure providers, AI providers, hosting providers, security providers, monitoring providers, analytics providers, email providers, integration providers, professional advisers, successors and assigns.`,
          `To the maximum extent permitted by law, customers and users must not attempt to hold any Protected Party personally responsible for downtime, support delays, feature changes, provider outages, customer-side issues, emergency maintenance, security actions, enforcement actions, data export issues, data recovery issues, AI feature interruptions, email provider issues, payment provider issues, integration failures, internet failures, maintenance, force majeure events, cyberattacks, service-level performance or any excluded event.`,
          `The Protected Parties are intended beneficiaries of the protections, restrictions, enforcement rights, indemnities, limitations, disclaimers and no-personal-liability provisions in this Service Level Agreement and the wider platform terms.`,
          `Nothing in this Service Level Agreement excludes liability where liability cannot lawfully be excluded.`,
        ],
      },
      {
        heading: "4. No Personal Claims Against Owners, Directors, Managers, Staff or Providers",
        paragraphs: [
          `To the maximum extent permitted by law, any claim, complaint, demand, dispute, proceeding or allegation relating to availability, uptime, downtime, support, response time, maintenance, outages, provider failures, platform restrictions, emergency security actions, service levels, service credits, data access, data export, data recovery, billing access, AI feature access, integrations or service performance must be brought only against Global Solutions Management LLC where a claim against ${PLATFORM} is legally permitted, and not personally against any owner, member, manager, director, officer, employee, staff member, contractor, consultant, adviser, shareholder or equityholder where applicable, beneficial owner where applicable, supplier, service provider, subprocessor, AI provider, hosting provider, payment provider, security provider, monitoring provider, email provider, integration provider or infrastructure provider.`,
          `Customers must not encourage, assist, fund, support, threaten or pursue personal claims against Protected Parties arising from service availability, support timing, customer-controlled systems, customer credentials, customer exports, customer integrations, customer users, customer senders, customer clients, customer instructions, customer-side incidents, provider events, enforcement actions or customer use of ${PLATFORM}.`,
          `This section applies to the maximum extent permitted by law in every jurisdiction and does not limit rights or liabilities that cannot legally be excluded.`,
        ],
      },
      {
        heading: "5. Nature of Service Targets",
        paragraphs: [
          `All availability, maintenance and support commitments in this Service Level Agreement are service targets only unless a separate signed written agreement expressly states that a binding service credit applies.`,
          `Service targets are not warranties, guarantees, conditions, representations, insurance commitments, business continuity commitments, disaster recovery commitments, data recovery commitments, deliverability commitments, support-resolution commitments, uptime guarantees, cyber-resilience guarantees or operational performance guarantees.`,
          `Failure to meet a target does not create any right to damages, refunds, chargebacks, credits, service credits, termination payments, extensions, compensation, fee offsets or other remedies unless expressly required by law or expressly provided in a separate signed written agreement accepted by Global Solutions Management LLC.`,
        ],
      },
      {
        heading: "6. Eligibility",
        paragraphs: [
          `This Service Level Agreement applies only to paid production workspaces that are in good standing.`,
          `A workspace is not in good standing if:`,
        ],
        bullets: [
          `fees are unpaid;`,
          `payment has failed;`,
          `a chargeback has been initiated;`,
          `billing information is inaccurate;`,
          `the account is under review;`,
          `the account is suspended;`,
          `the account is restricted;`,
          `the account is in breach of platform terms;`,
          `the workspace is being misused;`,
          `the workspace is non-production;`,
          `the workspace is free, trial, seeded, demo, beta, QA, test, founder preview, discounted or promotional unless expressly stated in a separate signed written agreement.`,
          `Customers not in good standing are not eligible for service targets, support targets or service credits unless expressly required by law.`,
        ],
      },
      {
        heading: "7. Covered Service",
        paragraphs: [
          `Unless expressly stated otherwise, any availability target applies only to paid production workspace access to the core ${PLATFORM} platform.`,
          `Core platform access means the ability of an authorized paid production user to access the primary workspace interface of ${PLATFORM}, excluding all exclusions in this Service Level Agreement.`,
          `Core platform access does not include:`,
        ],
        bullets: [
          `beta features;`,
          `preview features;`,
          `free features;`,
          `trial features;`,
          `seeded demo workspaces;`,
          `QA seed environments;`,
          `experimental tools;`,
          `discontinued features;`,
          `AI provider availability;`,
          `generated output quality;`,
          `email provider availability;`,
          `payment provider availability;`,
          `CRM availability;`,
          `third-party integrations;`,
          `exports;`,
          `imports;`,
          `support tools;`,
          `analytics tools;`,
          `marketing tools;`,
          `external websites;`,
          `customer systems;`,
          `customer networks;`,
          `customer devices;`,
          `customer browsers;`,
          `customer sender accounts;`,
          `customer email accounts;`,
          `customer domains;`,
          `customer DNS;`,
          `customer data quality;`,
          `customer configuration;`,
          `customer misuse;`,
          `customer-side security incidents.`,
        ],
      },
      {
        heading: "8. Availability Target",
        paragraphs: [
          `For eligible paid production workspaces, ${PLATFORM} aims to maintain 99% monthly availability for core workspace access.`,
          `This is a target only unless a separate signed written agreement expressly provides otherwise.`,
          `Availability is measured over a calendar month and excludes all excluded events, unavailable periods outside our reasonable control and any period that does not qualify as downtime under this Service Level Agreement.`,
        ],
      },
      {
        heading: "9. Availability Formula",
        paragraphs: [
          `Where availability is calculated, the monthly availability percentage may be calculated as:`,
          `Monthly Availability = 100% minus the percentage of total minutes in the calendar month during which qualifying downtime occurred.`,
          `Only qualifying downtime counts against the availability target.`,
          `Excluded events, scheduled maintenance, emergency maintenance, customer-side issues, third-party issues, provider issues, degraded performance, feature-level interruptions and unavailable periods outside our reasonable control are excluded from the calculation.`,
        ],
      },
      {
        heading: "10. Downtime Definition",
        paragraphs: [
          `For purposes of this Service Level Agreement, downtime means a material inability of eligible paid production users to access the core ${PLATFORM} workspace interface due solely to a failure of the ${PLATFORM} platform environment under the reasonable control of Global Solutions Management LLC.`,
          `Downtime does not include partial degradation, slow performance, feature-level interruption, third-party provider issue, customer-specific issue, user-specific issue, integration issue, email provider issue, AI provider issue, payment issue, export issue, import issue, customer data issue, customer browser issue, customer network issue, customer device issue, maintenance, security action, enforcement action or any excluded event.`,
          `Downtime must be measured using records reasonably available to Global Solutions Management LLC.`,
        ],
      },
      {
        heading: "11. Measurement and Evidence",
        paragraphs: [
          `Availability and downtime may be measured using platform logs, monitoring tools, hosting provider records, error logs, incident records, support records, status records or other records reasonably available to Global Solutions Management LLC.`,
          `Customer-side monitoring, third-party monitoring, screenshots, individual user reports, external tools or user assertions are not conclusive unless accepted by Global Solutions Management LLC in writing.`,
          `Availability calculations may exclude periods where the issue cannot be reproduced, verified or attributed solely to the ${PLATFORM} platform environment under our reasonable control.`,
          `${PLATFORM} records, provider records where available and platform monitoring records may be relied upon to determine whether downtime occurred and whether an exclusion applies.`,
        ],
      },
      {
        heading: "12. Scheduled Maintenance",
        paragraphs: [
          `We may perform scheduled maintenance to improve performance, security, reliability, scalability, compliance, provider compatibility, integrations, data integrity or functionality.`,
          `Where practical, we may provide advance notice of scheduled maintenance by platform notice, email, website notice, status notice or another reasonable method.`,
          `Scheduled maintenance is excluded from availability calculations and does not create any right to refund, credit, service credit, damages, compensation, chargeback, termination payment or extension.`,
          `Failure to provide advance notice of scheduled maintenance does not convert scheduled maintenance into qualifying downtime where the maintenance was reasonably necessary.`,
        ],
      },
      {
        heading: "13. Emergency Maintenance",
        paragraphs: [
          `We may perform emergency maintenance without advance notice where we reasonably believe action is needed for security, reliability, compliance, provider requirements, legal requirements, abuse prevention, data protection, payment integrity, platform integrity, cyber-risk mitigation, system stability or business continuity.`,
          `Emergency maintenance is excluded from availability calculations and does not create any right to refund, credit, service credit, damages, compensation, chargeback, termination payment or extension.`,
        ],
      },
      {
        heading: "14. Security, Abuse and Enforcement Actions",
        paragraphs: [
          `We may suspend, restrict, throttle, disable, disconnect, block, quarantine, modify, reroute, degrade, limit or terminate access, features, integrations, senders, exports, activation, workspaces or accounts where we reasonably believe there is security, legal, regulatory, privacy, payment, sanctions, abuse, AI, deliverability, provider, reputational, operational or platform risk.`,
          `Security, abuse and enforcement actions are excluded from availability calculations.`,
          `No security, abuse or enforcement action creates any right to refund, credit, service credit, damages, compensation, chargeback, termination payment or extension unless expressly required by law.`,
        ],
      },
      {
        heading: "15. Beta, Preview and Experimental Features",
        paragraphs: [`Availability and support targets do not apply to:`],
        bullets: [
          `beta features;`,
          `preview features;`,
          `early-access features;`,
          `seeded demo features;`,
          `QA seed environments;`,
          `test environments;`,
          `experimental tools;`,
          `prototype tools;`,
          `temporary tools;`,
          `discontinued tools;`,
          `free tools;`,
          `trial tools;`,
          `pilot features;`,
          `roadmap features;`,
          `features labelled as preview, beta, experimental, test, demo, QA, seeded, founder preview or similar.`,
          `Such features may be changed, withdrawn, restricted, suspended or discontinued at any time without liability.`,
        ],
      },
      {
        heading: "16. AI Feature Exclusions",
        paragraphs: [
          `${PLATFORM} may rely on AI providers for AI-assisted drafting, summaries, reports, classifications, templates, translations, segmentation, recommendations and generated outputs.`,
          `Availability targets do not apply to:`,
        ],
        bullets: [
          `AI provider outages;`,
          `AI provider degradation;`,
          `AI provider rate limits;`,
          `AI provider restrictions;`,
          `AI provider policy changes;`,
          `AI model changes;`,
          `AI latency;`,
          `AI output quality;`,
          `AI hallucinations;`,
          `AI refusals;`,
          `AI safety blocks;`,
          `AI-generated output errors;`,
          `AI feature restrictions;`,
          `AI usage limits;`,
          `AI credit consumption;`,
          `AI prompt failures;`,
          `AI moderation;`,
          `AI content filtering;`,
          `AI provider account actions;`,
          `AI safety or compliance controls.`,
          `AI-generated outputs are drafts only and are not subject to service-level guarantees.`,
        ],
      },
      {
        heading: "17. Third-Party Provider Exclusions",
        paragraphs: [`Availability and support targets do not apply to issues caused by, related to or dependent on third-party providers, including:`],
        bullets: [
          `hosting providers;`,
          `cloud infrastructure providers;`,
          `AI providers;`,
          `payment processors;`,
          `email providers;`,
          `DNS providers;`,
          `domain providers;`,
          `authentication providers;`,
          `analytics providers;`,
          `security providers;`,
          `monitoring providers;`,
          `logging providers;`,
          `communications providers;`,
          `support providers;`,
          `CRM providers;`,
          `translation providers;`,
          `browser providers;`,
          `operating system providers;`,
          `app stores;`,
          `telecom providers;`,
          `internet service providers;`,
          `external APIs;`,
          `customer-selected integrations;`,
          `provider networks;`,
          `provider accounts;`,
          `provider policies;`,
          `provider maintenance;`,
          `provider suspensions;`,
          `provider security events.`,
          `We are not responsible for third-party provider outages, degradation, restrictions, policy changes, suspensions, account actions, data handling, security incidents, maintenance, service changes or discontinuation outside our reasonable control.`,
        ],
      },
      {
        heading: "18. Customer-Side Exclusions",
        paragraphs: [`Availability and support targets do not apply to issues caused by, related to or dependent on:`],
        bullets: [
          `customer networks;`,
          `customer devices;`,
          `customer browsers;`,
          `customer operating systems;`,
          `customer firewalls;`,
          `customer VPNs;`,
          `customer security tools;`,
          `customer email accounts;`,
          `customer sender accounts;`,
          `customer domains;`,
          `customer DNS;`,
          `customer payment methods;`,
          `customer billing failures;`,
          `customer integrations;`,
          `customer data errors;`,
          `customer imports;`,
          `customer exports;`,
          `customer configuration;`,
          `customer misuse;`,
          `customer administrator actions;`,
          `customer user actions;`,
          `customer client users;`,
          `customer agency users;`,
          `customer permissions;`,
          `customer credentials;`,
          `customer-side security incidents;`,
          `customer-side business continuity failures;`,
          `customer violation of platform terms.`,
          `Customer-side issues are excluded from availability calculations and do not create any right to refund, credit, service credit, damages, compensation, chargeback, termination payment or extension.`,
        ],
      },
      {
        heading: "19. Cybersecurity Exclusions",
        paragraphs: [`Availability and support targets do not apply to issues caused by, related to or dependent on:`],
        bullets: [
          `cyberattacks;`,
          `denial-of-service attacks;`,
          `distributed denial-of-service attacks;`,
          `credential stuffing;`,
          `brute-force attacks;`,
          `malware;`,
          `ransomware;`,
          `phishing;`,
          `social engineering;`,
          `business email compromise;`,
          `bot activity;`,
          `scraping;`,
          `account takeover;`,
          `unauthorized access attempts;`,
          `suspicious activity;`,
          `security investigations;`,
          `emergency security restrictions;`,
          `provider security events;`,
          `customer-side compromise;`,
          `customer-side credential compromise;`,
          `customer-side device compromise;`,
          `customer-side integration compromise;`,
          `customer-side email compromise;`,
          `customer-side sender compromise.`,
          `We may restrict, suspend, throttle, disable or modify platform access or features where reasonably necessary to protect the platform, customers, users, recipients, data subjects, providers, Protected Parties or the business.`,
        ],
      },
      {
        heading: "20. Data, Import and Export Exclusions",
        paragraphs: [`Availability and support targets do not apply to:`],
        bullets: [
          `data quality issues;`,
          `duplicate records;`,
          `incorrect fields;`,
          `customer mapping errors;`,
          `upload errors caused by customer files;`,
          `malformed files;`,
          `unsupported file types;`,
          `oversized files;`,
          `corrupted files;`,
          `customer import choices;`,
          `customer export choices;`,
          `customer deletion actions;`,
          `customer overwrites;`,
          `customer-side storage;`,
          `downstream use after export;`,
          `third-party tool imports;`,
          `third-party tool exports;`,
          `customer failure to keep backups;`,
          `customer failure to maintain independent records.`,
          `Customers are responsible for validating data before upload, reviewing outputs before use and protecting data after export.`,
        ],
      },
      {
        heading: "21. Payment, Billing and Plan Exclusions",
        paragraphs: [`Availability and support targets do not apply to account restrictions, workspace restrictions, access interruptions, feature restrictions or support limitations caused by:`],
        bullets: [
          `failed payment;`,
          `expired payment method;`,
          `chargeback;`,
          `payment dispute;`,
          `billing verification;`,
          `suspected fraud;`,
          `unpaid fees;`,
          `tax issues;`,
          `plan downgrade;`,
          `plan cancellation;`,
          `plan limits;`,
          `credit exhaustion;`,
          `account review;`,
          `payment provider restriction;`,
          `banking issue;`,
          `card network issue;`,
          `currency or tax issue.`,
          `We may restrict or suspend access for payment, billing, fraud or chargeback risk without creating any service-level remedy.`,
        ],
      },
      {
        heading: "22. Force Majeure and External Service Events",
        paragraphs: [`Availability and support targets do not apply to events beyond our reasonable control, including:`],
        bullets: [
          `acts of God;`,
          `natural disasters;`,
          `extreme weather;`,
          `fire;`,
          `flood;`,
          `epidemic;`,
          `pandemic;`,
          `war;`,
          `terrorism;`,
          `civil unrest;`,
          `labor dispute;`,
          `utility failure;`,
          `internet failure;`,
          `telecom failure;`,
          `data centre failure;`,
          `cloud provider failure;`,
          `DNS failure;`,
          `cyberattack;`,
          `denial-of-service attack;`,
          `malware;`,
          `ransomware;`,
          `provider outage;`,
          `government action;`,
          `court order;`,
          `regulatory action;`,
          `sanctions;`,
          `export control restriction;`,
          `legal change;`,
          `payment network issue;`,
          `banking issue;`,
          `supplier failure;`,
          `software dependency failure;`,
          `hardware failure outside our reasonable control;`,
          `any other event beyond our reasonable control.`,
          `Such events are excluded from availability calculations and service targets.`,
        ],
      },
      {
        heading: "23. No Deliverability or Outreach Service Level",
        paragraphs: [`${PLATFORM} does not provide any service level for:`],
        bullets: [
          `email deliverability;`,
          `inbox placement;`,
          `reply rates;`,
          `open rates;`,
          `click rates;`,
          `bounce rates;`,
          `spam classification;`,
          `blacklist status;`,
          `sender reputation;`,
          `domain reputation;`,
          `recipient response;`,
          `sales;`,
          `revenue;`,
          `leads;`,
          `meetings;`,
          `conversions;`,
          `campaign performance;`,
          `compliance outcome.`,
          `Customers are responsible for sender setup, lawful basis, message quality, recipient selection, suppression, opt-outs, sender reputation, domain reputation and compliance with marketing laws.`,
        ],
      },
      {
        heading: "24. No Data Recovery, RTO or RPO Guarantee",
        paragraphs: [
          `${PLATFORM} may maintain backups or resilience processes for platform operations, but does not guarantee that all data can be restored, recovered, exported or reconstructed after deletion, corruption, customer error, provider failure, account closure, suspension, termination, cyberattack or incident.`,
          `Unless expressly agreed in a separate signed written agreement, ${PLATFORM} does not provide any guaranteed recovery time objective, recovery point objective, backup frequency, restoration window, archive availability, data reconstruction commitment or disaster recovery commitment.`,
          `Customers are responsible for maintaining independent copies of important records, exports, customer data, customer reports, legal records, consent records, suppression records, campaign records, accounting records and business-critical data.`,
        ],
      },
      {
        heading: "25. No Support Resolution Guarantee",
        paragraphs: [
          `Support response targets are targets for initial response only.`,
          `They are not guarantees of resolution, fix time, restoration time, workaround availability, escalation time, root-cause analysis, provider response, data recovery, feature change, custom development, legal answer, compliance advice, security advice, cyber remediation, deliverability advice or business outcome.`,
          `Resolution may depend on customer cooperation, issue complexity, provider availability, logs, reproduction steps, plan level, severity, security risk, legal risk, third-party systems and operational workload.`,
        ],
      },
      {
        heading: "26. Support Channels",
        paragraphs: [
          `Support is provided through designated support channels made available by ${PLATFORM}.`,
          `Support requests submitted through unofficial routes, personal accounts, social media, informal messages or non-designated channels may not be treated as support tickets.`,
          `Customers must submit sufficient information for investigation, including:`,
        ],
        bullets: [
          `account details;`,
          `workspace details;`,
          `affected feature;`,
          `timestamps;`,
          `screenshots where appropriate;`,
          `reproduction steps;`,
          `error messages;`,
          `affected users;`,
          `browser and device information;`,
          `integration details;`,
          `severity description;`,
          `business impact.`,
          `Incomplete requests may delay response or may not qualify for support targets.`,
        ],
      },
      {
        heading: "27. Business Days and Business Hours",
        paragraphs: [
          `Unless a separate signed written agreement states otherwise, support targets are measured in business days.`,
          `A business day means Monday to Friday, excluding public holidays and company closure days in the United States, United Kingdom or any jurisdiction where support operations or relevant providers are materially affected.`,
          `Support may not be available during weekends, public holidays, company closure days, maintenance periods, provider outages or force majeure events unless expressly agreed in writing.`,
        ],
      },
      {
        heading: "28. Support Severity Levels",
        paragraphs: [
          `${PLATFORM} may classify support requests by severity.`,
          `Severity classification is determined by Global Solutions Management LLC acting reasonably, based on available information.`,
          `Customer's requested severity is not binding.`,
          `Severity 1 — Critical Platform Access Issue`,
          `A verified issue affecting core workspace access for multiple paid production customers and materially preventing access to the core platform.`,
          `Target initial response: one business day.`,
          `Severity 2 — Major Paid Workspace Issue`,
          `A verified issue affecting a material feature of a paid production workspace where a reasonable workaround is not available.`,
          `Target initial response: two business days.`,
          `Severity 3 — Standard Product, Billing or Workspace Support`,
          `General product, billing, account, workspace, feature, configuration or usage support.`,
          `Target initial response: three business days.`,
          `Severity 4 — Guidance, Data, Export, Feature or Non-Critical Requests`,
          `Feature guidance, exports, data questions, non-critical issues, how-to requests, enhancement requests, roadmap questions, template questions, AI-output questions or general inquiries.`,
          `Handled in ordinary support queues without a guaranteed response time.`,
        ],
      },
      {
        heading: "29. Critical Issue Requirements",
        paragraphs: [`A support request will not qualify as a Severity 1 issue unless:`],
        bullets: [
          `it affects paid production access;`,
          `it affects multiple paid production customers or the core platform materially;`,
          `it can be reasonably verified by ${PLATFORM};`,
          `it is not caused by customer-side systems;`,
          `it is not caused by third-party providers outside our reasonable control;`,
          `it is not excluded by this Service Level Agreement;`,
          `sufficient information is provided for investigation.`,
        ],
      },
      {
        heading: "30. Support Exclusions",
        paragraphs: [`Support targets do not apply to:`],
        bullets: [
          `custom development;`,
          `custom integrations;`,
          `legal advice;`,
          `privacy advice;`,
          `marketing compliance advice;`,
          `tax advice;`,
          `accounting advice;`,
          `deliverability consulting;`,
          `campaign strategy;`,
          `sender reputation remediation;`,
          `data cleansing;`,
          `manual data repair;`,
          `customer data entry;`,
          `customer training outside ordinary support;`,
          `business consulting;`,
          `implementation services;`,
          `agency services;`,
          `client-management services;`,
          `professional services;`,
          `unsupported browsers;`,
          `unsupported devices;`,
          `unsupported integrations;`,
          `third-party tools;`,
          `customer-side incidents;`,
          `cybersecurity remediation for customer systems;`,
          `incident response for customer-side compromise.`,
        ],
      },
      {
        heading: "31. Customer Responsibilities",
        paragraphs: [`Customers must:`],
        bullets: [
          `maintain accurate account information;`,
          `maintain billing information;`,
          `use supported browsers;`,
          `secure credentials;`,
          `protect connected accounts;`,
          `protect sender accounts;`,
          `protect devices;`,
          `protect exports;`,
          `maintain backups;`,
          `report issues promptly;`,
          `provide sufficient details;`,
          `cooperate with troubleshooting;`,
          `follow platform policies;`,
          `comply with provider terms;`,
          `avoid misuse;`,
          `avoid actions that create avoidable incidents;`,
          `avoid actions that create deliverability risk;`,
          `avoid actions that create security risk;`,
          `maintain business continuity plans;`,
          `maintain internal approvals;`,
          `maintain lawful data and outreach practices.`,
          `Failure to meet customer responsibilities may exclude the issue from service targets and may delay or prevent support.`,
        ],
      },
      {
        heading: "32. Customer Cooperation",
        paragraphs: [
          `Customer must cooperate with reasonable troubleshooting, investigation, verification, security, billing, provider, legal and compliance requests.`,
          `This may include providing logs, screenshots, timestamps, reproduction steps, affected-user details, integration details, browser details, data samples, payment details, sender details or other information reasonably needed to investigate.`,
          `Failure to cooperate may suspend support obligations, exclude the issue from service targets or result in enforcement action where appropriate.`,
        ],
      },
      {
        heading: "33. Status Updates",
        paragraphs: [
          `Where practical, we may provide status updates during material incidents.`,
          `Status updates may be provided through platform notices, email, status page, support ticket, website notice or another reasonable method.`,
          `Status updates are informational only and do not create admissions of fault, liability, breach, downtime, security incident, negligence, causation or service-credit entitlement.`,
        ],
      },
      {
        heading: "34. Incident Communications",
        paragraphs: [
          `If a material platform incident occurs, we may investigate and communicate with affected customers where appropriate.`,
          `Incident communications may describe known facts, affected services, mitigation steps, workarounds or recommended customer actions.`,
          `Incident communications are not legal notices unless expressly stated.`,
          `Notification of an incident is not an admission of fault, liability, unlawful conduct, breach, downtime, security incident, negligence, causation or service-level failure.`,
        ],
      },
      {
        heading: "35. Root Cause Analysis",
        paragraphs: [
          `We are not required to provide root-cause analysis, post-incident reports, forensic reports, provider reports, internal logs, security details, architecture details, remediation reports or audit materials unless expressly required by law or agreed in a separate signed written agreement.`,
          `Where we choose to provide incident summaries, they may be limited to avoid disclosing confidential, security-sensitive, provider-sensitive, privileged or commercially sensitive information.`,
        ],
      },
      {
        heading: "36. Service Credits",
        paragraphs: [
          `No service credits apply unless a separate signed written order form, enterprise agreement or addendum expressly states that service credits apply.`,
          `If service credits are expressly provided, they are Customer's sole and exclusive remedy for the specific service-level failure covered by that signed written agreement.`,
          `Unless expressly required by law or expressly agreed in writing:`,
        ],
        bullets: [
          `service credits are not refunds;`,
          `service credits are not cash payments;`,
          `service credits are not transferable;`,
          `service credits may only be applied against future fees;`,
          `service credits cannot exceed the amount expressly stated in the signed agreement;`,
          `service credits do not apply to excluded events;`,
          `service credits do not apply to free, trial, beta, preview or non-production features;`,
          `service credits do not apply where Customer is in breach of platform terms;`,
          `service credits do not apply where fees are unpaid or disputed;`,
          `service credits do not apply where Customer is not in good standing;`,
          `service credits do not apply where Customer caused or contributed to the issue;`,
          `service credits do not extend subscription terms unless expressly agreed.`,
        ],
      },
      {
        heading: "37. Service Credit Claim Process",
        paragraphs: [
          `Where a signed written agreement expressly provides service credits, Customer must submit a written claim through the designated support route within 30 days after the end of the month in which the alleged qualifying downtime occurred, unless the signed written agreement states a different period.`,
          `The claim must include:`,
        ],
        bullets: [
          `account details;`,
          `workspace details;`,
          `dates and times of alleged downtime;`,
          `affected users;`,
          `affected services;`,
          `evidence of impact;`,
          `steps taken to verify the issue;`,
          `confirmation that no exclusion applies.`,
          `Failure to submit a timely and complete claim waives any right to service credits to the maximum extent permitted by law.`,
        ],
      },
      {
        heading: "38. Maximum Remedy",
        paragraphs: [
          `Unless expressly required by law or expressly stated in a separate signed written agreement, Customer's sole remedy for unavailability, delay, degradation, non-performance, maintenance, support delay, provider issue, service-level failure or operational interruption is continued use of the platform after restoration.`,
          `Where service credits are expressly agreed, those credits are the sole and exclusive remedy for the specific covered failure.`,
          `Nothing in this Service Level Agreement expands liability beyond the limits in the Platform Terms of Service, Customer Agreement or other applicable terms.`,
        ],
      },
      {
        heading: "39. No Refund, Chargeback, Offset or Compensation Right",
        paragraphs: [
          `Service issues, support delays, maintenance, outages, exclusions, third-party provider issues, AI feature issues, customer-side issues, enforcement actions, payment restrictions, beta feature issues, cyber events or force majeure events do not create any right to refund, credit, service credit, damages, compensation, chargeback, fee offset, withholding, termination payment or extension unless expressly required by law or expressly agreed in a separate signed written agreement.`,
          `Customers must not initiate chargebacks, payment disputes, fee offsets or withholding based on excluded events, target failures, third-party provider issues, support delays or non-guaranteed service targets.`,
        ],
      },
      {
        heading: "40. Changes to Features and Services",
        paragraphs: [
          `We may add, modify, replace, suspend, restrict, downgrade, rename, migrate, discontinue or remove features, integrations, providers, tools, workflows, templates, AI features, exports, activation paths, support processes, technical architecture or security controls where reasonably necessary for product development, security, compliance, reliability, provider requirements, legal obligations, abuse prevention, business operations or platform integrity.`,
          `Feature changes do not create any right to refund, credit, service credit, damages, compensation, termination payment or extension unless expressly required by law.`,
        ],
      },
      {
        heading: "41. Trial, Free, Founder, Promotional and Discounted Plans",
        paragraphs: [
          `Availability and support targets do not apply to free, trial, founder, promotional, discounted, seeded, demo, QA, preview, beta or non-production access unless expressly stated in a separate signed written agreement.`,
          `We may restrict, suspend, modify or discontinue such access at any time without liability.`,
        ],
      },
      {
        heading: "42. Enterprise or Custom Agreements",
        paragraphs: [
          `If a separate signed written enterprise agreement, order form, statement of work or addendum expressly provides different service levels, support terms or service credits, those specific signed terms apply only to the customer, service, workspace, period and scope expressly covered by that agreement.`,
          `No sales material, website copy, pricing page, proposal, support message, chat, demo, roadmap statement, status update or informal communication creates binding service levels or service credits unless included in a signed written agreement accepted by Global Solutions Management LLC.`,
        ],
      },
      {
        heading: "43. No Professional Services Commitment",
        paragraphs: [
          `This Service Level Agreement does not include implementation services, agency services, managed outreach, compliance review, legal review, data review, custom development, custom integrations, custom reporting, training, onboarding, migration assistance, incident response services or professional services unless expressly agreed in a separate signed written statement of work.`,
          `Support does not include legal, tax, accounting, financial, privacy, security, compliance, marketing, deliverability, employment, investment, immigration, cyber insurance, forensic or professional advice.`,
        ],
      },
      {
        heading: "44. Records and Evidence",
        paragraphs: [
          `Platform records, usage logs, billing logs, support records, security logs, audit logs, activation logs, export logs, acceptance logs, sender logs, integration logs, monitoring records, provider records where available and enforcement records may be used as evidence of activity, access, acceptance, breach, risk, investigation, payment status, activation, export, compliance review, security review, support handling, availability calculation or enforcement action.`,
          `Such records may be relied upon to the maximum extent permitted by law.`,
        ],
      },
      {
        heading: "45. Customer Warranties",
        paragraphs: [`Customer represents and warrants that:`],
        bullets: [
          `it will use ${PLATFORM} in accordance with platform terms;`,
          `its users are authorized;`,
          `its account information is accurate;`,
          `its billing information is accurate;`,
          `its payment method is authorized;`,
          `its connected integrations are authorized;`,
          `its sender accounts are authorized;`,
          `its data is lawful;`,
          `its exports are lawful and secure;`,
          `it will provide accurate support information;`,
          `it will cooperate with troubleshooting;`,
          `it will not misuse support;`,
          `it will not create avoidable incidents;`,
          `it will not abuse service targets;`,
          `it will not use service issues as a basis for improper chargebacks;`,
          `it will maintain its own backups and business continuity plans;`,
          `it will maintain appropriate customer-side security controls;`,
          `it will comply with third-party provider terms.`,
          `${PLATFORM} relies on these warranties when providing access to the platform.`,
        ],
      },
      {
        heading: "46. Customer Indemnity",
        paragraphs: [`Customer agrees to defend, indemnify and hold harmless the Protected Parties from and against all claims, demands, investigations, complaints, proceedings, losses, liabilities, damages, penalties, fines, settlements, costs and expenses, including reasonable legal fees, arising from or related to:`],
        bullets: [
          `customer-side systems;`,
          `customer networks;`,
          `customer devices;`,
          `customer browsers;`,
          `customer integrations;`,
          `customer sender accounts;`,
          `customer payment issues;`,
          `customer billing disputes;`,
          `customer data errors;`,
          `customer imports;`,
          `customer exports;`,
          `customer configuration;`,
          `customer misuse;`,
          `customer support misuse;`,
          `customer-side security incidents;`,
          `customer-side cyber incidents;`,
          `business email compromise;`,
          `customer failure to maintain backups;`,
          `customer failure to maintain business continuity;`,
          `customer failure to cooperate;`,
          `customer breach of this Service Level Agreement;`,
          `customer breach of platform terms;`,
          `customer breach of law;`,
          `customer breach of third-party provider rules;`,
          `improper chargebacks;`,
          `improper fee offsets;`,
          `improper withholding;`,
          `claims by customer clients or agency end clients;`,
          `downstream use after export;`,
          `customer contribution to downtime;`,
          `customer contribution to support delays;`,
          `customer contribution to security events.`,
          `We may control the defence of any matter subject to indemnity. Customer must cooperate with us and must not settle any claim in a way that imposes obligations, admissions, restrictions or liability on any Protected Party without prior written consent.`,
          `This indemnity survives suspension, termination, cancellation, account closure and deletion of data.`,
        ],
      },
      {
        heading: "47. Relationship with Other Terms",
        paragraphs: [
          `This Service Level Agreement supplements the Platform Terms of Service, Customer Agreement, Data Processing Agreement, Privacy Policy, Acceptable Use Policy, Marketing Compliance Policy, Cookie Policy and Platform Security Policy.`,
          `If there is any conflict, the stricter or more protective provision applies to the maximum extent permitted by law.`,
          `Nothing in this Service Level Agreement limits our rights under any other agreement, policy, law or equitable remedy.`,
        ],
      },
      {
        heading: "48. Local Law and Mandatory Rights",
        paragraphs: [
          `This Service Level Agreement applies to the maximum extent permitted by applicable law.`,
          `If mandatory law in a jurisdiction gives rights that cannot be excluded, restricted or modified, this Service Level Agreement does not exclude those rights, but all exclusions, limitations, restrictions, indemnities and protections continue to apply to the maximum extent permitted by that law.`,
          `Where a provision cannot be enforced as written in a particular jurisdiction, it will be interpreted, narrowed or reformed only to the minimum extent necessary to make it enforceable, and the rest of this Service Level Agreement remains effective.`,
        ],
      },
      {
        heading: "49. Changes to this Service Level Agreement",
        paragraphs: [
          `We may update this Service Level Agreement from time to time to reflect changes in technology, providers, service targets, support processes, legal requirements, product features, infrastructure, integrations, threat patterns, abuse patterns, business operations or platform maturity.`,
          `Material updates may be notified by posting, email, platform notice or other reasonable method.`,
          `The version posted on the ${PLATFORM} website or platform applies from its effective date unless otherwise stated.`,
        ],
      },
      {
        heading: "50. Survival",
        paragraphs: [
          `Any provision that by its nature should survive suspension, termination, cancellation, account closure or deletion of data will survive, including provisions relating to Protected Parties, no personal claims, customer responsibility, exclusions, service credits, no refund, no chargeback, no offset, customer warranties, legal requests, records as evidence, indemnity, relationship with other terms, local law and records.`,
        ],
      },
      {
        heading: "51. Contact",
        paragraphs: [
          `For service level, support, availability, maintenance, incident or policy questions, ${CONTACT_CTA}`,
          `Global Solutions Management LLC, Delaware, United States. Operator of ${PLATFORM}.`,
        ],
      },
    ],
  },
  "subprocessors": {
    title: "Subprocessor List",
    effectiveDate: "1 July 2026",
    lastUpdated: "1 July 2026",
    version: "1.0",
    sections: [
      {
        heading: "1. Purpose",
        paragraphs: [
          `This Subprocessor List identifies third-party providers that ${PLATFORM} may use to operate the platform, host customer workspaces, process payments, deliver AI-assisted features, provide analytics, monitor errors, support customers or otherwise support the operation of the ${PLATFORM} service.`,
          `${PLATFORM} is operated by Global Solutions Management LLC, a Delaware limited liability company.`,
          `This document is published for transparency. It supplements the Data Processing Agreement, Privacy Policy and Platform Security Policy and does not modify or replace any of them.`,
          `This document may be displayed in your browser's preferred language using an automated machine-translation layer, including GTranslate, Google Translate or similar translation technology. Translations are provided for convenience only. The English version controls if there is any conflict, ambiguity, inconsistency, error or difference between versions.`,
        ],
      },
      {
        heading: "2. Use of Subprocessors",
        paragraphs: [
          `${PLATFORM} may engage subprocessors and third-party providers to host, run, secure, monitor, support, bill, translate and improve the platform.`,
          `Subprocessors may process limited operational, workspace, account, billing, security, telemetry or customer-provided data as reasonably required to deliver the platform in accordance with the Data Processing Agreement, Privacy Policy and Platform Security Policy.`,
          `Subprocessors may change from time to time as providers, features, integrations, regions, security controls, compliance requirements or platform architecture evolve.`,
          `Customers should check this list periodically for updates.`,
        ],
      },
      {
        heading: "3. Current Subprocessor Categories",
        paragraphs: [
          `The categories below reflect the operational providers ${PLATFORM} may rely on. Specific provider entities, locations and product editions may change from time to time. Provider details are maintained as platform providers are finalised and updated.`,
        ],
        bullets: [
          `Hosting and cloud infrastructure — used to host the ${PLATFORM} application, database, storage, edge functions and workspace environments.`,
          `Database, storage and authentication platform — used to store workspace data, manage user authentication and enforce access controls.`,
          `Payments and billing — used to process paid plans, subscriptions, credits, top-ups, tax handling and invoicing.`,
          `AI processing — used to power AI-assisted drafting, summarisation, classification, translation, segmentation and other Generated Outputs. AI providers process customer prompts and data only as needed to return outputs and are contractually restricted from training public models on customer personal data unless expressly permitted.`,
          `Email delivery and connectivity — used to send transactional messages and, where customers connect their own sender accounts, to route customer outreach through customer-authorized providers.`,
          `Translation layer — used to render the public site and legal documents in the visitor's preferred language for convenience. The English version controls.`,
          `Analytics and product telemetry — used, where enabled and consented, to understand aggregate product usage, performance and stability.`,
          `Error monitoring and logging — used to detect, diagnose and resolve platform errors, security events and reliability issues.`,
          `Customer support tooling — used to receive, triage, respond to and resolve support requests routed through the Contact page and in-product surfaces.`,
        ],
      },
      {
        heading: "4. Provider Details",
        paragraphs: [
          `Provider details, including specific provider entities, processing regions and links to provider privacy and security pages, are maintained as platform providers are finalised.`,
          `Where a provider publishes a privacy notice, security page, trust centre or subprocessor list of its own, customers should refer to that provider's published information for authoritative details about that provider's processing, security controls, sub-processing, transfers and certifications.`,
          `${PLATFORM} does not warrant, certify or guarantee any specific provider commitment, certification, region, data residency, security posture or compliance status beyond what is expressly stated in the Data Processing Agreement or a signed written agreement.`,
        ],
      },
      {
        heading: "5. Changes to Subprocessors",
        paragraphs: [
          `${PLATFORM} may add, remove, replace, migrate or reorganize subprocessors from time to time to improve reliability, security, compliance, performance, cost, provider fit, feature coverage, geographic coverage or business continuity.`,
          `Where the Data Processing Agreement requires notice of changes to subprocessors that process customer personal data, notice will be provided in accordance with that agreement.`,
          `Continued use of ${PLATFORM} after an update to this list constitutes acknowledgement of the updated list to the extent permitted by law and the Data Processing Agreement.`,
        ],
      },
      {
        heading: "6. Customer Responsibilities",
        paragraphs: [
          `Customers are responsible for reviewing this list and the Data Processing Agreement before uploading customer personal data, connecting integrations, activating outreach or otherwise using the platform.`,
          `Customers must ensure that their own use of subprocessors and third-party providers, including any providers the customer chooses to connect, is lawful and consistent with the Data Processing Agreement, Privacy Policy, Acceptable Use Policy, Marketing Compliance Policy and applicable law.`,
        ],
      },
      {
        heading: "7. No Personal Claims",
        paragraphs: [
          `To the maximum extent permitted by law, any claim, complaint, demand, dispute, proceeding or allegation relating to a subprocessor, third-party provider, provider outage, provider policy, provider security event, provider decision, provider region or provider certification must be brought only against Global Solutions Management LLC where a claim against ${PLATFORM} is legally permitted, and not personally against any owner, member, manager, director, officer, employee, staff member, contractor, consultant, adviser, supplier, service provider, subprocessor, AI provider, hosting provider, payment provider, security provider, monitoring provider, email provider, integration provider or infrastructure provider.`,
          `Nothing in this Subprocessor List excludes liability where liability cannot lawfully be excluded.`,
        ],
      },
      {
        heading: "8. Contact",
        paragraphs: [
          `For subprocessor questions, ${CONTACT_CTA}`,
          `Global Solutions Management LLC, Delaware, United States. Operator of ${PLATFORM}.`,
        ],
      },
    ],
  },
};

const slugify = (heading: string) =>
  heading.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/(^-|-$)/g, "");

const LegalDocumentPage = () => {
  const { slug } = useParams<{ slug: string }>();
  const doc = slug ? docs[slug] : null;
  const [activeSection, setActiveSection] = useState("");
  const [openSections, setOpenSections] = useState<Set<number>>(new Set());
  const sectionRefs = useRef<(HTMLElement | null)[]>([]);

  useEffect(() => {
    if (doc) {
      setOpenSections(new Set(doc.sections.map((_, i) => i)));
    }
  }, [slug, doc]);

  useEffect(() => {
    if (!doc) return;
    const observer = new IntersectionObserver(
      (entries) => {
        for (const entry of entries) {
          if (entry.isIntersecting) {
            setActiveSection(entry.target.id);
          }
        }
      },
      { rootMargin: "-80px 0px -60% 0px", threshold: 0.1 }
    );
    sectionRefs.current.forEach((ref) => {
      if (ref) observer.observe(ref);
    });
    return () => observer.disconnect();
  }, [doc, openSections]);

  const toggleSection = (i: number) => {
    const next = new Set(openSections);
    next.has(i) ? next.delete(i) : next.add(i);
    setOpenSections(next);
  };

  if (!doc) {
    return (
      <div className="min-h-screen bg-background">
        <Navbar />
        <main className="pt-24 pb-20 text-center">
          <p className="text-muted-foreground">Document not found.</p>
          <Link to="/legal" className="text-accent underline mt-4 inline-block">Back to Legal Centre</Link>
        </main>
        <Footer />
      </div>
    );
  }

  return (
    <div className="min-h-screen bg-background">
      <Helmet>
        <title>{`${doc.title} — Velocity Vision Legal Centre`}</title>
        <meta name="description" content={`${doc.title} for the Velocity Vision platform, operated by Global Solutions Management LLC.`} />
      </Helmet>
      <Navbar />
      <main className="pt-24">
        <section className="section-padding bg-hero">
          <div className="max-w-6xl mx-auto">
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
              <Link to="/legal" className="inline-flex items-center gap-1.5 text-sm text-primary-foreground/70 hover:text-accent transition-colors mb-6">
                <ArrowLeft size={14} /> Back to Legal Centre
              </Link>
              <p className="text-accent font-semibold text-sm uppercase tracking-widest mb-3">Legal document</p>
              <h1 className="text-3xl md:text-5xl font-display font-bold text-primary-foreground">{doc.title}</h1>
              <div className="flex flex-wrap gap-x-6 gap-y-1 mt-4">
                <p className="text-sm text-primary-foreground/70"><span className="font-medium text-primary-foreground/90">Effective Date:</span> {doc.effectiveDate}</p>
                <p className="text-sm text-primary-foreground/70"><span className="font-medium text-primary-foreground/90">Last Updated:</span> {doc.lastUpdated}</p>
                <p className="text-sm text-primary-foreground/70"><span className="font-medium text-primary-foreground/90">Version:</span> {doc.version}</p>
              </div>
              <p className="text-xs text-primary-foreground/60 mt-3">Global Solutions Management LLC — Delaware, United States — operator of Velocity Vision</p>
              <p className="text-xs text-primary-foreground/60 mt-1 max-w-3xl">These terms govern access to and use of Velocity Vision from their stated effective date.</p>
              <p className="text-xs text-primary-foreground/60 mt-1 max-w-3xl italic">
                This document may be displayed in your browser's preferred language using an automated
                machine-translation layer (GTranslate / Google Translate). Translations are provided for
                convenience only. The English version controls if there is any conflict between translations.
              </p>
            </motion.div>
          </div>
        </section>
        <div className="max-w-6xl mx-auto px-6 lg:px-8 py-16">

          <div className="flex gap-10">
            <aside className="hidden lg:block w-64 shrink-0">
              <div className="sticky top-24">
                <p className="text-xs font-semibold uppercase tracking-wider text-muted-foreground/60 mb-3">Table of Contents</p>
                <nav className="space-y-0.5 border-l border-border/50">
                  {doc.sections.map((s, i) => {
                    const id = slugify(s.heading);
                    const isActive = activeSection === id;
                    return (
                      <a
                        key={i}
                        href={`#${id}`}
                        onClick={(e) => {
                          e.preventDefault();
                          if (!openSections.has(i)) {
                            const next = new Set(openSections);
                            next.add(i);
                            setOpenSections(next);
                          }
                          setTimeout(() => {
                            document.getElementById(id)?.scrollIntoView({ behavior: "smooth", block: "start" });
                          }, 100);
                        }}
                        className={cn(
                          "block pl-4 py-1.5 text-xs leading-snug border-l-2 -ml-px transition-colors",
                          isActive ? "border-accent text-accent font-medium" : "border-transparent text-muted-foreground hover:text-foreground hover:border-border"
                        )}
                      >
                        {s.heading}
                      </a>
                    );
                  })}
                </nav>
              </div>
            </aside>

            <div className="flex-1 min-w-0 max-w-[760px]">
              <div className="space-y-2">
                {doc.sections.map((s, i) => {
                  const id = slugify(s.heading);
                  const isOpen = openSections.has(i);
                  return (
                    <Collapsible key={i} open={isOpen} onOpenChange={() => toggleSection(i)}>
                      <div ref={(el) => { sectionRefs.current[i] = el; }} id={id} className="scroll-mt-24">
                        <CollapsibleTrigger asChild>
                          <button className="w-full flex items-center gap-2 py-3 text-left group hover:text-accent transition-colors border-b border-border/30">
                            {isOpen ? <ChevronDown size={16} className="text-muted-foreground shrink-0" /> : <ChevronRight size={16} className="text-muted-foreground shrink-0" />}
                            <h2 className="text-base lg:text-lg font-display font-semibold text-foreground group-hover:text-accent transition-colors">{s.heading}</h2>
                          </button>
                        </CollapsibleTrigger>
                        <CollapsibleContent>
                          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="pt-3 pb-5 pl-6 space-y-3">
                            {s.paragraphs.map((p, pi) => (
                              <p key={pi} className="text-sm text-foreground/75 leading-relaxed whitespace-pre-line">{p}</p>
                            ))}
                            {s.bullets && (
                              <ul className="space-y-1.5 pl-1">
                                {s.bullets.map((b, bi) => (
                                  <li key={bi} className="flex gap-2 text-sm text-foreground/75 leading-relaxed">
                                    <span className="text-accent mt-1 shrink-0">•</span>
                                    <span>{b}</span>
                                  </li>
                                ))}
                              </ul>
                            )}
                          </motion.div>
                        </CollapsibleContent>
                      </div>
                    </Collapsible>
                  );
                })}
              </div>

              <div className="mt-16 pt-8 border-t border-border/50">
                <div className="flex items-start gap-3 mb-4">
                  <Building2 size={18} className="text-accent shrink-0 mt-0.5" />
                  <div>
                    <p className="text-sm font-semibold text-foreground">Velocity Vision</p>
                    <p className="text-xs text-muted-foreground">Operated by Global Solutions Management LLC</p>
                    <p className="text-xs text-muted-foreground">Delaware, United States</p>
                  </div>
                </div>
                <Link
                  to="/contact"
                  className="inline-flex items-center gap-2 px-4 py-2 rounded-md bg-accent text-accent-foreground text-xs font-semibold hover:bg-accent/90 transition-colors"
                >
                  Use the Contact page →
                </Link>
              </div>
            </div>
          </div>
        </div>
      </main>
      <Footer />
    </div>
  );
};

export default LegalDocumentPage;
