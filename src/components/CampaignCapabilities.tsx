...
const cards = [
  ...
  { icon: Video, title: "Video content drafts", desc: "Prepare editable scripts, hooks, shot-list ideas, storyboard outlines and captions for customer review." },
  { icon: Inbox, title: "Reply Intent Command Centre", desc: "Triage replies by intent — interested, referral, out of office, not now, unsubscribe, bounce — with unsubscribe and bounce wording always taking precedence over a sales-positive label." },
  { icon: Users, title: "Referrals and out-of-office dates", desc: "Surface a referred contact for your review and record a detected return date, plus a queue of replies waiting more than 24 hours." },
  { icon: Send, title: "Meeting handoff", desc: "Move an interested reply into handoff with your own booking link and record the meeting as booked when it is confirmed. No calendar connection." },
  { icon: GitBranch, title: "Early pipeline records", desc: "Move customer-selected warm contacts into early opportunity records and track stage, value and next action without promising an outcome." },
  { icon: BarChart3, title: "Outcome Funnel and activity summary", desc: "See Contacted → Replied → Interested/Referral → Meeting booked → Opportunity → Won from your stored records. Reports describe workspace activity and do not guarantee performance." },
  { icon: Users, title: "Agency workspaces", desc: "Use one agency account with isolated client workspaces, pooled Campaign Credits, cross-client outcome visibility and account-wide send-usage visibility." },
];
    ...
    <div className="max-w-7xl mx-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {cards.map((card, index) => (
          <motion.div key={card.title} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.4, delay: (index % 6) * 0.05 }} className="bg-card border border-border/50 rounded-xl p-6 shadow-card hover:border-accent/40 transition-colors">
            <div className="w-10 h-10 rounded-lg bg-accent/10 flex items-center justify-center mb-4"><card.icon className="text-accent" size={20} /></div>
            <h3 className="font-display font-semibold text-foreground mb-2">{card.title}</h3>
            <p className="text-muted-foreground text-sm md:text-base leading-relaxed">{card.desc}</p>
          </motion.div>
        ))}
      </div>
    </div>
