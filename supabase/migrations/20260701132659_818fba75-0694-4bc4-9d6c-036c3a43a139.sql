...
BEGIN
  ...
  DELETE FROM public.contacts
    WHERE workspace_id = qa_ws
       OR company_id = qa_company
       OR (email IS NOT NULL AND email ILIKE '%@example.%');
  DELETE FROM public.companies
    WHERE id <> qa_company AND (workspace_id = qa_ws OR name ILIKE '% Example %' OR name ILIKE '%Example Ltd' OR name ILIKE '%Example Group' OR name ILIKE '%Example Services' OR name ILIKE '%Example Partners' OR name ILIKE '%Example Studio' OR name ILIKE '%Example Systems' OR name ILIKE '%Example Co' OR name ILIKE '%Example Labs');
  ...
  FOR i IN 1..array_length(companies_seed,1) LOOP
    INSERT INTO public.companies (name, account_type, status, country, created_by, industry, website, source_upload_id, workspace_id)
    VALUES (companies_seed[i],'business','prospect','GB', uid,'Internal QA','https://example.com', upload_id, qa_ws)
  ...
  FOR i IN 1..16 LOOP
    INSERT INTO public.contacts (
      ...
      quality_status, source_upload_id, workspace_id, country, job_title
    )
    VALUES (
      ...
      upload_id, qa_ws, 'GB','Head of Example'
    )
    RETURNING id INTO tmp;
